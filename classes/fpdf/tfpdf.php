...
function tFPDF($orientation='P', $unit='mm', $size='A4')
{
	...
	// Page sizes
	$this->StdPageSizes = array('a0'=>array(2383.94,3370.39), 'a1'=>array(1683.78,2383.94), 'a2'=>array(1190.55,1683.78), 'a3'=>array(841.89,1190.55), 'a4'=>array(595.28,841.89), 'a5'=>array(420.94,595.28), 'a6'=>array(230.47,595.28), 'a7'=>array(209.76,297.64),
		'letter'=>array(612,792), 'legal'=>array(612,1008), 'manual'=>array(227 , 550));
	...
	if ($uni) {
		...
		if (!isset($type) ||  !isset($name) || ($originalsize != $ttfstat['size'])) {
			...
			/*if (!class_exists('TTFontFile', false)) {
				include($this->_getfontpath().'unifont/ttfonts.php'); 
			}*/
			if(defined("_SYSTEM_TTFONTS"))
				include_once(_SYSTEM_TTFONTS.'ttfonts.php');
			else
				include_once($this->_getfontpath().'unifont\ttfonts.php');
...
function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='')	//(width, height, TEXT, border, \n, align)
{
	// Output a cell
	$k = $this->k;
	...
	if($txt!=='')
	{
		...
		elseif($align=='C')
			...
			$dx = $this->GetStringWidth($txt)-$this->GetStringWidth($txt)+$this->cMargin;				//$this->cMargin;
...
function BasicTable($header,$data) { 
	...
	foreach($header as $col) 
		$this->Cell(40,7,$col,1,1,'C'); 
	$this->Ln(); 
	//Data 
	foreach($data as $row) 
	{ 
		foreach($row as $col) 
		$this->Cell(40,6,$col,1,1,'C'); 
		$this->Ln(); 
	} 
} 
...
//border: whether to draw the border of the box. Default value: 1
...
//--------------------------------------------FontStyle--------------------------------------------------------
function UnderlineText($setflag)
{
	if (($setflag == true) || ($setflag == false))
		$this->underline = $setflag;
}

function Cell2Col($w_col1,$w,$txt_1,$txt_2,$border=0,$align_col1='L',$align_col2='L'){
	$this->Cell($w_col1,$w,$txt_1,$border,0,$align_col1);
	$this->Cell(0,$w,$txt_2,$border,1,$align_col2);
}

function DrawRect($x=0, $y=0, $w=0, $h=0, $count_i=0) {
	for($i=0;$i<$count_i;$i++) 
	{
		$this->Rect($x,$y,$w,$h);
		$x=$x+$w+1;
