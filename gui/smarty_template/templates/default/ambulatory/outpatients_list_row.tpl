...
{{* This template is used by /modules/nursing/nursing_station.php to populate the ward_occupancy_list.tpl template *}}
 ...
 {{/if}}
 		<td><input type="checkbox" name='{{$sCheckName}}' value="{{$sEnNr}}"></td>
		<td><font {{$sTimeFontClass}}>{{$sDate}} {{$sTime}}</font></td>
		...
		<td>&nbsp;{{$sAdmitDataIcon}} {{$sChartFolderIcon}} {{$sNotesIcon}} <br /> {{$sTransferIcon}} {{$sBillIcon}} {{$sDischargeIcon}} {{$sPrintPdf}}</td>
	</tr>
	<tr>
