...
class Measurement extends Core {
	...
	*/
	var $tb_encounter='care_encounter';//add by vy
	var $result;
	/**
	...
	*/
	...
	/*dd by vy 9/11/2011*/
	function getWeight($enc_nr=''){
	global $db;
	$this->enc_nr=$enc_nr;
	$this->sql="SELECT value FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr' AND msr_type_nr ='6' AND msr_date=(SELECT max(msr_date) FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr') AND msr_time=(SELECT max(msr_time) FROM care_encounter_measurement WHERE msr_date=(SELECT max(msr_date) FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr') and encounter_nr='$this->enc_nr')";
	//echo $this->sql;
	...
	function getHeight($enc_nr=''){
	global $db;
	$this->enc_nr=$enc_nr;
	$this->sql="SELECT value FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr' AND msr_type_nr ='7' AND msr_date=(SELECT max(msr_date) FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr') AND msr_time=(SELECT max(msr_time) FROM care_encounter_measurement WHERE msr_date=(SELECT max(msr_date) FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr') and encounter_nr='$this->enc_nr')";
	//echo $this->sql;
	if($buf=$db->Execute($this->sql)) {
	if($buf->RecordCount()) {
			   $buf2=$buf->FetchRow();
	...
	function getTemper($enc_nr=''){
	global $db;
	$this->enc_nr=$enc_nr;
	//echo $this->sql;
	$this->sql="SELECT value FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr' AND msr_type_nr ='3' AND msr_date=(SELECT max(msr_date) FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr') AND msr_time=(SELECT max(msr_time) FROM care_encounter_measurement WHERE msr_date=(SELECT max(msr_date) FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr') and encounter_nr='$this->enc_nr')";
	if($buf=$db->Execute($this->sql)) {
	if($buf->RecordCount()) {
			   $buf2=$buf->FetchRow();
				return $buf2['value'];
	...
	function getBloodPressure($enc_nr=''){
	global $db;
	$this->enc_nr=$enc_nr;
	$this->sql="SELECT value FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr' AND msr_type_nr ='1' AND msr_date=(SELECT max(msr_date) FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr') AND msr_time=(SELECT max(msr_time) FROM care_encounter_measurement WHERE msr_date=(SELECT max(msr_date) FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr') and encounter_nr='$this->enc_nr')";
	//echo $this->sql;
	if($buf=$db->Execute($this->sql)) {
	if($buf->RecordCount()) {
			   $buf2=$buf->FetchRow();
	...
	function getMach($enc_nr=''){
	global $db;
	$this->enc_nr=$enc_nr;
	$this->sql="SELECT value FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr' AND msr_type_nr ='2' AND msr_date=(SELECT max(msr_date) FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr') AND msr_time=(SELECT max(msr_time) FROM care_encounter_measurement WHERE msr_date=(SELECT max(msr_date) FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr') and encounter_nr='$this->enc_nr') ";
	if($buf=$db->Execute($this->sql)) {
	if($buf->RecordCount()) {
			   $buf2=$buf->FetchRow();
				return $buf2['value'];
	...
	function getNhiptho($enc_nr=''){
	global $db;
	$this->enc_nr=$enc_nr;
	$this->sql="SELECT value FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr' AND msr_type_nr ='10' AND msr_date=(SELECT max(msr_date) FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr') AND msr_time=(SELECT max(msr_time) FROM care_encounter_measurement WHERE msr_date=(SELECT max(msr_date) FROM care_encounter_measurement WHERE encounter_nr='$this->enc_nr') and encounter_nr='$this->enc_nr')";
	if($buf=$db->Execute($this->sql)) {
	if($buf->RecordCount()) {
			   $buf2=$buf->FetchRow();
				return $buf2['value'];
...
function increaseUnitHit($nr,$count='1'){
	...
	*/
	...
	var $temptabfields=array('nr',
									...
									'create_id',
									'create_time');
	var $tempTable = "dfck_person_measurement";
	/**
	* Constructor
	*/
	function setTempTable(){
		$this->setTable($this->tempTable);
		$this->setRefArray($this->temptabfields);
	}
}
?>
