...
class Ward extends Encounter {
	...
	*/
	...
	var $tb_type_ward = 'care_type_ward';
	...
	*/
	...
	function getWardTypeList($nr){
		global $db;
		$this->sql="SELECT nr, type, name, description FROM $this->tb_type_ward ";
		if($nr != NULL) $this->sql .= " where nr=$nr";
		if($this->res['gwtl']=$db->Execute($this->sql)) {
            if($this->rec_count=$this->res['gwtl']->RecordCount()) {
				 return $this->res['gwtl'];	 
			} else { return false; }
		} else { return false; }
	}
	
	function getRoomTypeList($nr=NULL){
		global $db;
		$this->sql="SELECT nr, type, name,LD_var, description FROM $this->tb_type_room ";
		if($nr != NULL) $this->sql .= " where nr=$nr";
             //   echo $this->sql;
		if($this->res['grtl']=$db->Execute($this->sql)) {
            if($this->rec_count=$this->res['grtl']->RecordCount()) {
				 return $this->res['grtl'];	 
			} else { return false; }
		} else { return false; }
	}
	
	/**
	* Get the department number of Ward number.
	* 
	* @access public
	*/
	function getDeptNr() {
	    global $db;
		if(!$this->ward_nr) return false;
	    $this->sql="SELECT dept_nr  FROM $this->tb_ward WHERE nr=$this->ward_nr";
        if($this->res['gdnr']=$db->Execute($this->sql)) {
            if($this->rec_count=$this->res['gdnr']->RecordCount()) {
				$row = $this->res['gdnr']->FetchRow();
				return	$row['dept_nr']; 
			} else { return false; }
		} else { return false; }
	}
	/**
         *  Get max Room_nr of ward from care_room
         *  
         */
        function getMaxRoomNr($ward_nr){
            global $db;
            if(!ward_nr) return 0;
            $this->sql="select max(room_nr) max_room_nr from care_room where ward_nr = $ward_nr";
            if($rs = $db->Execute($this->sql)){
                $row=$rs->FetchRow();
                return $row['max_room_nr'];
            }
            else return 0;
	...
	* @author Gjergj Sheldija
	*/
	function getAllWardsItemsObjectofDept($items, $dept_nr='0') {
	    global $db;
		$tmp = "";
		$this->sql="SELECT ";
		...
		$this->sql.=" FROM $this->tb_ward WHERE status NOT IN ($this->dead_stat)";
		if(isset($dept_nr)) 
			$this->sql.=" AND ( dept_nr = $dept_nr);";
        if($this->res['gawio']=$db->Execute($this->sql)) {
            if($this->rec_count=$this->res['gawio']->RecordCount()) {
				 return $this->res['gawio'];	 
			} else { return false; }
	...
	*/
	function getAllWardsItemsArray(&$items) {
	    ...
	    $this->sql.=substr($tmp,0,-2);
	    $this->sql .=" FROM $this->tb_ward WHERE  status NOT IN ('hidden','deleted','closed','inactive')";
        if($this->result=$db->Execute($this->sql)) {
            if($this->result->RecordCount()) {
	...
	function getWardsItemsArray(&$items) {
	    global $db;
		$this->sql="SELECT ";
		if(isset($items)&&!empty($items)){
			while (list($key, $val) = each($items)) {
	    ...
	    $this->sql.=substr($tmp,0,-2);
	    $this->sql .=" FROM $this->tb_ward WHERE (nr=$this->ward_nr) AND (status NOT IN ('hidden','deleted','closed','inactive'))";
        if($this->result=$db->Execute($this->sql)) {
            if($this->result->RecordCount()) {
	...
	*/
	function getWardInfo($ward_nr){
		global $db;
		$this->sql="SELECT w.*,d.nr AS dept_nr,d.name_formal AS dept_name, LD_var AS \"LD_var\" FROM $this->tb_ward AS w LEFT JOIN $this->tb_dept AS d ON w.dept_nr=d.nr
	...
	function _getWardOccupants($ward_nr,$date){
		...
		$this->sql="SELECT r.location_nr AS room_nr,
							...
							FROM $this->tb_location AS r 
									...
									LEFT JOIN $this->tb_ic AS i ON p.insurance_class_nr=i.class_nr
	...
	*/
	function updateWard($nr,&$data){
		...
		if(isset($this->data_array['is_temp_closed'])) $this->data_array['is_temp_closed'] = 0;
		if(isset($this->data_array['date_close'])) $this->data_array['date_close'] = '0000-00-00';
		if(isset($this->data_array['status'])) $this->data_array['status'] = null;
	...
	function getAllActiveWards() {
	    ...
	    $this->sql="SELECT w.*,d.name_formal AS dept_name
						FROM $this->tb_ward AS w 
				 		...
				 		WHERE w.status NOT IN ('closed','inactive','void','hidden','deleted') ORDER BY dept_name";
        //echo $this->sql;
        if($this->result=$db->Execute($this->sql)) {
            if($this->result->RecordCount()) {
				 return $this->result;	 
	...
	*/
	...
	function getRoomsNumber($nr){
	    global $db;
		$this->sql="SELECT room_nr  FROM $this->tb_room WHERE nr=$nr AND status NOT IN ('closed','inactive','void','hidden','deleted')";
        if($this->result=$db->Execute($this->sql)) {
            if($this->result->RecordCount()) {
				 return $this->result->FetchRow();
			} else { return false; }
	...
	function RoomExists($room_nr=0,$ward_nr=0){
		...
		$this->sql="SELECT room_nr FROM $this->tb_room 
							WHERE ward_nr=$this->ward_nr 
								AND room_nr=$room_nr 						
								";
              //AND date_close = '$dbf_nodate' AND status NOT IN ($this->dead_stat)
        if($this->result=$db->Execute($this->sql)) {
            if($this->result->RecordCount()) {
				 return true;
			} else { return false; }
	...
	*/
	function _getActiveRoomInfo($room_nr=0,$ward_nr=0){            
        ...
        function _getAllRoomsInfo($room_nr=0,$ward_nr=0){            
	    global $db,$dbf_nodate;
	    //$db->debug=1;
		if($ward_nr) $this->ward_nr=$ward_nr;
			elseif(!$this->ward_nr) return false;
	...
	function _getItemsActiveRoomInfo($items, $room_nr=0,$ward_nr=0){
	    global $db,$dbf_nodate;
	    //$db->debug=1;
		if($ward_nr) $this->ward_nr=$ward_nr;
			elseif(!$this->ward_nr) return false;
	...
	* @return mixed adodb record object or boolean
	*/
        function getAllRoomsInfo($ward_nr=0){
        if($this->_getAllRoomsInfo(0,$ward_nr)) {
			return $this->result;
		} else { return false; }
	}
	...
	function getAllItemsActiveRoomsInfo($items, $ward_nr=0){
        if($this->_getItemsActiveRoomInfo($items,0,$ward_nr)) {            
			return $this->result;
		} else { return false; }
	}
	/** 
	...
	*/
	...
	/**
	* Counts and returns the number of patients are in this room.
	* @access public
	* @param int Ward number, Room number
	* @return return mixed interger or boolean
	*/
	function countPatients($room_nr = 0){
	    global $db;
		if(!$room_nr) return false;
		if(!$this->ward_nr) return false;
		$this->sql="SELECT COUNT(encounter_nr) AS nr 
					FROM $this->tb_encounter 
					WHERE current_ward_nr=$this->ward_nr 
							AND current_room_nr=$room_nr 
							AND is_discharged=0;";
                //echo $this->sql;
		if($buf=$db->Execute($this->sql)) {
            if($buf->RecordCount()) {
				$row=$buf->FetchRow();
				if($row['nr'] == 0) $row['nr']= -1;
				return $row['nr'];
			} else { return false; }
		} else { return false; }
	}
	...
	/**
	* Close a room temperately.
	* @access public
	* @param int Ward number, Room number
	* @return return boolean
	*/
	function tempcloseRoom($room_nr = 0, $dept_nr){
	    global $db;
		global $_SESSION;
		if(!$room_nr) return false;
		if(!$this->ward_nr) return false;
		$roomdata['date_close']=date('Y-m-d');
		$roomdata['status']='inactive';
		$roomdata['history']="CONCAT(history,'Closed temporary: ".date('Y-m-d H:i:s')." ".$_SESSION['sess_user_name']."\n')";
		$roomdata['modify_id']=$_SESSION['sess_user_name'];
		$roomdata['modify_time'] = date('Y-m-d H:i:s');
		$roomdata['is_temp_closed'] = 1; 
		$this->sql = "UPDATE $this->tb_room 
						SET date_close = '".$roomdata['date_close']."', 
							status = 'inactive', 
							is_temp_closed = 1, 
							history = ".$roomdata['history'].",
							modify_id = '".$roomdata['modify_id']."',
							modify_time = '".$roomdata['modify_time']."' 
						WHERE ward_nr=$this->ward_nr 
							AND room_nr=$room_nr 
							";
		$this->where='';
        ...
        function cot_delRoom($nr){
            global $db;
            $this->sql="delete from $this->tb_room where nr=$nr";
            return $this->Transact($this->sql);
        }
        function cot_reopenRoom($nr){
            global $db;
		global $_SESSION;
		$roomdata['date_close']='0000-00-00';
		$roomdata['status']='';
		$roomdata['history']="CONCAT(history,'Reopen : ".date('Y-m-d H:i:s')." ".$_SESSION['sess_user_name']."\n')";
		$roomdata['modify_id']=$_SESSION['sess_user_name'];
		$roomdata['modify_time'] = date('Y-m-d H:i:s');
		$roomdata['is_temp_closed'] = 0; 
		$this->sql = "UPDATE $this->tb_room 
						SET date_close = '".$roomdata['date_close']."', 
							status = '".$roomdata['status']."', 
							is_temp_closed = '".$roomdata['is_temp_closed']."', 
							history = ".$roomdata['history'].",
							modify_id = '".$roomdata['modify_id']."',
							modify_time = '".$roomdata['modify_time']."' 
						WHERE nr=$nr							
	...
	function cot_closeRoom($nr){
        ...
        function cot_DecRoomCountInWard($nr){
            global $db;
            $this->sql="update $this->tb_ward set room_nr_end=(room_nr_end - 1) where nr =$nr";
            //echo $this->sql;
            return $this->Transact($this->sql);
        }
        function cot_updateWardCountRoom($ward_nr){
            global $db;
            $this->sql="update $this->tb_ward set room_nr_start=1, room_nr_end=(select count(nr) from $this->tb_room where ward_nr=$ward_nr) where nr =$ward_nr";
            //echo $this->sql;
            return $this->Transact($this->sql);
        }
	/**
	...
	* @access public
	* @param int Dept number, Ward number, Room number, number of bed
	* @return return boolean
	*/
	function updateRoomBeds($dept_nr, $room_nr, $beds, $roomname, $info,$type_nr,$roomid,$isclose){
	    global $db;
		global $_SESSION;
		if(!$room_nr) return false;
		if(!$this->ward_nr) return false;
		...
		$roomdata['history']="CONCAT(history,'Update: ".date('Y-m-d H:i:s')." ".$_SESSION['sess_user_name']."\n')";
		$roomdata['modify_id']=$_SESSION['sess_user_name'];
		$roomdata['modify_time'] = date('Y-m-d H:i:s');
		...
		$this->sql = "UPDATE $this->tb_room 
						SET nr_of_beds = $beds, 
							...
							modify_time = '".$roomdata['modify_time']."' ,
                                                        type_nr = '$type_nr',
                                                        status = '".$roomdata['status']."',
                                                        is_temp_closed = '".$isclose."',
                                                        date_close='".$roomdata['date_close']."',
                                                        room_nr = '$room_nr'
						WHERE  nr='$roomid' 
                        "; 							
		$this->where='';
	...
	* @return return boolean
	*/
	function createRoom($dept_nr, $room_nr, $beds, $roomname, $info,$type_nr){
		// Set the values common to all rooms 
		$roomdata['date_create']=date('Y-m-d');
		$roomdata['history']="Created: ".date('Y-m-d H:i:s')." ".$_SESSION['sess_user_name']."\n";
		//$roomdata['modify_id']=$_SESSION['sess_user_name'];
		$roomdata['create_id']=$_SESSION['sess_user_name'];
		$roomdata['create_time']=date('YmdHis');
		//$db->debug=1;
		if(!$this->RoomExists($room_nr)){
			$roomdata['dept_nr']=$dept_nr;
			$roomdata['room_nr']=$room_nr;
			$roomdata['roomname']=$roomname;
			$roomdata['nr_of_beds']=$beds;
			$roomdata['info']=$info;
			$roomdata['ward_nr'] = $this->ward_nr;
                        $roomdata['type_nr'] = $type_nr;                        
                        
			$this->saveWardRoomInfoFromArray($roomdata);
		}
	}
	...
	*/
	function createWaitingInpatientList($ward_nr=0){
		global $db;
		if($ward_nr) $cond="AND e.current_ward_nr='$ward_nr'";
			else $cond='';
		...
		$this->sql="SELECT e.encounter_nr, e.encounter_class_nr, e.current_ward_nr, p.pid, p.name_last, p.name_first, p.date_birth, p.sex,w.ward_id
				...
				WHERE e.encounter_class_nr='1' AND  e.is_discharged IN ('',0) $cond AND  e.in_ward IN ('',0)";
		//echo $this->sql;
	...
	*/
	...
	/** Tuyen 18/11/11
	* Returns all wards of a department
	* The returned adodb record object contains rows of arrays /or boolean
	* @access public
	* @param int Primary record key number
	*/
	function getAvaiWardOfDept($dept_nr=0){
	    global $db;
	    if(!$dept_nr) return FALSE;
		$this->sql="SELECT * FROM $this->tb_ward WHERE dept_nr='$dept_nr' AND status NOT IN ('closed','inactive','void','hidden','deleted')
							 ORDER BY ward_id";
        if($this->result=$db->Execute($this->sql)) {
            if($this->result->RecordCount()) {
				 return $this->result;
	...
	function checkWardExist($dept_nr, $ward_name, $ward_id){
		global $db;
	    if(!$dept_nr) return FALSE;
		if(!$ward_name) return FALSE;
		$this->sql="SELECT nr FROM $this->tb_ward WHERE (dept_nr=$dept_nr) AND (name='$ward_name' OR ward_id='$ward_id')";
        if($this->result=$db->Execute($this->sql)) {
            if($this->result->RecordCount()) {
				 return true;
			} else { return false; }
	...
	//add 11-10 cot
	function getDeptFromWard($wardnr){
        	global $db;
            $this->sql="SELECT w.dept_nr FROM care_ward w WHERE w.nr = $wardnr";
        //    var_dump($this->sql);
            if($this->result=$db->Execute($this->sql)){
                if($query=$this->result->RecordCount()){
