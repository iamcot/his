...
class Paginator {
	...
	*/
	function makePrevLink($txt='',$append='',$var){
		if (!$this->csx){
			return '';
		}else{
			$x=$this->prevIndex();
			...
			return '<a href="'.$this->page.URL_APPEND.$var.'&mode=paginate&pgx='.$x.'&totalcount='.$this->tcount.'&oitem='.$this->sort_item.'&odir='.$this->sort_dir.$append.'"><< '.$txt.'</a>';
		}
	}
	/**
	...
	*/
	function makeNextLink($txt='',$append='',$var){
		$x=$this->nextIndex();
		if ($x){
	...
	*/
	function SortLink($txt,$item,$dir,$flag=FALSE,$append='',$var){
		...
		if($flag){
			...
			return '<a href="'.$this->page.URL_APPEND.'&mode=paginate&pgx='.$this->csx.'&totalcount='.$this->tcount.'&oitem='.$item.'&odir='.$dir.$append.$var.'">'.$img.$txt.'</a>';
	}
	/**
