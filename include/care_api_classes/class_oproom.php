...
class OPRoom extends Core {
	...
	*/
	function CombinedORNrs(){
        ...
        function selectRoom($dept_nr='', $ward_nr=''){
            global $db;
            $this->res['aaoi']=array();
			$this->sql="SELECT nr
						FROM $this->tb_room
        ...
        function insertTime($enc_nr=""){
            global $db;
            $this->sql="UPDATE $this->tb_encounter_op SET timestart='".date('h:i:s')."' WHERE encounter_nr='$enc_nr' AND (status='pending' OR status='received')";
            if ($db->Execute($this->sql)) {
                return $db->Execute($this->sql);
            }
        }
		
        //chọn phòng khi xếp lập ê-kíp mổ
        function updateOP($batch_nr='',$saal='',$date,$hour,$minute){
            global $db;
            if(empty ($saal)) $saal='0';
            $this->sql="UPDATE $this->tb_encounter_op SET op_room=$saal,doc_date='$date',doc_time='".$hour.":".$minute."' WHERE batch_nr=$batch_nr";
            if ($this->res['oi']=$db->Execute($this->sql)) {
                return TRUE;
            }else{return false;}
        }
		
        //Cập nhật trạng thái của phòng hiện tại sau khi lên ê-kíp mổ
        function SetCloseRoom($saal,$flag){
            global $db;
            $this->sql="UPDATE $this->tb_room SET is_temp_closed=".$flag.",
                                history=".$this->ConcatHistory('Update: '.date('Y-m-d H:i:s').' '.$_SESSION['sess_user_name'].' closed='.$flag."\n").",
                                modify_id='".$_SESSION['sess_user_name']."',
                                modify_time='".date('Y-m-d H:i:s')."'
                                WHERE nr='$saal'";
            if ($this->res['oi']=$this->Transact($this->sql)) {
                return $this->res['oi'];
            }else{return false;}
        }
        ...
        function getNameUser(){
            global $db;
            $this->sql="SELECT name FROM $this->tb_user WHERE login_id='".$_SESSION['sess_user_name']."'";
            if ($this->res['aaoi']=$db->Execute($this->sql)) {
                if ($this->res['aaoi']->RecordCount()) {
                    return $this->res['aaoi']->FetchRow();
                    }else{return false;}
            }else{return false;}
        }
	}
