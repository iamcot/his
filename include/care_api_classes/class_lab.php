...
class Lab extends Encounter {
    ...
    function getBatchResult1($bn) {
        ...
        $this->sql = "SELECT * FROM $this->tb_find_chemlab INNER JOIN $this->tb_find_chemlab_sub ";
        $this->sql .= "ON ($this->tb_find_chemlab.job_id = $this->tb_find_chemlab_sub.job_id) ";
        $this->sql .= "WHERE $this->tb_find_chemlab.job_id=$bn";
        //echo $this->sql;
        if ($this->result = $db->Execute($this->sql)) {
            if ($this->rec_count = $this->result->RecordCount()) {
                return $this->result;
    ...
    // thong ke khoa xet nghiem
    function countHoasinh($startday, $endday) {
        global $db;
        $this->sql = "select count(job_id) from care_test_findings_chemlab where status='done' and test_date <= '$endday' and test_date >= '$startday' ";
        //echo $this->sql;
        if ($buf = $db->Execute($this->sql)) {
            if ($buf->RecordCount()) {
                $buf2 = $buf->FetchRow();
                return $buf2['count(job_id)'];
            } else {
                return FALSE;
            }
        } else {
    ...
    function countPTM($startday, $endday) {
        global $db;
        $this->sql = "select count(job_id) from care_test_findings_blood where status='done' and test_date <= '$endday' and test_date >= '$startday'";
         if ($buf = $db->Execute($this->sql)) {
            if ($buf->RecordCount()) {
                $buf2 = $buf->FetchRow();
                return $buf2['count(job_id)'];
    ...
    function countHIV($startday,$enday){
        global $db;
        $this->sql="select count(fo.batch_nr) from care_test_findings_other as fo left join care_test_request_other as tro on(fo.batch_nr=tro.batch_nr) 
                            where fo.status='done' and fo.findings_date<='$endday' and fo.findings_date >='$startdat' and tro.other_type='hiv'";
         if ($buf = $db->Execute($this->sql)) {
            if ($buf->RecordCount()) {
                $buf2 = $buf->FetchRow();
                return $buf2['count(fo.batch_nr)'];
            } else {
                return FALSE;
            }
        } else {
