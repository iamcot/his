...
class Address extends Core {
	...
	*/
	...
	function _useQuanHuyen(){
		$this->coretable=$this->tb_quanhuyen;
		$this->ref_array=$this->fld_quanhuyen;
	}
	
	function _usePhuongXa(){
		$this->coretable=$this->tb_phuongxa;
		$this->ref_array=$this->fld_phuongxa;
	}
	/**
	...
	*/
	function getAllActiveCityTown(){
		...
		*/
	    global $db;
		$this->sql="SELECT * FROM $this->tb_citytown WHERE status NOT IN ('inactive','hidden','deleted','void') order by use_frequency DESC"; //edit 0310 - cot
	...
	*/
	...
	//add by by 1-12
	function getLimitActiveQuanHuyen($len=30,$so=0,$oitem='name',$odir='ASC'){
		/**
		* @global ADODB-db-link
		*/
	    global $db;
		$this->sql="SELECT * FROM $this->tb_quanhuyen WHERE status NOT IN ('inactive','hidden','deleted','void') ORDER BY $oitem $odir";
	    if ($this->res['glaqh']=$db->SelectLimit($this->sql,$len,$so)) {
		    if ($this->rec_count=$this->res['glaqh']->RecordCount()) {
		        return $this->res['glaqh'];
			}else{return FALSE;}
		}else{return FALSE;}
	}
	function getLimitActivePhuongXa($len=30,$so=0,$oitem='name',$odir='ASC'){
		/**
		* @global ADODB-db-link
		*/
	    global $db;
		$this->sql="SELECT * FROM $this->tb_phuongxa WHERE status NOT IN ('inactive','hidden','deleted','void') ORDER BY $oitem $odir";
	    if ($this->res['glapx']=$db->SelectLimit($this->sql,$len,$so)) {
		    if ($this->rec_count=$this->res['glapx']->RecordCount()) {
		        return $this->res['glapx'];
			}else{return FALSE;}
		}else{return FALSE;}
	}
	/**
	...
	*/
	...
	//add by vy 1-12
	function countAllActiveQuanHuyen(){
		/**
		* @global ADODB-db-link
		*/
	    global $db;
		$this->sql="SELECT nr FROM $this->tb_quanhuyen WHERE status NOT IN ($this->dead_stat)";
	    if ($this->res['caaqh']=$db->Execute($this->sql)) {
		    return $this->res['caaqh']->RecordCount();
		}else{return 0;}
	}
	function countAllActivePhuongXa(){
		/**
		* @global ADODB-db-link
		*/
	    global $db;
		$this->sql="SELECT nr FROM $this->tb_phuongxa WHERE status NOT IN ($this->dead_stat)";
	    if ($this->res['caapx']=$db->Execute($this->sql)) {
		    return $this->res['caapx']->RecordCount();
		}else{return 0;}
	}
	/**
   ...
   function QuanHuyenExists($name='',$citytown='') {
		...
		*/
	    global $db, $sql_LIKE;
	    if(empty($name)) return FALSE;
		$this->sql="SELECT nr FROM $this->tb_quanhuyen WHERE name $sql_LIKE '$name' AND citytown_id $sql_LIKE '$citytown'";
	    if($buf=$db->Execute($this->sql)) {
	        if($buf->RecordCount()) {
			    return TRUE;
		    } else { return FALSE; }
    ...
    function PhuongXaExists($name='',$quanhuyen='') {
		...
		*/
	    global $db, $sql_LIKE;
	    if(empty($name)) return FALSE;
		$this->sql="SELECT nr FROM $this->tb_phuongxa WHERE name $sql_LIKE '$name' AND quanhuyen_id $sql_LIKE '$quanhuyen'";
	    if($buf=$db->Execute($this->sql)) {
	        if($buf->RecordCount()) {
			    return TRUE;
		    } else { return FALSE; }
   ...
   function getQuanHuyenInfo($nr='') {
		...
		*/
	    ...
	    //$db->debug=true;
	    if(empty($nr)) return FALSE;
		$this->sql="SELECT * FROM $this->tb_quanhuyen WHERE nr=$nr";
	    if($this->res['gqhi']=$db->Execute($this->sql)) {
	        if($this->res['gqhi']->RecordCount()) {
			    return $this->res['gqhi'];
		    } else { return FALSE; }
	   } else { return FALSE; }
   }
   function getPhuongXaInfo($nr='') {
		/**
		* @global ADODB-db-link
		*/
	    global $db;
	    //$db->debug=true;
	    if(empty($nr)) return FALSE;
		$this->sql="SELECT * FROM $this->tb_phuongxa WHERE nr=$nr";
	    if($this->res['gpxi']=$db->Execute($this->sql)) {
	        if($this->res['gpxi']->RecordCount()) {
			    return $this->res['gpxi'];
		    } else { return FALSE; }
	   } else { return FALSE; }
   }
	/**
	...
	*/
	...
	//add by vy 1-12
	function saveQuanHuyenInfoFromArray(&$data){
		global $_SESSION;
		$this->_useQuanHuyen();
		$this->data_array=$data;
		$this->data_array['status']='normal';
	...
	function savePhuongXaInfoFromArray(&$data){
		global $_SESSION;
		$this->_usePhuongXa();
		$this->data_array=$data;
		$this->data_array['status']='normal';
	...
	*/
	...
	function updateQuanHuyenInfoFromArray($nr,&$data){
		global $_SESSION;
		$this->_useQuanHuyen();
		$this->data_array=$data;
		// remove probable existing array data to avoid replacing the stored data
	...
	function updatePhuongXaInfoFromArray($nr,&$data){
		global $_SESSION;
		$this->_usePhuongXa();
		$this->data_array=$data;
		// remove probable existing array data to avoid replacing the stored data
	...
	*/
	...
	//add by vy 1-12
	function searchLimitActiveQuanHuyen($key,$len=30,$so=0,$oitem='name',$odir='ASC'){
		/**
		* @global ADODB-db-link
		*/
		global $db, $sql_LIKE;
		...
		$select="SELECT q.*  FROM $this->tb_quanhuyen AS q,$this->tb_citytown AS c ";
		$append=" ORDER BY q.name $odir";
		
		$this->sql="$select WHERE(q.citytown_id= c.nr) AND ((q.name $sql_LIKE '%$key%' ) OR(c.name $sql_LIKE '%$key%' )) $append";
		
		if($this->res['slaqh']=$db->SelectLimit($this->sql,$len,$so)){
			if($this->rec_count=$this->res['slaqh']->RecordCount()){
				return $this->res['slaqh'];
		    }else{
				$this->sql="$select WHERE(q.citytown_id= c.nr) AND ((q.name $sql_LIKE '%$key%' ) OR(c.name $sql_LIKE '%$key%' )) $append";
				if($this->res['slaqh']=$db->SelectLimit($this->sql,$len,$so)){
					if($this->rec_count=$this->res['slaqh']->RecordCount()){
	...
	function searchLimitActivePhuongXa($key,$len=30,$so=0,$oitem='name',$odir='ASC'){
		/**
		* @global ADODB-db-link
		*/
		global $db, $sql_LIKE;
        //$db->debug=true;
		if(empty($key)) return FALSE;
		$select="SELECT p.*  FROM $this->tb_phuongxa AS p,$this->tb_quanhuyen AS q,$this->tb_citytown AS c ";
		$append=" ORDER BY p.name $odir";
		$this->sql="$select WHERE(p.quanhuyen_id= q.nr)  AND (p.citytown_id=c.nr) AND ((p.name $sql_LIKE '%$key%' ) OR(q.name $sql_LIKE '%$key%' ) OR(c.name $sql_LIKE '%$key%' )) $append";
		if($this->res['slapx']=$db->SelectLimit($this->sql,$len,$so)){
			if($this->rec_count=$this->res['slapx']->RecordCount()){
				return $this->res['slapx'];
		    }else{
				$this->sql="$select WHERE(p.quanhuyen_id= q.nr) AND (p.citytown_id=c.nr) AND ((p.name $sql_LIKE '%$key%' ) OR(q.name $sql_LIKE '%$key%' )  OR(c.name $sql_LIKE '%$key%' )) $append";
				if($this->res['slapx']=$db->SelectLimit($this->sql,$len,$so)){
					if($this->rec_count=$this->res['slapx']->RecordCount()){
	...
	*/
	...
	//add by vy 1-12
	function searchCountActiveQuanHuyen($key){
		/**
		* @global ADODB-db-link
		*/
		global $db, $sql_LIKE;
		if(empty($key)) return FALSE;
		$select="SELECT nr FROM $this->tb_quanhuyen ";
		$append=" AND status NOT IN ($this->dead_stat)";
		$this->sql="$select WHERE ( name $sql_LIKE '$key%') $append";
		if($this->res['scaqh']=$db->Execute($this->sql)){
			if($this->rec_count=$this->res['scaqh']->RecordCount()){
				return $this->rec_count;
			}else{	
				$this->sql="$select WHERE ( name $sql_LIKE '%$key') $append";
				if($this->res['scaqh']=$db->Execute($this->sql)){
	...
	function searchCountActivePhuongXa($key){
		/**
		* @global ADODB-db-link
		*/
		global $db, $sql_LIKE;
		if(empty($key)) return FALSE;
		$select="SELECT nr FROM $this->tb_phuongxa ";
		$append=" AND status NOT IN ($this->dead_stat)";
		$this->sql="$select WHERE ( name $sql_LIKE '$key%') $append";
		if($this->res['scapx']=$db->Execute($this->sql)){
			if($this->rec_count=$this->res['scapx']->RecordCount()){
				return $this->rec_count;
			}else{	
				$this->sql="$select WHERE ( name $sql_LIKE '%$key') $append";
				if($this->res['scapx']=$db->Execute($this->sql)){
