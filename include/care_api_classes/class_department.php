...
class Department extends Core {
	...
	*/
	...
	function getAllActiveNoSubDeptSort($sort='') {
		return $this->_getalldata("is_sub_dept='0' AND is_inactive='0'",$sort);
	}
	/**
	...
	*/
	function getAllPharmacy() {
        ...
        /**
         * Gets all ACTIVE dept 
         */
        function cot_getAllDept() {
		return $this->_getalldata("is_inactive='0'");
	}
	/**
	...
	*/
	function getAllActiveORNrs($dept_nr){
		global $db;
	...
	*/
	...
	function checkDepartmentExist($formal_name, $id){
		global $db;
		if(!formal_name) return FALSE;
		if(!$id) return FALSE;
		$this->sql="SELECT nr FROM care_department WHERE name_formal='$formal_name' OR id='$id'";
		if($this->result=$db->Execute($this->sql)){
			if($this->result->RecordCount()){
				return TRUE;
			}else{return FALSE;}
        ...
        function checkNameDept($nr){
            global $db;
            $this->sql="SELECT LD_var FROM care_department WHERE nr=$nr";
            if($this->result=$db->Execute($this->sql)){
                if($query=$this->result->FetchRow()){
                        return $query;
                }else{return FALSE;}
            }else{return FALSE;}
        }
		
		function getAllDept($cond){
            global $db;
            $this->sql="SELECT * FROM care_department WHERE status NOT IN ($this->dead_stat) $cond  ORDER BY nr DESC";
//            echo $this->sql;
            if($this->result=$db->Execute($this->sql)){
