...
class CabinetMedipot extends Core {
     ...
     * @return boolean
     */
    function checkExistMedicineInAvaiDept($dept,$ward,$available_product_id,$typeput){
        global $db;
        $this->sql="SELECT *
					FROM $this->tb_med_avai_dept AS dept
					WHERE
					dept.department='$dept' AND dept.ward_nr='$ward' AND dept.typeput='$typeput'
					AND dept.available_product_id='$available_product_id'";
        if($this->result=$db->Execute($this->sql)) {
            if($this->result->RecordCount()){
                return $this->result->FetchRow();
            } else { return false; }
    ...
    function insertMedicineAvaiDept($available_product_id, $dept, $ward, $number, $typeput){
        global $db;
        if($available_product_id=='') return FALSE;
...
//            $avai_id=$result1->FetchRow();
            ...
            $this->sql="INSERT INTO $this->tb_med_avai_dept (available_product_id,department,ward_nr,available_number,init_number,typeput,update_at,create_at) VALUES ('".$available_product_id."','$dept','$ward','$number','$number','$typeput',now(),now())";
            return $this->Transact($this->sql);
//        } else {return false;}
    }
     ...
     */
    function ShowDistributeCabinet($dept_nr,$contidtion="")
    {
        global $db;
...
//				".$limit_number;
    ...
    function SearchDistributeCabinet($dept_nr, $condition)
    {
        ...
        $this->sql="SELECT DISTINCT khochan.product_name, donvi.unit_name_of_medicine, khochan.product_encoder, tatcakhoa.product_lot_id, tatcakhoa.exp_date , taikhoa.*
                FROM $this->tb_med_avai_dept AS taikhoa, care_med_products_main_sub1 AS tatcakhoa, care_med_products_main AS  khochan, care_med_unit_of_medicine AS donvi, care_ward
                WHERE taikhoa.available_product_id=tatcakhoa.id
					".$dept_ward." 
                    AND khochan.product_encoder=tatcakhoa.product_encoder 
                    AND donvi.unit_of_medicine=khochan.unit_of_medicine 
    ...
    function getInfoMedInAvaiDept($IDmed){
        global $db;
        $this->sql="SELECT DISTINCT khochan.product_name, donvi.unit_name_of_medicine, khochan.product_encoder, tatcakhoa.lotid, tatcakhoa.exp_date , taikhoa.*
                FROM $this->tb_med_avai_dept AS taikhoa, care_med_products_main_sub1 AS tatcakhoa, care_med_products_main AS  khochan, care_med_unit_of_medipot AS donvi
                WHERE taikhoa.ID='$IDmed' 
				AND taikhoa.available_product_id=tatcakhoa.id
                AND khochan.product_encoder=tatcakhoa.product_encoder 
                AND donvi.unit_of_medicine=khochan.unit_of_medicine	";
    ...
    function insertArchive($dept, $ward, $product_encoder, $available_product_id, $get_use, $number, $cost=0, $issuepaper_id=0, $pres_id=0, $use_id=0, $return_id=0, $destroy_id=0, $user, $typeput){
        ...
        $this->sql="INSERT INTO $this->tb_med_archive(nr,dept_nr,ward_nr,typeput,product_encoder,get_use,number,cost,issuepaper_id,pres_id, use_id,return_id,destroy_id,at_date_time,user,available_product_id)
