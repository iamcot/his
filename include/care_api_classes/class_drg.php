...
class DRG extends Encounter{
	...
	* @var string
	*/
	var $tb_lang_icd='en,vi,de,pt-br,es,bs,bg,tr,sq';
	/**
	* Language codes that have corresponding OPS/ICPM tables
	* @var string
	*/
	var $tb_lang_icpm='en,vi,de,es,it,sq';
	/**
	* Constructor.
	*
	...
	*/
	...
	//ssss
	function Icd10Exists($name='',$country='') {
	
		/**
		* @global ADODB-db-link
		*/
	    global $db, $sql_LIKE,$lang;
		# convert the dashes to underscores in the language code
		$intlang=strtr($lang,'-','_');
		...
		$this->sql="SELECT nr FROM $this->tb_diag_codes WHERE diagnosis_code $sql_LIKE '$name'";
   ...
   $intlang=strtr($lang,'-','_');
		...
		$this->coretable=$this->tb_diag_codes;
		$this->ref_array=$this->fld_icd10;
	}
   function saveIcd10InfoFromArray(&$data){
		global $_SESSION;
		$this->_useIcd10();
	...
	*/
	...
	function getIcd10Info($diagnosis_code='') {
		/**
		* @global ADODB-db-link
		*/
		 global $lang,$db;
	    ...
	    //$db->debug=true;
	    if(empty($diagnosis_code)) return FALSE;
		$this->sql="SELECT * FROM care_icd10_vi WHERE diagnosis_code='".$diagnosis_code."' ";
	    if($this->res['gii']=$db->Execute($this->sql)) {
	        if($this->res['gii']->RecordCount()) {
			    return $this->res['gii'];
		    } else { return FALSE; }
	   } else { return FALSE; }
   }
   
   function getLimitIcd10($len=30,$so=0,$oitem='diagnosis_code',$odir='ASC'){
		/**
		* @global ADODB-db-link
		*/
	    global $lang,$db;
   $intlang=strtr($lang,'-','_');
		...
		# Check if language has a corresponding table. if not use default table
			...
			$this->tb_diag_codes='care_icd'.$this->icd_version.'_'.$intlang; # construct the code source table e.g. "care_icd10_en"
		$this->sql="SELECT * FROM $this->tb_diag_codes ORDER BY $oitem $odir";
	    if ($this->res['gli10']=$db->SelectLimit($this->sql,$len,$so)) {
		    if ($this->rec_count=$this->res['gli10']->RecordCount()) {
		        return $this->res['gli10'];
			}else{return FALSE;}
		}else{return FALSE;}
	}
	function countIcd10(){
   ...
   $intlang=strtr($lang,'-','_');
		...
		# Check if language has a corresponding table. if not use default table
			...
			$this->tb_diag_codes='care_icd'.$this->icd_version.'_'.$intlang; # construct the code source table e.g. "care_icd10_en"
		$this->sql="SELECT diagnosis_code FROM $this->tb_diag_codes";
	    if ($this->res['ci10']=$db->Execute($this->sql)) {
		    return $this->res['ci10']->RecordCount();
		}else{return 0;}
	}
	function updateIcd10InfoFromArray($diagnosis_code,&$data){
		global $_SESSION;
		$this->_useIcd10();
		$this->data_array=$data;
		...
		return $this->updateDataFromInternalArray($diagnosis_code);
	}
	//add 0810 cot
	function insertICD10Group($groupcode,$cbegin,$cend,$info){
		global $db;
		$this->sql="INSERT INTO dfck_icd10_vi_group (groupcode,cbegin,cend,`info`) VALUES ('$groupcode','$cbegin','$cend','$info')";
		$rs = $db->Execute($this->sql);
		return $rs;
	}
	function insertICD10Auto($code,$info){
		global $db;
		$sql="INSERT INTO care_icd10_vi (diagnosis_code,`description`,`type`) VALUES ('$code','$info',(SELECT groupcode FROM  dfck_icd10_vi_group WHERE cbegin <= '".$code."' AND cend >= '".$code."'))";
		//echo $sql;
		$rs = $db->Execute($sql);
		return $rs;
	}
	function getICD10Fromkey($key){
		global $db;
		$this->sql = "SELECT * FROM care_icd10_vi WHERE `description` LIKE '$key%' OR  `description` LIKE '% $key%' order by diagnosis_code";
		if($this->result=$db->Execute($this->sql)) {
		    if($this->result->RecordCount()) {
				return $this->result;
		    } else { return FALSE;}
