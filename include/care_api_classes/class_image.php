...
class Image extends Core{
	...
	*/
	function DicomImages($key=''){
		global $db, $sql_LIKE;
		$this->sql="SELECT i.*, LENGTH(i.notes) AS note_len,
		...
		$this->sql.="  i.pid=p.pid AND i.img_type='dicom' AND i.status NOT IN ('cancelled',$this->dead_stat) 
					ORDER BY p.name_last, p.name_first, i.pid, i.encounter_nr, i.upload_date ";
		//echo $this->sql;
	...
	*/
	...
	function updateImgDiagnosticData(&$data, $nr){
	    global $db;
		# set core to care_encounter_img_diagnostic
		$this->useImgDiagnostic();
		return $this->updateDataFromArray($data,$nr);
	}
	
	function deleteAllItemInFolder($dirname) 
	{
	   if (is_dir($dirname))
	   ...
	   while($file = readdir($dir_handle)) {
		  if ($file != "." && $file != "..") {
			 if (!is_dir($dirname."/".$file))
				...
				$this->deleteAllItemInFolder($dirname.'/'.$file);    
		  }
	   }
	   closedir($dir_handle);
	   //rmdir($dirname);   //ko dung duoc rmdir tren server??
	   return true;
	}
	...
	*/
	...
	function updateImgMaxNr($nr=0,$count){
	    global $db;
		if(!$nr) return false;
		$this->sql="UPDATE $this->tb_img_diag SET max_nr=max_nr+$count WHERE nr=$nr";
		return $this->Transact();
	}
	/**
	...
	*/
	...
	function SelectImgDiagInfo($nr=0){
	    global $db;
		if(!$nr) return false;
		$this->sql="SELECT * FROM $this->tb_img_diag WHERE nr=$nr";
		if($buff=$db->Execute($this->sql)) {
		    if($buff->RecordCount()) {
				return $buff->FetchRow();
		    } else { return false;}
		} else { return false;}
	}
	...
	*/
	...
	function savefindingsradiosub($encounter_nr, $batch_nr, $item_code, $path){
		//kiem tra care_test_findings_radio_sub
		global $db;
		if($batch_nr!='' && $item_code!=''){
			$this->sql="SELECT count(*) AS count FROM care_test_findings_radio_sub WHERE batch_nr='".$batch_nr."' AND item_bill_code='".$item_code."'";	
			if($result_sub=$db->Execute($this->sql)){
				$temp=$result_sub->FetchRow();
				if($temp['count']>0) 
					$_exist_sub=true;
			...
			if ($_exist_sub){
				$this->sql="UPDATE care_test_findings_radio_sub 
							SET
							img_path = '".$path."'   
							WHERE batch_nr = '".$batch_nr."' AND item_bill_code = '".$item_code."' ";
			}else{
				$this->sql="INSERT INTO care_test_findings_radio_sub 
							(sub_id, batch_nr, item_bill_code, kq_sieuam, encounter_nr, img_path, img_name)
						VALUES ('0', '".$batch_nr."', '".$item_code."', '', '".$encounter_nr."', '".$path."', '')";
			}
			return $this->Transact();
		}
	}
