...
class Person extends Core {
	...
	*/
	var $basic_list='pid,title,name_first,name_last,name_2,name_3,name_middle,name_maiden,name_others,date_birth,
				           sex,addr_str,addr_str_nr,addr_quanhuyen_nr,addr_phuongxa_nr,addr_zip,addr_citytown_nr,photo_filename,tuoi';
	/**
...
*/
	...
	*/
	function getAllInfoObject($pid='') {
	    ...
	    $this->sql="SELECT p.*, addr.name AS addr_citytown_name,addrqh.name AS addr_quanhuyen_name,addrpx.name AS addr_phuongxa_name,ethnic.name AS ethnic_orig_txt
					FROM $this->tb_person AS p
					LEFT JOIN  $this->tb_citytown AS addr ON p.addr_citytown_nr=addr.nr
					LEFT JOIN  $this->tb_quanhuyen AS addrqh ON p.addr_quanhuyen_nr=addrqh.nr
					LEFT JOIN  $this->tb_phuongxa AS addrpx ON p.addr_phuongxa_nr=addrpx.nr
					LEFT JOIN  $this->tb_ethnic_orig AS ethnic ON p.ethnic_orig=ethnic.id
	...
	*/
	...
	function BloodGroup(){
		return $this->getValue('blood_group');
	}
	/**
	...
	*/
	...
	function QuanHuyenName($code_nr=''){
	    global $db;
		$this->sql="SELECT name FROM care_address_quanhuyen WHERE nr=$code_nr";
	...
	function PhuongXaName($code_nr=''){
	    global $db;
		$this->sql="SELECT name FROM care_address_phuongxa WHERE nr=$code_nr";
	...
	*/
	function CurrentEncounter($pid){
	    global $db;
		if(!$pid) return false;
		$this->sql="SELECT encounter_nr FROM $this->tb_enc WHERE pid='$pid' AND is_discharged=0 AND encounter_status <> 'cancelled' AND flag=0 AND status NOT IN ($this->dead_stat)";
		if($buf=$db->Execute($this->sql)){
		    if($buf->RecordCount()) {
				$buf2=$buf->FetchRow();
				//echo $this->sql;
...
//			$suchwort=(int) $suchwort;
			...
			//if(sizeof($comp)>1){
			//	$sql2=" WHERE ((name_first $sql_LIKE '%".strtr($ln,'+',' ')."%' OR name_first $sql_LIKE '%".strtr($ln,'+',' ')."' ) OR  (name_last $sql_LIKE '".strtr($fn,'+',' ')."%' OR name_last $sql_LIKE '%".strtr($fn,'+',' ')."%'))";
			//	if(!empty($bd)){
			//		$DOB=@formatDate2STD($bd,$date_format);
				
			//		if($DOB=='') {
			//			$sql2.=" OR date_birth $sql_LIKE '$bd' ";
			//		}else{
			//			$sql2.=" OR date_birth $sql_LIKE '$DOB' ";
			//		}
			//	}
			//}else{
				# Check if * or %
				...
				//	if(strtotime($suchwort)){
					//$DOB=date("Y-m-d",strtotime($suchwort));
					//}
					...
					if($DOB) {
						$sql2=" WHERE date_birth $sql_LIKE '$DOB' OR date_birth $sql_LIKE '%$DOB%' OR date_birth $sql_LIKE '$DOB%'
								OR (CONCAT(name_last,' ',name_first) $sql_LIKE '$searchkey%') OR (CONCAT(name_last,' ', name_first) $sql_LIKE '%$searchkey%')
	...
	*/
	...
	// function get infor insurance
		function getInfoInsurEnc($pid){
		  global $db;
		  $this->sql="select insurance_nr,insurance_class_nr,insurance_start,insurance_exp,madkbd,is_traituyen from care_person where pid=$pid";
		  if($this->result=$db->Execute($this->sql)){
		    if($this->result->RecordCount()) {
