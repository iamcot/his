			...
			while($zeile=$ergebnis->FetchRow()){
				...
				if($zeile['status']=='' || $zeile['status']=='normal'){
					...
					if($buf->RecordCount()){
						...
						$sql="SELECT name FROM care_class_insurance WHERE class_nr='".$buf2['insurance_class_nr']."'";
						$temp=$db->Execute($sql);
						$temp->RecordCount();
						$temp2=$temp->FetchRow();
						$this->smarty->assign('LDLanKhamGanDay',formatDate2Local($buf2['encounter_date'],$date_format,TRUE,TRUE)."-".formatDate2Local($buf2['encounter_date'],$date_format)."(".$temp2['name'].")");
						}else{
						$this->smarty->assign('LDLanKhamGanDay',"Đã xuất viện - ".formatDate2Local($buf2['discharge_date'],$date_format));
						}
					
					}else {
										
						$this->smarty->assign('LDLanKhamGanDay','Bệnh nhân chưa nhập viên');
					}
