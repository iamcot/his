...
* @package care_api
*/
class LabBlood extends Encounter {
	...
	* @var string
	*/
	var $tb_find_blood='care_test_findings_blood';
	/**
	* Table name for test findings for chemical lab paramters
	* @var string
	*/
	var $tb_find_blood_sub='care_test_findings_blood_sub';
	...
	*/
	var $tgroups;
	/**
	* Field names for care_test_findings_blood table
	* @var array
	*/
	var $fld_find_blood=array(
				'batch_nr',
				'encounter_nr',
				'test_date',
				'test_time',
				...
				'create_time');
	/**
	* Field names for care_test_findings_blood_sub table
	* @var array
	*/
	var $fld_find_blood_sub=array(
				'batch_nr',
				'job_id',
				'encounter_nr',
				'paramater_name',
	...
	*/
	...
	function LabBlood($enc_nr='', $subTable = false){
		if(!empty($enc_nr)) $this->enc_nr=$enc_nr;
		if($subTable == true) {
			$this->setTable($this->tb_find_blood_sub);
			$this->setRefArray($this->fld_find_blood_sub);
		} else {
			$this->setTable($this->tb_find_blood);
			$this->setRefArray($this->fld_find_blood);
		}
	...
	*/
	function createResultsList($enc_nr){
		...
		$this->sql="SELECT job_id,test_date,test_time,group_id FROM $this->tb_find_blood WHERE encounter_nr='$enc_nr' AND status<>'hidden' ORDER BY batch_nr DESC";
	...
	*/
	function BatchNr($enc_nr,$job_id,$grp_id){
	    global $db;
		$this->sql="SELECT batch_nr FROM $this->tb_find_blood WHERE encounter_nr='$enc_nr' AND job_id='$job_id' AND group_id='$grp_id'";
		if($this->result=$db->Execute($this->sql)){
		    if($this->rec_count=$this->result->RecordCount()) {
	...
	*/
	function hideResultIfExists($enc_nr,$job_id){
		global $_SESSION;
		$this->sql="UPDATE $this->tb_find_blood_sub SET status='hidden',history=".$this->ConcatHistory("Hide ".date('Y-m-d H:i:s')." ".$_SESSION['sess_user_name']."\n")."
	...
	* The array contains the test result data with index keys as outlined in the <var>$fld_find_blood</var> array.
	* @access public
	...
	*/
	function getBatchResult($bn){
		global $db;
		$this->sql  = "SELECT * FROM $this->tb_find_blood INNER JOIN $this->tb_find_blood_sub ";
		$this->sql .= "ON ($this->tb_find_blood.job_id = $this->tb_find_blood_sub.job_id) ";
		$this->sql .= "WHERE $this->tb_find_blood.batch_nr=$bn";
		//echo $this->sql;
		if($this->result=$db->Execute($this->sql)){
		    if($this->rec_count=$this->result->RecordCount()) {
				return $this->result;
	...
	function getBatchResult1($bn){
		global $db;
		$this->sql  = "SELECT * FROM $this->tb_find_blood INNER JOIN $this->tb_find_blood_sub ";
		$this->sql .= "ON ($this->tb_find_blood.job_id = $this->tb_find_blood_sub.job_id) ";
		$this->sql .= "WHERE $this->tb_find_blood.job_id=$bn";
		//echo $this->sql;
		if($this->result=$db->Execute($this->sql)){
		    if($this->rec_count=$this->result->RecordCount()) {
				return $this->result;
	...
	*/
	function getResult($job_id,$grp_id,$enc_nr=''){
		...
		$this->sql  = "SELECT * FROM $this->tb_find_blood INNER JOIN $this->tb_find_blood_sub ";
		$this->sql .= "ON ($this->tb_find_blood.job_id = $this->tb_find_blood_sub.job_id) ";
		$this->sql .= "WHERE $this->tb_find_blood.encounter_nr='$this->enc_nr' AND $this->tb_find_blood.job_id='$job_id' AND $this->tb_find_blood.group_id='$grp_id' AND $this->tb_find_blood.status<>'hidden'";
		//echo $this->sql;
		if($this->result=$db->Execute($this->sql)){
		    if($this->rec_count=$this->result->RecordCount()) {
				return $this->result;
	...
	*/
	function getAllResults($enc_nr=''){
		...
		$this->sql  = "SELECT * FROM $this->tb_find_blood INNER JOIN $this->tb_find_blood_sub ";
		$this->sql .= "ON ($this->tb_find_blood.job_id = $this->tb_find_blood_sub.job_id) ";
		$this->sql .= "WHERE $this->tb_find_blood.encounter_nr='$this->enc_nr' AND $this->tb_find_blood.status NOT IN ($this->dead_stat) ORDER BY $this->tb_find_blood_sub.test_date";
		//echo $this->sql;
		if($this->result=$db->Execute($this->sql)){
		    if($this->rec_count=$this->result->RecordCount()) {
				return $this->result;
	...
	*/
	function TestParams($group_id=''){
		...
		$this->sql="SELECT * FROM $this->tb_test_param WHERE $cond status NOT IN ($this->dead_stat) ORDER BY sort_nr";
	...
	*/
	function TestParamsAdmin($group_id=''){
		...
		$this->sql="SELECT * FROM $this->tb_test_param WHERE $cond ORDER BY sort_nr";
		if($this->tparams=$db->Execute($this->sql)){
		    if($this->rec_count=$this->tparams->RecordCount()) {
				return $this->tparams;
			} else {return FALSE;}
	...
	function GetTestsToDo($id=''){
		...
		if(empty($id)) $cond='';
			...
			$sub = "_sub";
		$this->sql  = "SELECT paramater_name, parameter_value FROM $this->tb_req_blood$sub  WHERE $cond order by sub_id";
		if($this->tparams=$db->Execute($this->sql)){
		    if($this->rec_count=$this->tparams->RecordCount()) {
				return $this->tparams;
			} else {return FALSE;}
	...
	function GetTestUrgent($id=''){
		...
		$this->sql="SELECT urgent FROM $this->tb_req_blood WHERE $cond";
		if($this->tparams=$db->Execute($this->sql)){
		    if($this->rec_count=$this->tparams->RecordCount()) {
				return $this->tparams;
			} else {return FALSE;}
	...
	*/
	function TestActiveGroups(){
		global $db;
		$this->sql="SELECT * FROM $this->tb_test_param WHERE group_id = '-2' AND status NOT IN ($this->dead_stat) ORDER BY sort_nr";
		if($this->tgroups=$db->Execute($this->sql)){
		    if($this->rec_count=$this->tgroups->RecordCount()) {
				return $this->tgroups;
			} else {return FALSE;}
	...
	*/
	function ResultExists($enc_nr=''){
		...
		$buf;
		if(!$this->internResolveEncounterNr($enc_nr)) return FALSE;
		$this->sql="SELECT nr FROM $this->tb_find_blood WHERE encounter_nr='$this->enc_nr' AND status NOT IN ($this->dead_stat)";
	...
	*/
	function searchEncounterLabResults($key='',$add_opt='',$limit=FALSE,$len=30,$so=0){
		...
		if(is_numeric($key)){
			$key=(int)$key;
			$this->sql.=" WHERE e.encounter_nr $sql_LIKE '%$key' OR e.encounter_nr $sql_LIKE '%$key%'";
		}else{
	...
	function searchEncounterLabDhuyetResults($key='',$add_opt='',$limit=FALSE,$len=30,$so=0){
		global $db, $sql_LIKE;
		if(empty($key)) return FALSE;
		$this->sql="SELECT f.encounter_nr,f.batch_nr, e.encounter_class_nr, p.pid, p.name_last, p.name_first, p.date_birth, p.sex
				FROM care_test_findings_duonghuyet AS f
				LEFT JOIN $this->tb_enc AS e ON e.encounter_nr = f.encounter_nr
				LEFT JOIN $this->tb_person AS p ON p.pid = e.pid";
		if(is_numeric($key)){
			$key=(int)$key;
		...
		# Append the common condition
		$this->sql.=" GROUP BY f.encounter_nr,f.batch_nr, e.encounter_class_nr, p.pid, p.name_last, p.name_first, p.date_birth, p.sex  $add_opt";
		//echo $this->sql;
		if($limit){
	    	$this->res['selr']=$db->SelectLimit($this->sql,$len,$so);
		}else{
	...
	function searchEncounterLabVsinhResults($key='',$add_opt='',$limit=FALSE,$len=30,$so=0){
		global $db, $sql_LIKE;
		if(empty($key)) return FALSE;
		$this->sql="SELECT f.encounter_nr,f.batch_nr, e.encounter_class_nr, p.pid, p.name_last, p.name_first, p.date_birth, p.sex
				FROM care_test_findings_visinh AS f
				LEFT JOIN $this->tb_enc AS e ON e.encounter_nr = f.encounter_nr
				LEFT JOIN $this->tb_person AS p ON p.pid = e.pid";
		if(is_numeric($key)){
			$key=(int)$key;
	...
	*/
	...
	function searchLimitEncounterLabDhuyetResults($key,$len,$so,$sortitem='',$order='ASC'){
		if(!empty($sortitem)){
			$option=" ORDER BY $sortitem $order";
		}else{
			$option='';
		}
		return $this->searchEncounterLabDhuyetResults($key,$option,TRUE,$len,$so);
	}
	function searchLimitEncounterLabVsinhResults($key,$len,$so,$sortitem='',$order='ASC'){
		if(!empty($sortitem)){
			$option=" ORDER BY $sortitem $order";
		}else{
			$option='';
		}
		return $this->searchEncounterLabVsinhResults($key,$option,TRUE,$len,$so);
	}
	/**
	...
	*/
	function getLastModifyTime($enc_nr=0){
		...
		$row;
		if(!$this->internResolveEncounterNr($enc_nr)) return FALSE;
		$this->sql="SELECT modify_time FROM $this->tb_find_blood WHERE encounter_nr='$this->enc_nr' AND status NOT IN ($this->dead_stat) ORDER BY modify_time DESC";
	...
	*/
	function getGroupName( $id ){
		global $db;
		$this->sql="SELECT name FROM $this->tb_test_param WHERE group_id = '-2' AND status NOT IN ($this->dead_stat) AND id = '$id'";
		if($this->tgroups=$db->Execute($this->sql)){
		    if($this->rec_count=$this->tgroups->RecordCount()) {
				return $this->tgroups;
			} else {return FALSE;}
