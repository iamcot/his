...
class Khambenh extends Core {
	...
	*/
	...
	var $rhm_ngoaitru='dfck_rhm_ngoaitru';
	var $rhm_ngoaitru_array=array(
						'nr',
						'encounter_nr',
						'tiensubenh',
						'hamtren_trai',
...
//		echo $this->sql;
	...
	function KhambenhRHMNgoaitru()
	{
		$this->setTable($this->rhm_ngoaitru);
		$this->setRefArray($this->rhm_ngoaitru_array);
	}
	function getKhambenhRHMNgoaitru($nr){
		return $this->_getKhambenhRHMNgoaitru("encounter_nr=$nr",'');
	}
	 function _getKhambenhRHMNgoaitru($cond,$order='ORDER BY date,time DESC'){
	    global $db;
		$this->sql="SELECT * FROM $this->rhm_ngoaitru WHERE $cond $order";
//		echo $this->sql;
	    if ($this->result=$db->Execute($this->sql)) {
		    if ($this->result->RecordCount()) {
		        return $this->result;
