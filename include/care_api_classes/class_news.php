...
class News extends Core {
	...
	*/			
	...
	//add 0510 - cot
	function updateNews($newsnr, &$news){
		global $db;
		$this->sql="UPDATE  $this->tb SET
							...
							preface = '".addslashes($news['preface'])."',
							body = '".addslashes($news['body'])."',
							pic = '".$news['pic']."',
	...
	//add 0410 - cot
	function mainGetNews($limit=5,$page=0){
		global $db;
		if($_SESSION['sess_login_userid']!='admin') $show = ' and `status`= 0';else $show ='';
		$this->sql="select * from $this->tb where dept_nr=0 $show order by nr DESC limit $page,$limit";
		if($this->result=$db->Execute($this->sql)) {
            if($this->result->RecordCount())  return $this->result;
			    else return false;
	    }
	}
	function ishavelessnews($limit=5,$prevpage){
		global $db;
		if($_SESSION['sess_login_userid']!='admin') $show = ' where `status`= 0';else $show ='';
		$this->sql="select nr from $this->tb  $show order by nr DESC limit $prevpage,$limit";
		if($this->result=$db->Execute($this->sql)) {
            if($this->result->RecordCount())  return true;
			    else return false;
	}
	}
	function ishavemorenews($limit=5,$nextpage){
		global $db;
		if($_SESSION['sess_login_userid']!='admin') $show = ' where `status`= 0';else $show ='';
		$this->sql="select nr from $this->tb  $show order by nr DESC limit $nextpage,$limit";
		if($this->result=$db->Execute($this->sql)) {
            if($this->result->RecordCount())  return true;
			    else return false;
	}
	}
	function updateNewsStatus($nr,$status){
		global $db;
		$this->sql="update $this->tb set `status`= $status where nr = $nr ";
		if($db->Execute($this->sql))
		return 1;
		else return 0;
	}
	function get1News($nr){
		global $db;
		$this->sql="select * from $this->tb $show where nr=$nr";
		if($this->result=$db->Execute($this->sql)) {
