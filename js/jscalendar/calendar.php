...
class DHTML_Calendar {
    ...
    function get_load_files_code() {
		...
		$code .= ( '<script type="text/javascript" src="' .
        		   ...
        		   'jquery.min.js"></script>' . NEWLINE );
		 $code .= ( '<script type="text/javascript" src="' .
        		   $this->calendar_lib_path .'jquery.maskedinput-1.3.js"></script>' . NEWLINE );
        return $code;
    }
    ...
    function show_calendar ($calendar,$date_format,$input_field,$value='') {
		...
		if($value != DBF_NODATE && !empty($value)) {
			$sepChars=array('-','.','/',':',',');
			$value = formatDate2Local($value,$date_format,false,false,$sepChars);
		} else {
