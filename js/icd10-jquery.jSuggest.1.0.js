		...
		$(this).bind("keyup click", function(e){
			...
			if (this.value.length >= opts.minchar  ) {
				...
				$(jC).css({
					...
					left: offSet.left  +"px",
					width: $(this).outerWidth() + opts.widthplus + "px",
				...
				// if enter key
					...
					if ($(jH).length == 1)
						...
						$(idcode).val(textBox.value.split(/\s/)[0]);
				}
				// if down arrow
				...
				//doing = 1;
					...
					//<img src="'+opts.loadingImg+'" align="bottom" />
					...
					if (opts.data == '')
						...
						opts.data =  "searchQuery=" + $(this).val();
