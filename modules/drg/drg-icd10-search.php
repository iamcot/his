...
if($mode=='save'){
	...
	if(!empty($keyword)){
		...
		if(strlen($keyword)<3){
			...
			$sql="SELECT $fielddata FROM $drg->tb_diag_codes WHERE (diagnosis_code $sql_LIKE '%$keyword%' OR description $sql_LIKE '$keyword%' $sAddWhere) AND type <> 'table' ORDER BY diagnosis_code";
...
//echo $sql;
		...
		if($ergebnis){
			...
			if ($linecount=$ergebnis->RecordCount()){
				if(strlen($keyword)<3){
					$advsql="SELECT sub_level FROM $drg->tb_diag_codes WHERE (diagnosis_code $sql_LIKE '%$keyword%' OR description $sql_LIKE '$keyword%') AND type <> 'table' ORDER BY diagnosis_code";
				}else{
					$advsql="SELECT sub_level FROM $drg->tb_diag_codes WHERE (diagnosis_code $sql_LIKE '%$keyword%' OR description $sql_LIKE '%$keyword%') AND type <> 'table' ORDER BY diagnosis_code";
				}
