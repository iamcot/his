...
if (isset ( $target )) {
	switch ($target) {
		...
		case 'requestrepair':
			$searchkey = str_replace(" ","%", $searchkey);
			$sql = "SELECT pn.nr AS 'request_person.value', CONCAT(p.name_last, ' ',p.name_first) AS 'request_person_name.value', 0 AS use_frequency FROM care_personell AS pn, care_person AS p WHERE (pn.pid = p.pid) ";
			if ($mode == 'search') {
				$sql .= " AND ((nr $sql_LIKE '%$searchkey%') OR (CONCAT(p.name_last, ' ',p.name_first) $sql_LIKE '$searchkey%') OR (CONCAT(p.name_last, ' ', p.name_first) $sql_LIKE '%$searchkey%'))";
				$sql .= " ORDER BY p.name_first DESC";
			} else {
