...
if($dblink_ok)
{
	...
	if($mode=='save'){
		if($isdelete=='delete'){
			if($editid!=''){
				$q="delete from care_encounter_custom_noc where nr = '".$editid."' ";
		...
		elseif(($indatetime_time&&$verbal&&$moton&&$eyes)){
			...
			if ($_POST['editid'] != "") {
				...
				$indatetime_date=formatDate2STD($indatetime_date,$date_format);
				$indatetime_time=$_POST['indatetime_time'].':00';
				$q="insert into care_encounter_custom_noc (encounter_nr,createid,indatetime,time,verbal,moton,eyes) values (
					'".$pn."',
...
echo '<font size="4">'.$NOC_title.' <p><font size=2>';
...
?>
  ...
  <script language=JavaScript>
		...
		function deleteRecord()
		{
			...
			document.berichtform.action='nursing-station-patientdaten-custom-noc.php<?php echo URL_REDIRECT_APPEND?>&isdelete=delete&station=<?php echo $station ?>&pn=<?php echo $pn ?>&edit=<?php echo $edit; ?>&dblink_ok=1&editid='+editid;
...
while ($iod=mysql_fetch_assoc($res)) {
	$cols++;
		...
		$time_top.="<span id='id_".$iod['nr']."_indatetime_time'>".date("H:i",strtotime($iod['time']))."</span>";
		$time_top.="</td>";
		$row_verbal.="<td></td>";
...
if($edit) { 
		...
		$row_top.="<td align=center><a href='nursing-station-patientdaten-custom-noc.php".URL_REDIRECT_APPEND."&station=".$station."&pn=".$pn."&edit=".$edit."'><span style='background-color:white'> ".$LDRefresh." </span> </a>&nbsp;&nbsp;";
		$row_top.="<a href='javascript:deleteRecord();'> <span style='background-color:white'>".$LDCLEAR."</span> </a></td>";
		$time_top.="<td>";
		...
		$calendar->load_files();
			$dfbuffer = $calendar->show_calendar($calendar,$date_format,'indatetime_date',date('Y-m-d'));
		//end : gjergji  
		$time_top.="$dfbuffer";
		$time_top.="</font></td></tr></table>".
		$LDClockTime.":<br>";
		$time_top.="<input type=text size=4 maxlength=5 name=indatetime_time id=indatetime_time  value='".date('H:i')."' onBlur=\"checkTime(this)\"><br>
		<script language=\"javascript\">
		$(function(){
