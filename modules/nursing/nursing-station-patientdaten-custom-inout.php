...
if($dblink_ok)
{
	...
	if($mode=='save'){
		if($isdelete=='delete'){
			if($editid!=''){
				$q="delete from care_encounter_custom_inout where nr = '".$editid."' ";
		...
		}elseif(($indatetime_date&&$indatetime_time&&$pint&&$solution&&$solutionamount) || ($indatetime_date&&$indatetime_time&&$oralfluid&&$oralfluidamount)){
		...
		if ($_POST['editid'] != "") {
			...
			indatetime='".$indatetime_date." ".date('H:i:s')."',
			time='".$indatetime_time."',
			...
			$indatetime_date=formatDate2STD($indatetime_date,$date_format);
			$indatetime_time=$_POST['indatetime_time'].':00';
			$q="insert into care_encounter_custom_inout (encounter_nr,createid,indatetime,time,pint,solution,solutionamount,initial,oralfluid,oralfluidamount,urinetime,urineamount,rta,drain) values (
				'".$pn."',
				'".$_SESSION['sess_login_userid']."',
				'".$indatetime_date." ".date('H:i:s')."',
				'".$indatetime_time."',
				'".$pint."',
				'".$solution."',
				'".$solutionamount."',
...
<style type="text/css">
div.fva2_ml10 {font-size: 12; margin-left: 10; text-align:center;}
div.fa2_ml10 {font-size: 12; margin-left: 10;}
div.fva2_ml3 {font-size: 12; margin-left: 3; text-align:center;}
...
function pruf(d){
	if(isNaN(d.solutionamount.value)){
		alert("<?php echo $LDAlertInsertNumber; ?>");
		d.solutionamount.focus();
		return false;
	}else if(isNaN(d.oralfluidamount.value)){
		alert("<?php echo $LDAlertInsertNumber; ?>");
		d.oralfluidamount.focus();
		return false;
	}else if(isNaN(d.urineamount.value)){
		alert("<?php echo $LDAlertInsertNumber; ?>");
		d.urineamount.focus();
		return false;
	}else if(isNaN(d.rta.value)){
		alert("<?php echo $LDAlertInsertNumber; ?>");
		d.rta.focus();
		return false;
	}else if(isNaN(d.drain.value)){
		alert("<?php echo $LDAlertInsertNumber; ?>");
		d.drain.focus();
		return false;
	}

	if(((d.indatetime_date.value)&&(d.indatetime_time.value)&&(d.solution.value)&&(d.solutionamount.value))||((d.indatetime_date.value)&&(d.oralfluid.value)&&(d.oralfluidamount.value))) return true;
	else 
	{
...
<td colspan=12 bgcolor="#99ccff"><div class=fa2_ml10>

<?php

echo '<center><font size="4">'.$IO_title.' </center><p></font>'.$LDNoteTurningChart.'<p><font size=1>'.$LDNoteIntakeOutputChartTxt.'</font>';

?>
</div></td></tr>
<?php
echo '	<tr bgcolor="#99ccff">
		<td colspan=9><div class=fva2_ml10><font color="#000099"><b>'.$LDIntake.'</b></div></td>
		<td colspan=6><div class=fva2_ml10><font color="#000099"><b>'.$LDOutput.'</b></div></td>
		</tr>';
echo '	<tr bgcolor="#99ccff">
		<td colspan=6><div class=fva2_ml10></div></td>
		<td colspan=3><div class=fva2_ml10></div></td>
		<td colspan=2><div class=fva2_ml10><font color="#000000"><b>'.$LDUrine.'</b></div></td>
		<td colspan=2><div class=fva2_ml10><b>'.$LDTrongdo.'</b></div></td>
		<td colspan=2><div class=fva2_ml10></div></td>
		</tr>';	
...
echo '	<tr bgcolor="#99ccff">
		...
		function Edit(id) {
			...
			document.getElementsByName('indatetime_date')[0].value=eval("document.getElementById('id_"+id+"_indatetime_date').innerHTML");
			document.getElementsByName('indatetime_date')[0].style.backgroundColor=editcolor;
		...
		function deleteRecord()
		{
			...
			document.berichtform.action='nursing-station-patientdaten-custom-inout.php<?php echo URL_REDIRECT_APPEND?>&isdelete=delete&station=<?php echo $station ?>&pn=<?php echo $pn ?>&edit=<?php echo $edit; ?>&dblink_ok=1&editid='+editid;
...
while ($iod=mysql_fetch_assoc($res)) {
	...
	<tr bgcolor="#99ccff">
		...
		<td><div class=fva2_ml3 id="id_<?php echo $iod['nr']?>_indatetime_time"><?php echo date("H:i",strtotime($iod['time']))?></div></td>
...
?>
		...
		<?php
			...
			$calendar->load_files();
  			echo $calendar->show_calendar($calendar,$date_format,'indatetime_date',date('Y-m-d'));
			//end : gjergji  
 		?>
         </td>
		 
		<td valign="top"><?php echo $LDClockTime ?>:<br>
		<input type=text size=4 maxlength=5 name="indatetime_time" id="indatetime_time"  value="<?php echo date('H:i'); ?>" onBlur="checkTime(this)" ><br>
		<script language="javascript">
		$(function(){
			$("#indatetime_time").mask("**:**");
		});
		</script>
		</td>
		...
		<input type=text size=5 maxlength=6 name="initial"  id="initial" value="<?if (!($saved)) echo $_SESSION['sess_login_userid']; ?>">
		</td>
		...
		<input type=text size=5 maxlength=5 name="urinetime"  id="urinetime" value="<? echo date('H:i');?>" onBlur="checkTime(this)"  >
		<script language="javascript">
		$(function(){
			$("#urinetime").mask("**:**");
		});
		...
		</td><td colspan=2 align=center><a <?php echo 'href=\'nursing-station-patientdaten-custom-inout.php'.URL_REDIRECT_APPEND.'&station='.$station.'&pn='.$pn.'&edit='.$edit.'\''; ?>> <span style="background-color:white"> <?php echo $LDRefresh ?> </span> </a>
		<p>
		<a href="javascript:deleteRecord();"> <span style="background-color:white"> <?php echo $LDCLEAR; ?> </span> </a>
		</td>
		</tr>
