...
$fileforward='include/inc_issuepaper_medipot_savedepot.php'.URL_APPEND;
...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDIssueMedipot.' :: '.$LDDepotTabMed);
...
<!--
function chkform(d) {
	document.createdepotform.action="include/inc_issuepaper_medipot_savedepot.php";
...
{
	var win = 'include/search_medipot.php?id_number=' + id_number;
...
{
	var process_file='include/inc_issuepaper_autocomplete_medipot.php?mode=filldata';
	var name_med='medicine'+i;
...
$smarty->assign('titleForm',$LDIssueMedipot);
$smarty->assign('LDSTT',$LDSTT);
$smarty->assign('LDPresID',$LDMedipotID);
...
if($target=='depot') 
{
	...
	if(!isset($mode) || $mode!='update'){
		...
		$IssuePaper = new IssueMedipot;
		
		$listIssue = $IssuePaper->getDetailIssueMedipotInfo($issue_id);
		...
		if(is_object($listIssue)){
			...
			for ($i=1;$i<=$maxid;$i++){
				...
				//$rowIssue['product_encoder'],$inventory
					$sql = "SELECT SUM(available_number) FROM care_med_available_product WHERE product_encoder='".$rowIssue['product_encoder']."'
GROUP BY product_encoder";
					if($tempinven = $db->Execute($sql))
						if($tempinven->RecordCount()){
							$tempcount=$tempinven->FetchRow();
							$inventory=$tempcount['available_number'];
						}	
				ob_start();
		...
		$listIssue = $IssueMedipot->getDetailIssueMedipotInfo($issue_id);
		if(is_object($listIssue)){
			$sTempDivStt='';$sTempDiv='';
			$maxid=$listIssue->RecordCount();
			for ($i=1;$i<=$maxid;$i++){
