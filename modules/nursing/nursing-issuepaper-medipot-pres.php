...
$fileforward='nursing-issuepaper-medipot-depot.php'.URL_APPEND.'&target=sum';
...
$Ward = new Ward;
//$wardname = $Ward->WardName($ward_nr);
...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDIssueMedipot.' :: '.$LDPrescriptionTab);
...
if($target=='pres') 
{
	...
	$sql=	"SELECT prs.prescription_id, prs.encounter_nr, prs.date_time_create, enc.current_room_nr, per.name_first, per.name_last, per.sex  
			FROM care_med_prescription_info AS prs, care_med_type_of_prescription AS tp, care_encounter AS enc, care_person AS per 
