...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDUseMedipotReport.' :: '.$LDMedipotReport);
 ...
 # Window bar title
 $smarty->assign('title',$LDUseMedipotReport);
...
{
	urlholder="<?php echo $root_path;?>modules/pdfmaker/duoc/baocaosudungVTYT.php<?php echo URL_APPEND; ?>&type=medipot&dept_nr="+dept+"&ward_nr="+ward+"&monthreport="+month+"&yearreport="+year;
	testprintpdf=window.open(urlholder,"BaoCaoSuDungVTYT","width=1000,height=760,menubar=yes,resizable=yes,scrollbars=yes");
}
-->
</script>
<?php
...
$listReport = $CabinetMedipot->reportMonth($dept_nr,$ward_nr,$monthreport,$yearreport);
if(is_object($listReport)){
	...
	while($rowReport = $listReport->FetchRow())	{
		...
		if ($flag){
			...
			echo	'<td align="left">'.$rowReport['product_encoder'].'</td>';	//Ma thuoc
