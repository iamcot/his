...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDReturnMedicine.' :: '.$LDListReportTab);
...
{
	var win = '../include/inc_return_showdetail.php<?php echo URL_APPEND; ?>' +'&report_id=' + issue_id;
...
$new_report_url='medicine_return_medicine.php';
	...
	if(is_object($listItem)){
		...
		for ($i=0;$i<$listItem->RecordCount();$i++)
		{
			...
			if ($rowItem['status_finish']){
				...
				$editbutton='medicine_return_medicine.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr.'&return_id='.$rowItem['return_id'].'&target=update';
