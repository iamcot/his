...
$fileforward='../include/inc_return_medipot_save.php'.URL_APPEND;
...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDReturnMedipot.' :: '.$LDNewReportTab);
 ...
 # Window bar title
 $smarty->assign('title',$LDReturnMedipot);
...
<!--
function chkform(d) {
	document.newreturnform.action="../include/inc_return_medipot_save.php";
...
function insertRow()
{
  ...
  var lastRow = tbl.tBodies[0].rows.length;
	...
	xmlhttp.open("GET","../include/inc_returnmed_addmedicine.php?i="+idnum,true);
	xmlhttp.send();
...
{
	var win = 'cabinet_search_medipot.php?id_number='+id_number+'&dept_nr=<?php echo $dept_nr;?>&ward_nr=<?php echo $ward_nr;?>';
...
function Medicine_AutoComplete(i){
			...
			var includeScript = "../include/inc_med_cabinet_autocomplete_medipot.php?mode=auto&dept_nr=<?php echo $dept_nr;?>&ward_nr=<?php echo $ward_nr;?>&k="+i;
...
{
	var process_file='../include/inc_med_cabinet_autocomplete_medipot.php?mode=filldata&dept_nr=<?php echo $dept_nr;?>&ward_nr=<?php echo $ward_nr;?>';
	var name_med='medicine'+i;
	...
	xmlhttp.open("GET",process_file+"&lotid="+document.getElementById('lotid'+i).value+"&search="+document.getElementById('medicine'+i).value,true);
    xmlhttp.send();
...
$list_report_url='medipot_return_medipot_list.php';
ob_start();
...
$smarty->assign('sRegForm','<form name="newreturnform" method="POST"  onSubmit="return chkform(this)">');

//***********************************NOI DUNG TRANG********************************
$smarty->assign('deptname',$LDDept.': '.$deptname);
$smarty->assign('ward',$LDWard.': '.$wardname);
$smarty->assign('titleForm',$LDReturnMedipot);
...
if(!isset($target) || ($target=='new') || ($target=='create')){
		...
		$smarty->assign('IssueId',$LDIssueId.': '.'<input type="text" size=8 name="IssueId" value="'.$return_id.'" style="text-align:center;border-color:white;border-style:solid;" readonly>');
		
		require_once($root_path.'include/care_api_classes/class_cabinet_medipot.php');
		$CabinetMedipot = new CabinetMedipot;
		
		$listReport = $CabinetMedipot->getDetailReturnInfo($return_id);
...
$smarty->assign('sMainBlockIncludeFile','nursing/medicine_returnmed.tpl');
