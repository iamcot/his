<?php
    ...
    */
    ...
    $smarty->assign('sToolbarTitle',$LDDestroyChemical.' :: '.$LDListReportTab);
...
<!--
    ...
    {
	var win = '../include/inc_destroy_chemical_showdetail.php<?php echo URL_APPEND; ?>' +'&report_id=' + issue_id;
...
<?php
    ...
    $new_report_url='chemical_destroy_chemical.php';
    ...
    if ($search==''){
        ...
        $total_items=$Cabinet->countDestroyChemicalItems($condition);
        ...
        $listItem = $Cabinet->listSomeDestroyChemicalSplitPage($current_page,$number_items_per_page,$condition);
        ...
        $listItem = $Cabinet->listAllDestroyChemical($condition);	
    ...
    if(is_object($listItem)){
        ...
        if ($rowItem['status_finish']){
            ...
            $editbutton='chemical_destroy_chemical.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr.'&des_id='.$rowItem['destroy_id'].'&target=update';
        ...
        $sTemp=$sTemp.'<tr bgColor="#ffffff" >
                            <td align="center"><input type="image" '.createComIcon($root_path,'info3.gif','0','',TRUE).' OnClick="viewDetail('.$rowItem['destroy_id'].')" /></td>
