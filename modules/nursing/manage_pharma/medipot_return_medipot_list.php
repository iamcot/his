...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDReturnMedipot.' :: '.$LDListReportTab);
...
{
	var win = '../include/inc_return_medipot_showdetail.php<?php echo URL_APPEND; ?>' +'&report_id=' + issue_id;
...
$new_report_url='medipot_return_medipot.php';
...
if ($search==''){
	...
	$total_items=$Cabinet->countReturnItems($condition);
	...
	$listItem = $Cabinet->listSomeReturnSplitPage($current_page,$number_items_per_page,$condition);
	...
	if (strrpos($search,'/') || strrpos($search,'-')){
		...
		$listItem = $Cabinet->listAllReturn($condition);	
	...
	if(is_object($listItem)){
		...
		for ($i=0;$i<$listItem->RecordCount();$i++)
		{
			...
			if ($rowItem['status_finish']){
				...
				$editbutton='medipot_return_medipot.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr.'&return_id='.$rowItem['return_id'].'&target=update';
			...
			$sTemp=$sTemp.'<tr bgColor="#ffffff" >
								<td align="center"><a href=""><input type="image" '.createComIcon($root_path,'info3.gif','0','',TRUE).' onclick="viewDetail('.$rowItem['return_id'].')"></a></td>
