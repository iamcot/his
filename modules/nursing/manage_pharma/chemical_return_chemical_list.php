<?php
    ...
    */
    ...
    $smarty->assign('sToolbarTitle',$LDRequestChemicalReturn.' :: '.$LDListReportTab);
    ...
    # Window bar title
    $smarty->assign('Name',$LDReturnChemical);
...
<!--
    function chkform(d) {
        document.listreturnform.action="";
        document.listreturnform.submit();
    }

    function search()
    {
	var search = document.getElementById('search').value;
	document.listreturnform.action="<?php echo $urlsearch;?>&search="+search;
	document.listreturnform.submit();
    }

    function viewDetail(issue_id)
    {
	var win = '../include/inc_return_chemical_showdetail.php<?php echo URL_APPEND; ?>' +'&report_id=' + issue_id;
...
<?php
    ...
    $new_report_url='chemical_return_chemical.php';
    ...
    $smarty->assign('sRegForm','<form name="listreturnform" method="POST"  onSubmit="return chkform(this)">');
    ...
    if ($search==''){
        ...
        $total_items=$Cabinet->countReturnChemicalItems($condition);
        ...
        $listItem = $Cabinet->listSomeReturnChemicalSplitPage($current_page,$number_items_per_page,$condition);
        ...
        if (strrpos($search,'/') || strrpos($search,'-')){
            ...
            $condition=" WHERE ".$condition_dept_ward." AND return_id LIKE '%".$search."%' ORDER BY return_id";	
            $listItem = $Cabinet->listAllChemicalReturn($condition);	
        ...
        if(is_object($listItem)){
            ...
            for ($i=0;$i<$listItem->RecordCount();$i++)
            {
                ...
                if ($rowItem['status_finish']){
                        $editbutton="javascript:alert('".$LDCannotEditIssuePaperChemical."')";
                        $status_finish=$LDReturnOk; $temp1='check-r.gif';
                }
                else{
                        $editbutton='chemical_return_chemical.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr.'&return_id='.$rowItem['return_id'].'&target=update';
                        $status_finish=$LDReturnNotYet; $temp1='warn.gif';
                }			
                ...
                $sTemp=$sTemp.'<tr bgColor="#ffffff" >
                                    <td align="center"><input OnClick="viewDetail('.$rowItem['return_id'].')" type="image" '.createComIcon($root_path,'info3.gif','0','',TRUE).' /></td>
                                    <td align="center">'.$rowItem['return_id'].'</td>
...
$smarty->assign('pbSearch','<input type="image" '.createComIcon($root_path,'Search.png','0','',TRUE).' OnClick="search()" >');
...
$smarty->assign('sMainBlockIncludeFile','nursing/medicine_returnmed_list.tpl');
