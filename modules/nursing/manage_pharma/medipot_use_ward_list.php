...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDUseVTYT.' :: '.$LDListReportTab);
...
{
	var win = '../include/inc_use_medipot_showdetail.php<?php echo URL_APPEND; ?>' +'&report_id=' + issue_id;
...
$new_report_url='medipot_use_ward.php';
	...
	if(is_object($listItem)){
		...
		for ($i=0;$i<$listItem->RecordCount();$i++)
		{
			...
			if ($rowItem['status_finish']){
				...
				$editbutton='medipot_use_ward.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr.'&use_id='.$rowItem['use_id'].'&target=update';
