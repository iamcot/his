<?php	
    ...
    $sql="SELECT khochan.product_encoder, khochan.product_name, donvi.unit_name_of_chemical, tatcakhoa.product_lot_id, tatcakhoa.exp_date, donvi.unit_name_of_chemical as name_unit, khochan.price,
            taikhoa.* , grp.chemical_group_name, sup.supplier, sup.supplier_name 
            FROM care_chemical_available_department AS taikhoa, care_chemical_available_product AS tatcakhoa, 
            care_chemical_products_main AS khochan, care_chemical_unit_of_medicine AS donvi, care_chemical_group AS grp, care_supplier AS sup 
            ...
            AND khochan.chemical_generic_drug_id=grp.chemical_group_id 
...
//	echo $sql;
	...
	if($ergebnis=$db->Execute($sql))
	{
		...
		for($i=0; $i<$n; $i++)
		{
			...
			echo '<td><input type="button" class="mybutton" onClick="sendValue(\''.$item['product_name'].'\',\''.$item['product_lot_id'].'\',\''.$item['name_unit'].'\',\''.$item['product_lot_id'].'\',\''.$item['price'].'\');"></td></tr>';
		}
	}
