...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDUseMedicine.' :: '.$LDListReportTab);
...
{
	//alert('Open');
	var win = '<?php echo $root_path.'modules/nursing/'; ?>include/inc_use_showdetail.php<?php echo URL_APPEND; ?>' +'&report_id=' + issue_id;
	//alert(win);
	myWindow=window.open( win , 'View Details' , 'height=500,width=700,resizeable=yes,scrollbar=yes' );
	//myWindow.focus();
}
-->
</script>
...
$new_report_url='medicine_use_ward.php';
...
if ($search==''){
	...
	$total_items=$Cabinet->countUseItems($condition);
	...
	$listItem = $Cabinet->listSomeUseSplitPage($current_page,$number_items_per_page,$condition);
	...
	if (strrpos($search,'/') || strrpos($search,'-')){
		...
		$listItem = $Cabinet->listAllUse($condition);	
	...
	if(is_object($listItem)){
		...
		for ($i=0;$i<$listItem->RecordCount();$i++)
		{
			...
			$sTemp=$sTemp.'<tr bgColor="#ffffff" >
								<td align="center"><a href=""><input onclick="viewDetail('.$rowItem['use_id'].')" type="image" '.createComIcon($root_path,'info3.gif','0','',TRUE).'></a></td>
