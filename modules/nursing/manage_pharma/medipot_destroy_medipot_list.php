...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDDestroyMedipot.' :: '.$LDListReportTab);
...
{
	var win = '../include/inc_destroy_medipot_showdetail.php<?php echo URL_APPEND; ?>' +'&report_id=' + issue_id;
...
$new_report_url='medipot_destroy_medipot.php';
	...
	if(is_object($listItem)){
		...
		for ($i=0;$i<$listItem->RecordCount();$i++)
		{
			...
			if ($rowItem['status_finish']){
				...
				$editbutton='medipot_destroy_medipot.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr.'&des_id='.$rowItem['destroy_id'].'&target=update';
