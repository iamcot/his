...
$fileforward= 'medicine_destroy_med.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr;
...
<table border="0" cellSpacing="1" cellPadding="3" width="95%" bgColor="#C3C3C3">
	<tr bgColor="#EDF1F4">
		...
		<th><?php echo $LDCabinetMedicineSum; ?></th>	
	</tr>																																
	<?php 
	if ($search==''){
		...
		if ($listItem = $Product->SearchExpCabinet($dept_nr, $ward_nr, $condition)){		
		...
		if ($total_pages>1)
			$listItem = $Product->ShowExpCabinet($dept_nr, $ward_nr, $current_page, $number_items_per_page);
		...
		$listItem = $Product->SearchExpCabinet($dept_nr, $ward_nr, $condition);
