...
$fileforward='../include/inc_use_patient_med_save.php'.URL_APPEND;
...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDIssuePaper.' :: '.$LDPhatVTYTChoBN);
...
{
	var win = 'show_med_prescription_issue_detail.php<?php echo URL_APPEND; ?>' + '&enc_nr=' + encounter_nr +'&pres_id=' + pres_id;
...
$smarty->assign('calendar',$LDDateIssue.': '.$calendar->show_calendar($calendar,$date_format,'dateissue',date('Y-m-d')));
	...
	$sql=	"SELECT prs.prescription_id, prs.encounter_nr, prs.date_time_create, enc.current_room_nr, 
			...
			FROM care_med_prescription_info AS prs, care_encounter AS enc, care_person AS per 
		...
		if($count){
			...
			for($i=0;$i<$count;$i++) {
				...
				echo '<tr bgcolor="#eeeeee">
						<td valign="top"><table width="100%"><tr><td><b>'.$item['name_last'].' '.$item['name_first'].'</b></td><td align="right"><a href="javascript:#"> 
				...
				echo '<td colspan="4">';
					...
					$sql_get_medicine = "SELECT * FROM care_med_prescription WHERE prescription_id='".$item['prescription_id']."' ORDER BY nr";
					if($medicine_result=$db->Execute($sql_get_medicine)){
						...
						for ($j=0; $j<$medicine_result->RecordCount();$j++) {
							...
							$total_items++;	
							if($ketqua = $Pres->getMedipotIssue($item['prescription_id'], $items_in_sheet['product_encoder']))
								$tongthuocdaphat = $ketqua['sum'];
							else $tongthuocdaphat=0;
							
							echo '<tr><td width="50%"><b>'.$items_in_sheet['product_name'].'</b><br></td>';
			...
			$sListRows='<tr bgColor="#eeeeee"><td colspan="6">'.$LDNoPresMedipot.'</td></tr>';
		}
		$itemcode=$itemcode1;
