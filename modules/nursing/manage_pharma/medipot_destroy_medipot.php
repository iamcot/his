...
$fileforward='../include/inc_destroy_medipot_save.php'.URL_APPEND;
...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDDestroyMedipot.' :: '.$LDNewReportTab);
 ...
 # Window bar title
 $smarty->assign('title',$LDDestroyMedipot);
...
<!--
function chkform(d) {
	document.newdestroyform.action="../include/inc_destroy_medipot_save.php";
...
function insertRow()
{
  ...
  var lastRow = tbl.tBodies[0].rows.length;
	...
	xmlhttp.open("GET","../include/inc_destroymed_addmedicine.php?i="+idnum,true);
	xmlhttp.send();
...
$list_report_url='medipot_destroy_medipot_list.php';
ob_start();
...
$smarty->assign('titleForm',$LDDestroyMedipot);
...
if(!isset($target) || ($target=='new') || ($target=='create')){
		...
		for ($i=1;$i<=$maxid;$i++){
			...
			ob_start();
				require('../include/inc_destroymed_addmedicine.php');
		...
		$listReport = $CabinetMedipot->getDetailDestroyInfo($des_id);
...
$smarty->assign('sMainBlockIncludeFile','nursing/medicine_destroymed.tpl');
