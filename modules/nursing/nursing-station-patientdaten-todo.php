...
if($mode=='save'){
	if(($dateput&&$timeput&&$berichtput&&$author)||($dateput2&&$berichtput2&&$author2)){
		...
		if($dateput2&&$berichtput2&&$author2){
			...
			$_POST['timeput']=$_POST['timeput2'].':00';
...
<td colspan=4 bgcolor="#ffcccc"><div class=fva2_ml10>

<?php

echo '<font size="6"  >'.$LDDocsPrescription.' </font><p><font size=1>'.$LDTraoDoiBSNote.'<p><font size=2>'.$LDPage.' 1/1
...
echo '	<tr bgcolor="#ffcccc">
		<td colspan=4 align="center"><div class=fva2_ml10><font color="#000099"><b>'.$LDDocsPrescription.'</b></div></td>
		<td colspan=4 align="center"><div class=fva2_ml10><font color="#000099"><b>'.$LDQueries.'</b></div></td>
		</tr>';	

echo '	<tr bgcolor="#ffcccc">
		<td><div class=fva2_ml3><b>'.$LDDate.'</b></div></td>
		<td><div class=fva2_ml3><b>'.$LDClockTime.'</b></div></td>
		<td><div class=fva2_ml3>&nbsp;</div></td>
		<td><div class=fva2_ml3><b>'.$LDSignature.'</b></div></td>
		<td><div class=fva2_ml3><b>'.$LDDate.'</b></div></td>
		<td><div class=fva2_ml3><b>'.$LDClockTime.'</b></div></td>
		<td><div class=fva2_ml3>&nbsp;</div></td>
		<td><div class=fva2_ml3><b>'.$LDSignature.'</b></div></td>
...
if($cnt){
	...
	for ($i=0;$i<$cnt;$i++){
		...
		<tr bgcolor="#ffcccc">';
			...
			$strbuf=str_replace('~~','</span>',wordwrap(stripcslashes(nl2br($buf['notes'])),35,"<br />\n",TRUE));	
			...
			<div class=fa2_ml3>'.$buf['personell_name'].'</div>
				...
				echo '&nbsp;</div>
				<td><div class=fa2_ml3>'.$buf['eff_time'].'</div></td>
				</td>
				<td><div class=fva2_ml3><i>';
				if(stristr($buf['eff_aux_notes'],'warn')) echo '<img '.createComIcon($root_path,'warn.gif','0','absmiddle',TRUE).'> ';
				$strbuf=str_replace('~~','</span>',wordwrap(stripcslashes(nl2br($buf['eff_notes'])),35,"<br />\n",TRUE));	
				echo str_replace('~','<span style="background:yellow">',$strbuf).'</i></div>
				</td>
				<td>
...
<!-- 		<a href="javascript:document.berichtform.dateput.value='h';setDate(document.berichtform.dateput);"><img <?php echo createComIcon($root_path,'arrow-t.gif','0') ?> alt="<?php echo $LDInsertDate ?>"></a>
 -->		
		...
		<input type=text size="4" id="timeput" name="timeput"  value="<?php echo date('H:i'); ?>"  onBlur="checkTime(this)" ><br>
					<script language="javascript">
						$(function(){
							$("#timeput").mask("**:**");
						});
...
<!-- 		<td valign="top"><?php echo $LDDate ?>:<br><input type=text size="8" name="dateput2" value="<?php if(!$saved) echo $dateput2; ?>" onKeyUp="setDate(this)" onFocus="this.select()"><br>
 ...
 -->		
		...
		<?php
		//gjergji : new calendar
		echo $calendar->show_calendar($calendar,$date_format,'dateput2',date('Y-m-d'));
		//end gjergji
		?>
</font>
...
<!-- 		<a href="javascript:document.berichtform.dateput2.value='h';setDate(document.berichtform.dateput2);"><img <?php echo createComIcon($root_path,'arrow-t.gif','0') ?> alt="<?php echo $LDInsertDate ?>"></a>
 -->		
		...
		<td valign="top"><?php echo $LDClockTime ?>:<br>
		<input type=text size="4" id="timeput2" name="timeput2"  value="<?php echo date('H:i'); ?>"  onBlur="checkTime(this)" ><br>
					<script language="javascript">
						$(function(){
							$("#timeput2").mask("**:**");
						});
