...
if($ward_ok){
   ...
   $sListRows='';
	...
	for ($i=$ward_info['room_nr_start'];$i<=$ward_info['room_nr_end'];$i++)
        //for ($i=1;$i<=$ward_info['room_nr_start'];$i++)
        {
		...
		for($j=1;$j<=$room_info['nr_of_beds'];$j++){
			...
			if($j==1){
				...
				$smarty->assign('sRoom',strtoupper($ward_info['roomprefix']).$room_info['room_nr']);
                                $smarty->assign('sRoomname',$room_info['roomname']);
			} else{
				$smarty->assign('sRoom','');
                                $smarty->assign('sRoomname','');
			}
			...
			if($edit){
				...
				if(!$bed_locked){
                 ...
                 //  $sAstart = $sAstart.$root_path.'modules/registration_admission/aufnahme_daten_zeigen.php'.URL_APPEND.'&encounter_nr='.$bed['encounter_nr'].'" title="'.$LDClk2Show.'">';
                }else{
			...
			if($edit){
				if(($is_patient)&&!empty($bed['encounter_nr'])){
                   ...
                   // $smarty->assign('sAdmitDataIcon','<a href="'.$root_path.'modules/registration_admission/aufnahme_daten_zeigen.php'.URL_APPEND.'&encounter_nr='.$bed['encounter_nr'].'" title="'.$LDAdmissionData.' : '.$LDClk2Show.'"><img '.createComIcon($root_path,'pdata.gif','0','',TRUE).' alt="'.$LDAdmissionData.' : '.$LDClk2Show.'"></a>');   //==>n
	...
	if($edit&&$patients_ok){
		...
		';
             //  An 2 icon Muc dich va Giay Vang
...
if(!$edit){
	$smarty->assign('sOpenWardMngmt','<a href="nursing-station-pass.php'.URL_APPEND.'&mode=ward&edit=1&rt=pflege&ward_nr='.$ward_nr.'&station='.$ward_info['name'].'"><img '.createComIcon($root_path,'uparrowgrnlrg.gif','0','absmiddle',TRUE).'> '.$LDOpenWardManagement.'</a>');
}
