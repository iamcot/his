...
if(isset($mode) && $mode=='save_beds'){
	$saved_ok=false;
	$_POST['dept_nr']=$ward_obj->getDeptNr();
	...
	//for($i=$room_nr_start;$i<=$room_nr_end;$i++)
        for($i=1;$i<=$room_nr_end;$i++)
        {

		if(!$ward_obj->RoomExists($_POST['room_nr'.$i])){
			$beds='beds'.$i;
			$info='info'.$i;
                        $_POST['type_nr'] = $_POST['type_nr'.$i];
			$roomname = 'roomname'.$i;
			$_POST['room_nr']=$_POST['room_nr'.$i];
			if(empty($$beds)) $$beds=DEFAULT_NR_OF_BEDS;
			$_POST['nr_of_beds']=$$beds;
			if(empty($$info)) $_POST['info'] = ' ';
				else $_POST['info']=$$info;
			if(empty($$roomname)) $_POST['roomname'] = ' ';
				else $_POST['roomname']=$$roomname;
...
$bgc=$root_path.'gui/img/skin/default/tableHeaderbg3.gif';
echo '<tr>
		<td background="'.$bgc.'"><font face="verdana,arial" size="2" >&nbsp;<b>'.$LDRoomNr.'</b></td>
		<td background="'.$bgc.'"><font face="verdana,arial" size="2" >&nbsp;<nobr><b>'.$LDRoomName.'</b></nobr></td>
		<td background="'.$bgc.'"><font face="verdana,arial" size="2" >&nbsp;<nobr><b>'.$LDRoomType.'</b></nobr></td>
...
$toggle=0;
	...
	//for($i=$ward['room_nr_start'];$i<=$ward['room_nr_end'];$i++)
		...
		echo '
	<tr bgcolor="'.$trc.'">
    <td>&nbsp;<font face="Verdana, Arial" size=2>'.strtoupper($ward['roomprefix']).' <input type="text" maxlength=4  style="width:40px" name="room_nr'.$i.'" value="'.$i.'">&nbsp;&nbsp;&nbsp;</td> 
	<td>&nbsp; <input type="text" name="roomname'.$i.'" style="width:200px" maxlength=150 value=""></td> 
	<td><select name="type_nr'.$i.'">'.$roomtypelist.'</select></td>
	<td>&nbsp;<font color="#ff0000"><b>*</b><input type="text" name="beds'.$i.'" size=2 maxlength=2 value="2" '.($ward['type']==1?"readonly=true":"").'>&nbsp;</td> 
...
<input type="hidden" name="room_nr" value="">
