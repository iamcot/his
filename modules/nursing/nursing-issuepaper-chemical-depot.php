<?php
    ...
    $fileforward='include/inc_issuepaper_savedepot_chemical.php'.URL_APPEND;
...
<!--
    function chkform(d) {
            document.createdepotform.action="include/inc_issuepaper_savedepot_chemical.php";
    ...
    function insertRow()
{
  ...
  var lastRow = tbl.tBodies[0].rows.length;
	...
	xmlhttp.open("GET","include/inc_issuepaper_addchemical.php?i="+idnum,true);
	xmlhttp.send();
...
{
	var win = 'include/search_chemical.php?' + 'id_number=' + id_number;
...
function Chemical_AutoComplete(i){
			var name_med='chemical'+i;
			var includeScript = "include/inc_issuepaper_autocomplete_chemical.php?mode=auto&k="+i;
...
{
	var process_file='include/inc_issuepaper_autocomplete_chemical.php?mode=filldata';
	var name_med='chemical'+i;
	...
	xmlhttp.open("GET",process_file+"&search="+document.getElementById('chemical'+i).value,true);
    xmlhttp.send();

}
/*    function Chemical_AutoComplete(i)
    {
        var process_file='include/inc_issuepaper_autocomplete_chemical.php?mode=auto';
            ...
            $.ajax({
                    ...
                    cache: false,
                    async: false,
                    data: "search="+document.getElementById(name_chemical).value,
                    success: function(data) {
                            var a = data;
                            var item = a.split('@');
                            var id_text = item[0];
                            var value_text = item[1];

                            var item_id = id_text.split('#');
                            var item_value = value_text.split('#');

                            var text=[{id: item_id[0],value: item_value[0]}, {id: item_id[1],value: item_value[1]}, {id: item_id[2],value: item_value[2]}, {id: item_id[3],value: item_value[3]},{id: item_id[4],value: item_value[4]},{id: item_id[5],value: item_value[5]},{id: item_id[6],value: item_value[6]},{id: item_id[7],value: item_value[7]},{id: item_id[8],value: item_value[8]},{id: item_id[9],value: item_value[9]},{id: item_id[10],value: item_value[10]}];

                            $("#"+name_chemical).autocomplete({
                                    source: text,
                                    select: function(event, ui) {
                                            var encode_number_unit_price = ui.item.id;
                                            var fill_value = encode_number_unit_price.split('_');

                                            $('#encoder'+i).val(fill_value[0]);
                                            $('input[name=inventory'+i+']').val(fill_value[1]);
                                            $('input[name=units'+i+']').val(fill_value[2]);
                            ...
                            $("#"+name_chemical).unbind("autocompleteselect");
                    }
    ...
    function Fill_Data(i)
    {
        var process_file='include/inc_issuepaper_autocomplete_chemical.php?mode=filldata';
            ...
            $.ajax({
                    ...
                    cache: false,
                    async: false,
                    data: "search="+document.getElementById('chemical'+i).value,                
                    success: function(data) {
                            var a = data.split("@#");
                            $('#encoder'+i).val(a[0]);
                            $('input[name=inventory'+i+']').val(a[1]);
                            $('input[name=units'+i+']').val(a[2]);
...
<?php
    ...
    $smarty->assign('titleForm',$LDIssuePaperChemical);
    ...
    $smarty->assign('LDInventory',$LDInventoryKC);
	$smarty->assign('LDTYPE',$LDTypePutIn1);
//echo $target.' '.$maxid.' '.$itemid;
...
if($target=='depot') 
{
	...
	if(!isset($mode) || $mode!='update'){
		...
		for ($i=1;$i<=$maxid;$i++){
			if($flag) {
				...
				if($listIssue = $Product->SearchExpCabinetChemical($dept_nr, $ward_nr, $condition)){
			...
			ob_start();
				require('./include/inc_issuepaper_addchemical.php');
		...
		$listIssue = $IssuePaper->getDetailChemicalIssuePaperInfo($issue_id);
	...
	if(!isset($mode)){
		...
		$listIssue = $Pres->sumChemicalByListPresId($list_pres_id);
		if(is_object($listIssue)){
			$sTempDivStt='';$sTempDiv='';
			$maxid=$listIssue->RecordCount();
			...
			for ($i=1;$i<=$maxid;$i++){
				...
				//$rowIssue['product_encoder'],$inventory
					$sql = "SELECT allocation_temp FROM care_chemical_products_main WHERE product_encoder='".$rowIssue['product_encoder']."'";
				...
				$sTempDivStt.='<tr bgColor="#ffffff">
                                                    <td align="center">
                                                        <input name="stt'.$i.'" type="text" size=1 value="'.$i.'" style="text-align:center;border-color:white;border-style:solid;" readonly />
                                                    </td>
                                                </tr>';			
			}	
			$sTempDiv = '<div id="myDiv">'.$sTempDiv.'</div>';		
		...
		if(is_object($listIssue)){
			...
			//	echo $inventory;
				...
				$sTempDivStt.='<tr bgColor="#ffffff">
                                                <td align="center" ><input name="stt'.$i.'" type="text" size=1 value="'.$i.'" style="text-align:center;border-color:white;border-style:solid;" readonly />
                                                    </td>
                                            </tr>';						
			}	
			$create_id= $rowIssue ['nurse'];
