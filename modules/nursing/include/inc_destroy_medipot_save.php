...
//echo $target;
$patmenu="../manage_pharma/medipot_destroy_medipot_list.php".URL_REDIRECT_APPEND."&pid=".$_SESSION['sess_pid']."&ward_nr=".$_POST['ward_nr'].'&dept_nr='.$_POST['dept_nr'];
...
switch($target){
		...
		case 'update': 
				...
				if($Cabinet->updateDataFromArray($pharma_destroy_info,$id)) {
					...
					$Cabinet->useCabinetDestroy('des');
					$Cabinet->deleteAllMedipotInDestroy($id);
					$n = $_POST['maxid'];
					for ($j=1; $j<=$n; $j++)
					{
						if($_POST['medicine'.$j]!=""){
