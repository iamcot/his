...
//echo $target;
$patmenu="../manage_pharma/chemical_use_ward_list.php".URL_REDIRECT_APPEND."&pid=".$_SESSION['sess_pid']."&ward_nr=".$_POST['ward_nr'].'&dept_nr='.$_POST['dept_nr'];
...
switch($target){
		...
		case 'create':
				//destroy info
				$use_id = $Cabinet->getLastUseChemicalID();
				if ($use_id==false)
					$id=1;
				else
					$id = $use_id['use_id']+1;
				...
				$Cabinet->useCabinetUseChemical('use_info');
				...
				//return sub
				$Cabinet->useCabinetUseChemical('use');
				$n = $_POST['maxid'];
				for ($j=1; $j<=$n; $j++)
				{
					if($_POST['chemical'.$j]){
		...
		case 'update': 
				...
				if($Cabinet->updateDataFromArray($pharma_use_info,$id)) {
					...
					$Cabinet->useCabinetUseChemical('use');
					$Cabinet->deleteAllChemicalInUse($id);
					$n = $_POST['maxid'];
					for ($j=1; $j<=$n; $j++)
					{
						if($_POST['chemical'.$j]!=""){
