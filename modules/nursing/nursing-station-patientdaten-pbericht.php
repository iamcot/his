	...
	if($mode=='save'){
		if(($dateput&&$timeput&&$author)){
			...
			if($dateput&&$timeput&&$author){
				...
				$_POST['morenote']=deactivateHotHtml($berichtput2);
				if($report_obj->saveNursingReport($_POST)){
					$logs->writeline_his($_SESSION['sess_login_userid'], $thisfile, $report_obj->getLastQuery(), date('Y-m-d H:i:s'));
...
function pruf(d){
	if((d.dateput.value)&&(d.timeput.value)&&(d.author.value)) 
		return true;
	else 
	{
...
function printOut(enc){
	urlholder="<?php echo $root_path; ?>modules/pdfmaker/dieuduong/phieuchamsoc.php<?php echo URL_REDIRECT_APPEND; ?>&pn="+enc;
	window.open(urlholder,'PhieuChamSoc',"width=1000,height=800,menubar=no,resizable=yes,scrollbars=yes");
}
...
function updateitem(nr){
	urlholder="update_phieuchamsoc.php?nr="+nr+"<?php echo "&sid=$sid&lang=$lang&edit=$edit&station=$station&pn=$pn"; ?>";
	updatevalue=window.open(urlholder,"update","width=900,height=250,menubar=no,resizable=yes,scrollbars=yes");
}
function refresh(){
...
<tr  valign="top">
<td colspan=3 bgcolor="#99ccff"  width="50%">
<?php
...
<?php
//echo '	<tr bgcolor="#99ccff"><td colspan=5><div class=fva2_ml10><font color="#000099"><b>'.$LDNursingReport.'</b></div></td></tr>';	

echo '	<tr bgcolor="#99ccff">
		<td><div class=fa2_ml3><b>'.$LDDate.'</b></div></td>
		<td><div class=fa2_ml3><b>'.$LDHour1.'</b></div></td>
		<td><div class=fa2_ml3><b>'.$LDTheoDoiDienBien.'</b></div></td>
		<td><div class=fa2_ml3><b>'.$LDThucHienYLenh.'</b></div></td>
		<td><div class=fa2_ml3><b>'.$LDSignature.'</b></div></td>
		<td><div class=fa2_ml3><b>'.$LDEdit1.'</b></div></td>
		</tr>';	
...
if($cnt){
	...
	for ($i=0;$i<$cnt;$i++){
		...
		/*if($buf['type_nr']==17){
			...
			$buf['personell_name']='';
			$buf['aux_morenote']=$buf['aux_notes'];
			$buf['aux_notes']='';
		}*/
		echo '	
		<tr bgcolor="#99ccff">';
			...
			<td><div class=fa2_ml3>'.$buf['time'].'</div></td>
			<td><i>';
			if(stristr($buf['aux_notes'],'warn')) echo '<img '.createComIcon($root_path,'warn.gif','0','absmiddle',TRUE).'> ';
			$strbuf=str_replace('~~','</span>',wordwrap(stripcslashes(nl2br($buf['notes'])),40,"\n",TRUE));	
			echo str_replace('~','<span style="background:yellow">',$strbuf).'</i>
			</td>
			...
			// Column for the effectivity report
				echo '
				<td><i>';
				if(stristr($buf['aux_morenote'],'warn')) echo '<img '.createComIcon($root_path,'warn.gif','0','absmiddle',TRUE).'> ';
				$strbuf=str_replace('~~','</span>',wordwrap(stripcslashes(nl2br($buf['morenote'])),40,"\n",TRUE));	
				echo str_replace('~','<span style="background:yellow">',$strbuf).'</i> 
				</td>
				<td>
			...
			if($buf['nr'])
				echo '<a href="javascript:updateitem(\''.$buf['nr'].'\')"><img '.createComIcon($root_path,'pencil.gif','0').' title="'.$LDUpdate.'" ></a>';
		echo'</td></tr>';	
	}
}
...
<?php
		...
		echo $calendar->show_calendar($calendar,$date_format,'dateput',date('Y-m-d'));
		//end gjergji
?>
</font>
...
<!-- 		<a href="javascript:document.berichtform.dateput.value='h';setDate(document.berichtform.dateput);"><img <?php echo createComIcon($root_path,'arrow-t.gif','0') ?> alt="<?php echo $LDInsertDate ?>"></a>
 -->		
		...
		<input type=text size=4 maxlength=5 id="timeput" name="timeput"  value="<?php echo date('H:i'); ?>" onBlur="checkTime(this)" ><br>
					<script language="javascript">
						$(function(){
							$("#timeput").mask("**:**");
						});
					</script>
		<td><?php echo $LDTheoDoiDienBien ?>:<br>&nbsp;<textarea rows="4" cols="25" name="berichtput"><?php if(!$saved) echo $berichtput; ?></textarea><br>
		...
		<td><?php echo $LDThucHienYLenh ?>:<br>&nbsp;<textarea rows="4" cols="25"  name="berichtput2"><?php if(!$saved) echo $berichtput2; ?></textarea><br>
		...
		<td valign="top" colspan="2"><?php echo $LDSignature ?>:<br><textarea rows="4" cols="10" name="author"><?php echo $_SESSION['sess_login_userid']; ?></textarea>
		</td>		
		</tr>
...
<a href="javascript:printOut('<?php echo $pn; ?>')"><img <?php echo createLDImgSrc($root_path,'printout.gif','0') ?>  width=99 height=24 alt="<?php echo $LDPrint; ?>"></a>
&nbsp;
