...
if($dblink_ok)
{
	...
	if($mode=='save'){
		...
		if($isdelete=='delete'){
			if($editid!=''){
				$q="delete from care_encounter_custom_ddc where nr = '".$editid."' ";
		...
		elseif($indatetime_date&&$indatetime_time&&$bloodsugar){
		...
		if ($_POST['editid'] != "") {
			...
			$indatetime_date=formatDate2STD($indatetime_date,$date_format);
			$indatetime_time=$_POST['indatetime_time'].':00';
			$q="insert into care_encounter_custom_ddc (encounter_nr,createid,indatetime,time,urinesugar,acetone,bloodsugar,tablets,insulin) values (
				'".$pn."',
...
function pruf(d){
	/*if(isNaN(d.urinesugar.value)){
		alert("<?php echo $LDAlertInsertNumber; ?>");
		d.urinesugar.focus();
		return false;
	}else if(isNaN(d.bloodsugar.value)){
		alert("<?php echo $LDAlertInsertNumber; ?>");
		d.bloodsugar.focus();
		return false;
	}else if(isNaN(d.insulin.value)){
		alert("<?php echo $LDAlertInsertNumber; ?>");
		d.insulin.focus();
		return false;
	}*/
	
	if((d.indatetime_date.value)&&(d.bloodsugar.value)) 
		return true;
	else 
	{
		alert("<?php echo $LDInsertLuongDuongTrongMau; ?>");
		return false;
	}
}
...
<tr  valign="top">
<td colspan=3 bgcolor="#99ccff"  width="40%">
<?php
...
<td colspan=5 bgcolor="#99ccff"><div class=fva2_ml10>

<?php

echo '<font size="4">'.$DDC_title.' <p><font size=2>';
...
echo '	<tr bgcolor="#99ccff">
		...
		function deleteRecord()
		{
			...
			document.berichtform.action='nursing-station-patientdaten-custom-ddc.php<?php echo URL_REDIRECT_APPEND?>&isdelete=delete&station=<?php echo $station ?>&pn=<?php echo $pn ?>&edit=<?php echo $edit; ?>&dblink_ok=1&editid='+editid;
...
?>
		...
		<input type=text size=15 name="urinesugar"  id="urinesugar" value="<?if (!($saved)) echo $_POST['urinesugar']?>">
		</td>
		<td valign="top"><?php echo $LDAcetone ?>:<br>
		<input type=text size=15 name="acetone"  id="acetone" value="<?if (!($saved)) echo $_POST['acetone']?>">
		</td>
		<td valign="top"><?php echo $LDBloodSugar ?>:<br>
		<input type=text size=15 name="bloodsugar"  id="bloodsugar" value="<?if (!($saved)) echo $_POST['bloodsugar']?>">
		</td>
		<td valign="top"><?php echo $LDTablets ?>:<br>
		<input type=text size=15 name="tablets"  id="tablets" value="<?if (!($saved)) echo $_POST['tablets']?>">
		</td>
		<td valign="top"><?php echo $LDInsulin ?>:<br>
		<input type=text size=10 name="insulin"  id="insulin" value="<?if (!($saved)) echo $_POST['insulin']?>">
		</td>
	<td align=center>
	<a <? echo 'href=\'nursing-station-patientdaten-custom-ddc.php'.URL_REDIRECT_APPEND.'&station='.$station.'&pn='.$pn.'&edit='.$edit.'\''; ?>> <span style="background-color:white"> <?php echo $LDRefresh; ?> </span> </a><p>
