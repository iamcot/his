...
function getrem(pn){
	...
	patientwin=window.open(urlholder,pn,"width=800,height=500,menubar=no,resizable=yes,scrollbars=yes");
	}
...
$smarty->assign('LDFullname',$LDFullname);
...
if($ward_ok){
	...
	for ($i=$ward_info['room_nr_start'];$i<=$ward_info['room_nr_end'];$i++){
		...
		for($j=1;$j<=$room_info['nr_of_beds'];$j++){
			...
			//var_dump(strtoupper($ward_info['roomprefix']).$room_info["room_nr"].' '.$room_info["roomname"]);
			# If bed nr  is 1, show the room number
			if($j==1){
				$smarty->assign('sRoom',(strtoupper($ward_info['roomprefix']).$room_info["room_nr"].' '.$room_info["roomname"]));
			} else{
				$smarty->assign('sRoom','');
			}
