...
// $ward_info=&$ward_obj->getWardsItemsObject_dieutri($items, $_SESSION['department_nr']);
//edit 02012012 - cot
 $ward_info=&$ward_obj->getWardsItemsObject_dieutri($items, '');
 ...
 */
 ...
 if(is_object($ward_info))
 {
  ...
  while($stations=$ward_info->FetchRow()) {
   $sWardInfo = $sWardInfo.'<tr><td><a href="'.strtr('nursing-station-pass.php'.URL_APPEND.'&mode=ward&rt=pflege&edit=1&station='.$stations['ward_id'].'&location_id='.$stations['ward_id'].'&ward_nr='.$stations['nr'].'&dept_nr='.$stations['dept_nr'],' ',' ').'"><div class="wardname"><li>'.$stations['ward_id'].'&nbsp;</div></a> ';
   $sWardInfo = $sWardInfo."\n";
   $sWardInfo = $sWardInfo.'</td><td>'.$stations['name'].'</td>';
   $sWardInfo = $sWardInfo.'</td><td>'.$stations['dept_name'].'</td></tr>';
  }
...
$aSubMenuItem = array('LDNursingStations' => $LDNursingStations." <img ".createComIcon($root_path,'dwn-arrow-grn.gif','0','absmiddle').">",
										'LDIssuePaper' => "<a href=\"nursing-station-pass.php".URL_APPEND."&mode=issue&station=$LDCabinetMedicine\">$LDCabinetMedicine</a>",
