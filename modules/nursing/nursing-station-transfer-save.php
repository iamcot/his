		...
		switch($mode){
			...
			case 'transferward': 
			{
				if($ward_obj->ReplaceWard($pn,$trwd)){
					...
					//var_dump($pn);
					$ecrrtrans = $enc_obj->getEncounterCurrTrans($pn);		
					//var_dump($ecrrtrans);
					if($ecrrtrans != null){
					//$ecrrtrans['dateout'] = date("Y-m-d H:i:s");
					$ecrrtrans['status'] = "1";
					$enc_obj->updateOldEncounterTrans($ecrrtrans['nr'],$ecrrtrans);
					$encounter_transfer = array(
												...
												"dept_to" => $ward_obj->getDeptFromWard($trwd),
												"home" => "",
												"status" => "",
												"login_id" => $_SESSION['sess_login_userid'],
												"type_encounter" => $ecrrtrans['type_encounter']
												);			
					//var_dump($encounter_transfer);
					$enc_obj->insertEncounterTransfer($encounter_transfer);
					}
					header($forwardfile);
					exit;
				}
