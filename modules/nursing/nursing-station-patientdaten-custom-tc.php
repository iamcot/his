...
$custom_lang_file = $root_path."language/".$lang.'/lang_vi_custom.php';
...
if($dblink_ok)
{
	...
	if($mode=='save'){
		if($isdelete=='delete'){
			if($editid!=''){
				$q="delete from care_encounter_custom_tc where nr = '".$editid."' ";
...
function pruf(d){
	...
	if((d.indatetime_date.value)&&(d.turntime.value)) 
		return true;
	else 
	{
...
echo '<font size="4">'.$TC_title.' </font><p>'.$LDNoteTurningChart.'<p><font size=1>'.$LDNoteTurningChartTxt.'</font>';
...
function deleteRecord()
{
	...
	document.berichtform.action='nursing-station-patientdaten-custom-tc.php<?php echo URL_REDIRECT_APPEND?>&isdelete=delete&station=<?php echo $station?>&pn=<?php echo $pn?>&edit=<?php echo $edit; ?>&dblink_ok=1&editid='+editid;
...
function EditRecord(id,rec) {
	...
	// prefill date
	document.getElementsByName('indatetime_date')[0].value=eval("document.getElementById('date_'+id).value");
...
<table width=215 style="font-size:11" bgcolor="#ffffff" cellpadding=0 cellspacing=0>
<tr>
...
if (stristr($LDPRONE,$pos)) {$color="#fdad29";}
if (stristr($LDHALF_LYING_SITTING,$pos)) {$color="#A4A4A4";}
if (stristr($LDLIELOW,$pos)) {$color="#B40486";}
if (stristr($LDLIEHIGH,$pos)) {$color="#0B3861";}
...
<div id="<?php echo $id?>">
<table height=10 width=568 bgcolor="<?php echo $color?>" cellpadding=5 cellspacing=0>
	<tr><td>
		<input type=hidden id="date_<?php echo $tc?>" value="<?php echo date("d/m/Y",strtotime($var))?>">
		<span style="font-size:12px"><font color=white><b><span id="time_<?php echo $tc?>"><?php echo date("H:i",strtotime($time))?></span> <span><?php echo " (".date("h:i a",strtotime($time)).")"?></span></b> : <span id="pos_<?php echo $tc?>"><?php echo $pos?></span></font></font>
		</td>
		<td align=right><span style="background-color:yellow">&nbsp;<a href="javascript:EditRecord('<?php echo $tc?>','<?php echo $id?>')"><?php echo $LDEDIT ?></a>&nbsp;</span>
	</td>
</tr></table>
</div>
...
<br>
<table bgcolor="#99ccff" width=568 >
<tr>
...
<?php echo $LDTime?><br>
<table><tr><td>
	<input size=10 type=text name="turntime" id="turntime" maxlength=5 value="<?php echo date('H:i') ?>" onBlur="checkTime(this)" ></td><td>HH:MM (13:00)
	<script language="javascript">
		$(function(){
			$("#turntime").mask("**:**");
		});
...
<option><?php echo $LDPRONE ?></option>
<option><?php echo $LDHALF_LYING_SITTING ?></option>
<option><?php echo $LDLIELOW ?></option>
<option><?php echo $LDLIEHIGH ?></option>
</select>
</td></tr></table>
</td>

<td>
	<a href='nursing-station-patientdaten-custom-tc.php<?php echo URL_REDIRECT_APPEND?>&station=<?php echo $station?>&pn=<?php echo $pn?>&edit=<?php echo $edit?>'><span style="background-color:white">&nbsp;<?php echo $LDRefresh; ?>&nbsp;</span></a>
	<p>
	<a href="javascript:deleteRecord()"><span style="background-color:white">&nbsp;<?php echo $LDDelete; ?>&nbsp;</span></a>
</td>
</tr></table>
