...
//		else $cond.=' AND p.pid=e.pid GROUP BY r.location_nr';
	...
	if($dblink_ok){			
		$sqlselect="SELECT r.location_nr AS room_nr, p.name_last, p.name_first,p.date_birth,
					e.encounter_nr, e.encounter_class_nr, e.in_ward, e.current_dept_nr, e.current_ward_nr,
...
var urlholder;
  ...
  function gotoWard(ward_nr,dept_nr,st,y,m,d){
<?php
	if($cfg['dhtml'])
	{
...
?>
	...
	urlholder="nursing-station-pass.php?rt=pflege&sid=<?php echo "$sid&lang=$lang"; ?>&mode=ward&pday="+d+"&pmonth="+m+"&pyear="+y+"&edit=1&retpath=search_patient&ward_nr="+ward_nr+"&dept_nr="+dept_nr+"&station="+st;
...
?>
	<table border=0>
		<tr>
			...
			<td class="prompt">
				<?php echo "$LDSearchKeyword <font color=#0000ff>\"$searchkey\"</font> ".str_replace("~rows~",$totalcount,$LDWasFound)?> <br>                                
				<?php echo $LDPlsClk ?> <br>
                                <?php echo $LDShowing.' '.$pagen->BlockStartNr().' '.$LDTo.' '.$pagen->BlockEndNr(); ?> <br>
			</td>
		</tr>
	</table>
...
*/		
	...
	$buf="javascript:gotoWard('".$result['ward_nr']."','".$result['current_dept_nr']."','".addslashes($result['ward_name'])."','$pyear','$pmonth','$pday')";
