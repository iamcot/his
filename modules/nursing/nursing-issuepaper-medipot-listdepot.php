...
$breakfile='nursing-issuepaper-medipot-depot.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr;
$fileforward='nursing-issuepaper-medipot-depot.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr;
...
require_once($root_path.'include/care_api_classes/class_issuepaper_medipot.php');
$IssueMedipot = new IssueMedipot;
...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDIssueMedipot.' :: '.$LDListDepotTabMed);

 # href for help button
 $smarty->assign('pbHelp',"javascript:gethelp('submenu1.php','$LDIssueMedipot')");

 $smarty->assign('breakfile',$breakfile);

 # Window bar title
 $smarty->assign('title',$LDIssueMedipot);
...
{
	var win = 'include/inc_issuepaper_medipot_showdetail.php<?php echo URL_APPEND; ?>' +'&issue_id=' + issue_id;
...
ob_start();
	require('./include/inc_issuepaper_medipot_tabs.php');
...
if ($search==''){
	...
	$total_items=$IssueMedipot->countIssuePaperItems($condition);
	...
	$listIssue = $IssueMedipot->listSomeIssuePaperSplitPage($current_page,$number_items_per_page,$condition);
	...
	$listIssue = $IssueMedipot->listAllIssuePaper($condition);
	$breakfile='nursing-issuepaper-medipot-listdepot.php'.URL_APPEND.'&ward_nr='.$ward_nr;
}
...
if(is_object($listIssue)){
	...
	for ($i=0;$i<$listIssue->RecordCount();$i++)
	{
		...
		if ($rowIssue['status_finish']){
			...
			$editbutton='nursing-issuepaper-medipot-depot.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr.'&issue_id='.$rowIssue['issue_paper_id'].'&mode=update&target='.$target;
	...
	$sTemp='<tr bgColor="#ffffff"><td colspan="9">'.$LDIssueMedipotNotFound.'</td></tr>';
	$smarty->assign('listItem',$sTemp);
}
