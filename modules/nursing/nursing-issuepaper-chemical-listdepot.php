<?php
    ...
    $breakfile='nursing-issuepaper-chemical-depot.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr;
    $fileforward='nursing-issuepaper-chemical-depot.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr;
    ...
    */
    ...
    $smarty->assign('sToolbarTitle',$LDIssuePaperChemical.' :: '.$LDListChemicalDepotTab);
...
{
	var win = 'include/inc_issuepaper_chemical_showdetail.php<?php echo URL_APPEND; ?>' +'&issue_id=' + issue_id;
...
<?php
    ...
    if ($search==''){
            ...
            $total_items=$IssuePaper->countIssuePaperChemicalItems($condition);
            ...
            $listIssue = $IssuePaper->listSomeIssuePaperChemicalSplitPage($current_page,$number_items_per_page,$condition);
            ...
            $listIssue = $IssuePaper->listAllIssuePaperChemical($condition);
            $breakfile='nursing-issuepaper-chemical-listdepot.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr;
    }
    ...
    if(is_object($listIssue)){
        ...
        for ($i=0;$i<$listIssue->RecordCount();$i++)
        {
            ...
            if ($rowIssue['status_finish']){
                    ...
                    $editbutton='nursing-issuepaper-chemical-depot.php'.URL_APPEND.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr.'&issue_id='.$rowIssue['issue_paper_id'].'&mode=update&target='.$target;
            ...
            $sTemp=$sTemp.'<tr bgColor="#ffffff" >
                                <td align="center"><input OnClick="viewDetail('.$rowIssue['issue_paper_id'].')" type="image" '.createComIcon($root_path,'info3.gif','0','',TRUE).'></td>
        ...
        $sTemp='<tr bgColor="#ffffff"><td colspan="9">'.$LDIssueChemicalNotFound.'</td></tr>';
        $smarty->assign('listItem',$sTemp);
    }
