...
if(!isset($mode)){
	...
	$redirect=true;
	include('./include/save_soket.php');
}
...
if($mode=='show') //xem tom tat chi can show phan ket qua
{
	$sql="SELECT e.encounter_nr, e.is_discharged, nd.*, type.sort_nr  
		...
		WHERE  e.encounter_nr='".$encounter_nr."'
			...
			AND nd.type_nr='45'
			ORDER BY nd.short_notes, type.sort_nr";
...
}elseif(($mode=='update')||($mode=='details')&&!empty($encounter_nr)&&!empty($time_nr)){
...
/* Load GUI page */
require('./gui_bridge/default/gui_show_soket.php');
?>
