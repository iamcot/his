...
if(isset($mode)&&($mode=='search'||$mode=='paginate')&&!empty($searchkey)){
	...
	$address=$address_obj->searchLimitActivePhuongXa($searchkey,$GLOBAL_CONFIG['pagin_address_search_max_block_rows'],$pgx,$oitem,$odir);
	...
	if(isset($totalcount) && $totalcount){
		...
		$totalcount=$address_obj->searchCountActivePhuongXa($searchkey);
		$pagen->setTotalDataCount($totalcount);
	}
...
# Title in toolbar
 $smarty->assign('sToolbarTitle',"$LDAddress :: $LDSearchPhuongXa");
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"$LDAddress :: $LDSearchPhuongXa");
...
<table border=0 cellpadding=2 cellspacing=1>
  <tr class="wardlisttitlerow">
	  ...
	  <?php 
		...
		echo $pagen->SortLink($LDPhuongXaName,'name',$odir,$flag); 
			 ?></b>
	</td>

      <td><b>
	  <?php 
	  	if($oitem=='quanhuyen_id') $flag=TRUE;
			else $flag=FALSE; 
		echo $pagen->SortLink($LDQuanHuyenName,'quanhuyen_id',$odir,$flag); 
			 ?></b>
	</td>
	 <td><b>
	  <?php 
	  	if($oitem=='citytown_id') $flag=TRUE;
			else $flag=FALSE; 
		echo $pagen->SortLink($LDCityTown,'citytown_id',$odir,$flag); 
			 ?></b>
	</td>
...
?>
  <tr  class="<?php echo $bgc ?>">
    <td><a href="phuongxa_info.php<?php echo URL_APPEND.'&retpath=search&nr='.$addr['nr']; ?>"><?php echo $addr['name']; ?></a></td>
     <?php $sql="SELECT name FROM care_address_quanhuyen WHERE nr='".$addr['quanhuyen_id']."'";
  
  $buf=$db->Execute($sql);
  $buf2=$buf->FetchRow();
  echo '<td>'.$buf2['name'].'</td>';
   ?>
 <?php $sql="SELECT name FROM care_address_citytown WHERE nr='".$addr['citytown_id']."'";
