...
if(isset($mode)&&($mode=='search'||$mode=='paginate')&&!empty($searchkey)){
	...
	$address=$address_obj->searchLimitActiveQuanHuyen($searchkey,$GLOBAL_CONFIG['pagin_address_search_max_block_rows'],$pgx,$oitem,$odir);
	...
	if(isset($totalcount) && $totalcount){
		...
		$totalcount=$address_obj->searchCountActiveQuanHuyen($searchkey);
		$pagen->setTotalDataCount($totalcount);
	}
...
# Title in toolbar
 $smarty->assign('sToolbarTitle',"$LDAddress :: $LDSearchQuanHuyen");
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"$LDAddress :: $LDSearchQuanHuyen");
...
<table border=0 cellpadding=2 cellspacing=1>
  <tr class="wardlisttitlerow">
	  ...
	  <?php 
		...
		echo $pagen->SortLink($LDQuanHuyenName,'name',$odir,$flag); 
			 ?></b>
	</td>
...
?>
  <tr  class="<?php echo $bgc ?>">
    <td><a href="quanhuyen_info.php<?php echo URL_APPEND.'&retpath=search&nr='.$addr['nr']; ?>"><?php echo $addr['name']; ?></a></td>
...
<form action="quanhuyen_new.php" method="post">
