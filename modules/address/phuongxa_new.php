	...
	switch($mode)
	{
		case 'save':
		{
			...
			if(!empty($_POST['name'])){
				...
				#
				if($address_obj->PhuongXaExists($_POST['name'],$_POST['quanhuyen_id'])){
					#
					# Do notification
					#
					$mode='phuongxa_exists';
				}else{
					if($address_obj->savePhuongXaInfoFromArray($_POST)){
						#
...
# Title in toolbar
 $smarty->assign('sToolbarTitle',"$LDAddress :: $LDNewPhuongXa");
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"$LDAddress :: $LDNewPhuongXa");
...
function check(d)
{
	if((d.name.value=="")){
		...
		alert("<?php echo $LDEnterQuanHuyenName.'\n'.$LDEnterQMark; ?>");
...
<table border=0>
  <tr>
    <td align=right class="adm_item"><font color=#ff0000><b>*</b></font> <?php echo $LDPhuongXaName ?>: </td>
