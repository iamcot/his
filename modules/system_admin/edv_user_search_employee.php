...
*/
$lang_tables=array('personell.php','edp.php', 'departments.php');
...
if(($mode=='search')and($searchkey)){	
	...
	$sql="SELECT ps.nr, ps.is_discharged, p.name_last, p.name_first, p.date_birth, u.login_id, dept.LD_var
              FROM care_person as p
              LEFT JOIN care_personell as ps ON ps.pid=p.pid
              LEFT JOIN care_personell_assignment as pns ON pns.personell_nr=ps.nr
              LEFT JOIN care_department as dept ON dept.nr=pns.location_nr
              LEFT JOIN care_users AS u ON u.personell_nr=ps.nr ";
	if($numeric) $sql.="WHERE ps.nr=".$suchbuffer."";
		else $sql.= "WHERE CONCAT(p.name_last,' ', p.name_first) LIKE '".addslashes($suchwort)."%'
                                OR CONCAT(p.name_last,' ', p.name_first) LIKE '%".addslashes($suchwort)."%'
                                OR p.name_last LIKE '".addslashes($suchwort)."%'
			...
			if(($linecount==1) && $numeric){
				...
				if(!empty($zeile['login_id'])){
					$append='&mode=edit&userid='.$zeile['login_id'];
						...
						$append='&is_employee=1&personell_nr='.$zeile['nr'].'&username='.strtr(($zeile['name_last']).' '.$zeile['name_first'],' ','+').'&userid='.strtr($zeile['name_last'].' '.$zeile['name_first'],' ','_');
				}
...
?>
		...
		<td><b><?php echo $LDLastName; ?>&nbsp;<?php echo $LDFirstName; ?></td>
                <td><b><?php echo $LDDept; ?></b></td>
		<!--//gjergji
		//changed to search only the users table-->
...
<?php
		while($zeile=$ergebnis->FetchRow()){
			...
			if(!empty($zeile['login_id'])){
				...
				$mode='';
				$alt=$LDCreate;
				$append='&is_employee=1&personell_nr='.$zeile['nr'].'&username='.strtr(($zeile['name_last'].' '.$zeile['name_first']),' ','+').'&userid='.strtr($zeile['name_last'],' ','_');
			}
			echo "</td>";
			...
			echo "</td>";
			echo"<td>";
			echo "&nbsp;".ucfirst($zeile['name_last']).' '.ucfirst($zeile['name_first']);
			echo "</td>";
                        echo"<td>";
			echo "&nbsp;".$$zeile['LD_var'];
			echo "</td>";
