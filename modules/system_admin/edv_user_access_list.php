...
<?php
    ...
    echo '<table border=0 class="frame" cellpadding=0 cellspacing=0>
                <tr>
                    <td>
                        <table border="0" cellpadding="5" cellspacing="1" width="100%">';
                            echo '<tr class="submenu">';
                                echo "<td colspan=8 align='center'><font color=\"#800000\"><b>$LDActualAccess</b></font></td>";
                                echo "</tr>"; 
                            ...
                            if($zeile['user_role'] != $old_role) {
                                ...
                                echo '<tr class="wardlistrow2"><td colspan=8>
                                        <span style="cursor:pointer;font-weight:bold;float: left;" onClick="new Effect.toggle(\''.$zeile['user_role'].'\', \'blind\' );" />
                                            <font face=verdana,arial size=2 color=maroon>-> '.$role->role['role_name'].'</font>
                                        </span><br>';
...
//                            echo $sTemp . "</td>\n<td>\n";
//                            if ($zeile['lockflag'])
//                                echo '<img '.$img_padlock.'>';
//                            else echo '<img '.$img_arrow.'>';
//                                echo "</td>\n <td>";

                            /* Display the permitted areas */
                            $area=explode(' ',$zeile['permission']);
                            for($n=0;$n<sizeof($area);$n++) echo $area_opt[$area[$n]].'<br>';															
                            echo '</td><td> '.convertTimeToLocal($zeile['s_time']).'<br> Ngày '.formatDate2Local($zeile['s_date'],$date_format).'</td>';
                            echo '<td>'.$zeile['create_id'].'</td>';
                            echo "<td width='10%'>
                                    <a href=edv_user_access_edit.php?sid=$sid&lang=$lang&mode=edit&userid=".str_replace(' ','+',$zeile['login_id'])." title=\"$LDChange\"> $LDChange</a>
                                    <br><br>
                                    <a href=edv_user_access_lock.php?sid=$sid&lang=$lang&itemname=".str_replace(' ','+',$zeile['login_id'])." ";
                            if ($zeile['lockflag']) echo "title=\"$LDUnlock\" > $LDInitUnlock"; else echo "title=\"$LDLock\"> $LDLock";
                            echo "</a>
                                  <br><br>
                                  <a href=edv_user_access_delete.php?sid=$sid&lang=$lang&itemname=".str_replace(' ','+',$zeile['login_id'])." title=\"$LDDelete\">$LDDelete</a>
                                  </td>";
                            echo "</tr>";
                            if($zeile['user_role'] != $old_role) {
