    ...
    for (int j=0; j < tileR; j++) {
      ...
      for (int i=0; i < tileC; i++) {
        ...
        if(showStudyInfo) {
          ...
          // 右上
          // 文字列の長さがもっとも長いものを探す
          if(studyID[imgNo] != null && maxlength < fm.stringWidth(studyID[imgNo]))
			maxlength = fm.stringWidth(studyID[imgNo]);
          if(studyDate[imgNo] != null && maxlength < fm.stringWidth(studyDate[imgNo]))
			maxlength = fm.stringWidth(studyDate[imgNo]);
          if(studyTime[imgNo] != null && maxlength < fm.stringWidth(studyTime[imgNo]))
