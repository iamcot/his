...
public class hGrid extends Panel implements KeyListener, MouseMotionListener, MouseListener, AdjustmentListener {
  // drawing
  Dimension dMinimum;
  Font fFont;
  ...
  public void setCallback(ActionListener c) {
    cCallback=c;
  }
  ...
  public void keyPressed(KeyEvent evt) {
    ...
    if(r<0 || r>=iRowCount) return;
    switch(key) {
    case 9: // Tab
      if((evt.getModifiers() & Event.SHIFT_MASK)!=0) c--;
      else c++;
  ...
  public void mousePressed(MouseEvent e) {
    int x = e.getX();
    int y = e.getY();
    if(iRowHeight==0 || x>iWidth || y>iHeight) return;
    x+=iX-iRowHeadWidth;
    int col=0;
    for(int i=0;i<iColCount;i++) {
      int w=iColWidth[i];
    ...
    if(y>iRowHeight) {
      ...
      if(r>=0 && r<iRowCount) {
        select(r,col);
        if(cCallback!=null) {
          ActionEvent v=new ActionEvent(
            this,Event.ACTION_EVENT,""+iSelectRow);
          cCallback.actionPerformed(v);
        }
      }
    } else if(!bDrag) {
      sort(col);
