...
public class Viewer extends Applet {
  ...
  public String[][] getParameterInfo() {
    String[][] pinfo =
    {
      {"isLtlEndian", "boolean", "転送構\文。LittleEndianならtrue"},
  ...
  public void init() {
    ...
    try {
      ...
      for (int i=0; i<NUM ; i++) {
          ...
          if (debug_level > 3) System.out.println("Parameter imgURL: " + imgURL[i]);
      }
  ...
  private void jbInit() throws Exception {
    ...
    // タグボタン関係
    tag_B.addMouseListener(new MouseListener () {
      public void mouseClicked(MouseEvent e) {
        if (tag_B.getLabel().indexOf("Show") != -1) {
  ...
  // スレッドからのデータを受け取る。
  public synchronized boolean postData(int indexNo, int start, int end) {
    int len = dicomData_tmp.length;
    int arrayNo =0;
    int remove_index =0;
    ...
    if (debug_level > 3) System.out.println("Now Loading imgURL: " + imgURL[indexNo]);
    // Dicomイメージを生成する。
    showStatus("Now Creating Dicom Image....  (" + (indexNo+1) + ")");
    if (!tmpimageData.setData(tmpdicomData))
    {
        showStatus("Dicom File....  (" + (indexNo+1) + ") Contains No Pixel Data");
        return false;
    }
