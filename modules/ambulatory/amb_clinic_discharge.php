		...
		if($released){
			if($discharged_type!=6){
                            ...
                            if($_POST['chandoan_notes']){
                                $data_array['notes']=$_POST['chandoan_notes'].'@@'.$_POST['chandoan_notes_BP'];
...
$(function(){
	...
	urlholder="../nursing/chuyenvien_classifications.php<?php echo URL_REDIRECT_APPEND.'&name='; ?>"+document.getElementById('nguoiduadi_notes').value;
...
if($patient_ok){
	...
	if($released) $smarty->assign('x_time',nl2br($x_time));
        ...
        if($info2){
            $info_encounter_1=$info2->FetchRow();
			$info_encounter1=explode('@@',$info_encounter_1['notes']);
        }
        ...
        $smarty->assign('chandoan_notes',nl2br($info_encounter1['0']));
		...
		$smarty->assign('LDChandoan_BP','Bệnh phụ');
        $smarty->assign('chandoan_notes_BP',nl2br($info_encounter1['1']));
        $smarty->assign('chandoan_Input_BP','<input type="text" name="chandoanravien_code_BP" id="referrer_diagnosis_code_BP" size=10 maxlength=10 value="'.$info_encounter1['chandoanravien_code_BP'].'" onblur="fill(this,this.value,\'referrer_diagnosis_BP\');"><input type="text" name="chandoan_notes_BP" id="referrer_diagnosis_BP" size=70 maxlength=250 value="'.nl2br($info_encounter1['chandoan_notes_BP']).'" ondblclick="this.value=\'\'"/>');
