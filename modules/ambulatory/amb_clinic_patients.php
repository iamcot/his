...
/*
* Neu la discharge theo list checkbox
*/
if(isset($DischargSelect)){
	# Get all outpatients for this dept
	$p_obj=&$enc_obj->OutPatientsBasic($dept_nr);
	while ($patient=$p_obj->FetchRow()){
		$e_nr = $patient['encounter_nr'];
		if($_POST['check'.$e_nr] == $e_nr){
			//print_r($patient);
			$discharged_type = 1;
			$pn = $e_nr;
...
function transenclass(pid){
   urlholder="<?php echo $root_path ?>modules/registration_admission/aufnahme_start_1.php<?php echo URL_APPEND; ?>&pid="+pid+"&origin=patreg_reg&encounter_class_nr=1";
	window.location.href=urlholder;
}
...
if($rows){
	...
	while ($patient=$opat_obj->FetchRow()){
			...
			#checkbox name
			$smarty->assign('sCheckName','check'.$patient['encounter_nr']);
			$smarty->assign('sEnNr',$patient['encounter_nr']);
			# Reset elements
			...
			$smarty->assign('sPrintPdf','');
			$smarty->assign('sTarget','');
			$smarty->assign('sTransEnClassIcon','');
			...
			//var_dump($patient['time']);
			if($patient['time']) $smarty->assign('sTime',$patient['time']);
			if($patient['date']) $smarty->assign('sDate',$patient['date']);
			...
			if($edit){
               ...
               // $smarty->assign('sAdmitDataIcon','<a href="'.$root_path.'modules/registration_admission/aufnahme_daten_zeigen.php'.URL_APPEND.'&encounter_nr='.$patient['encounter_nr'].'" title="'.$LDAdmissionData.' : '.$LDClk2Show.'"><img '.createComIcon($root_path,'pdata.gif','0','',TRUE).' alt="'.$LDAdmissionData.' : '.$LDClk2Show.'"></a>');   //==>n
				...
				$smarty->assign('sPrintPdf','<a href="'.$root_path.'modules/registration_admission/aufnahme_pass_pdf.php'.URL_APPEND.'&target=search&fwd_nr='.$patient['encounter_nr'].'" title="'.$LDPrintPdf.' : '.$LDClk2Show.'"><img '.createComIcon($root_path,'icon_acro.gif','0','',TRUE).' alt="'.$LDPrintPdf.' : '.$LDClk2Show.'"></a>');
				//$smarty->assign('sTransEnClassIcon','<a href="javascript:transenclass(\''.$patient['pid'].'\')"><img '.createComIcon($root_path,'bestell.gif','0','',TRUE).' alt="'.$LDReleasePatient.'"></a>');
			}
...
$TP_Legend1_BLOCK.= '&nbsp;<img '.createComIcon($root_path,'pdata.gif','0','absmiddle',TRUE).'> <b>'.$LDAdmissionData.'</b><br>
		...
		&nbsp;<img '.createComIcon($root_path,'dollarsign.gif','0','absmiddle',TRUE).'> <b>'.$LDBillPatient.'</b><br>
		&nbsp;<img '.createComIcon($root_path,'bestell.gif','0','absmiddle',TRUE).'> <b>'.$LDRelease.'</b><br>
		&nbsp;<img '.createComIcon($root_path,'icon_acro.gif','0','absmiddle',TRUE).'> <b>'.$LDPrintPdf.'</b><br>
		';
