...
if(isset($mode)&&($mode=='search'||$mode=='paginate')&&isset($searchkey)&&($searchkey)){
	...
	if($mode!='paginate'){
		...
		if(is_numeric($suchwort)) {
			...
			$sql2=" WHERE ( ps.nr='$suchwort' OR ps.nr $sql_LIKE '%$suchwort%' OR ps.nr $sql_LIKE '$suchwort%' OR ps.nr = '$suchbuffer' )";
		...
		$searchkey = str_replace(" ","%", $searchkey);
			...
			if(sizeof($comp)>1){
				...
				$sql2=" WHERE ( p.name_last $sql_LIKE '".strtr($fn,'+',' ')."%' OR p.namelast $sql_LIKE '%".strtr($fn,'+',' ')."%'
			                		OR p.name_first $sql_LIKE '%".strtr($ln,'+',' ')."' OR p.name_first $sql_LIKE '%".strtr($ln,'+',' ')."%'";
				...
				$sql2=" WHERE (p.name_last $sql_LIKE '$suchwort%' OR p.name_last $sql_LIKE '%$suchwort%'
			                		OR p.name_first $sql_LIKE '%$suchwort' OR p.name_first $sql_LIKE '%$suchwort%' OR CONCAT(p.name_last, ' ',p.name_first) $sql_LIKE '$searchkey' ";
				if($DOB) $sql2.=" OR p.date_birth = '$DOB' OR p.date_birth $sql_LIKE '%$DOB')";
					else $sql2.=')';
			...
			if($oitem=='nr') $sql3.='ORDER BY ps.'.$oitem.' '.$odir;
				else $sql3 ='ORDER BY p.name_last';
