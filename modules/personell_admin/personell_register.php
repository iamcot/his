...
if($pid||$personell_nr){
	...
	# Get the patient global configs
		...
		if (($mode=='save')){
			$error=FALSE;
			# Check some values
			if(empty($_POST['job_function_title'])|| empty($_POST['date_join'])|| empty($_POST['contract_start']))
			{
				$error=TRUE;
			}
			...
			if(!$error) {
				if($update || $personell_nr){
					...
					$logs->writeline_his($_SESSION['sess_login_userid'], $thisfile, $personell_obj->getLastQuery(), date('Y-m-d H:i:s'));
