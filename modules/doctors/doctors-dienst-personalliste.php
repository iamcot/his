...
if($_SESSION['sess_user_origin']=='System_Admin'){
...
function update(b){
	document.getElementById('chucvu_nr').value = b;
}
function adddoc(nr){
    str=document.getElementById("chucvu").value;
	...
	xmlhttp.open("GET","getdoctor.php?chucvu_nr="+str+"&nr="+nr+"&dept_nr="+<?php echo $dept_nr; ?>,true);
	xmlhttp.send();
...
<!--     <td  align=center  class="v13" colspan=2><font><nobr>&nbsp;<?php echo $LDDateOfBirth; ?></nobr></td>
 -->    <td  align=center  class="v13" colspan=2><nobr>&nbsp;<?php echo $LDFunction; ?></nobr></td>
  <td  align=center  class="v13" colspan=2><nobr>&nbsp;<?php echo $LDChucVu; ?></nobr></td>
	...
	?>
  	<tr bgcolor="#ffffff">
    <td  class="v13" colspan=3><nobr>&nbsp;<a href="<?php echo $root_path?>modules/personell_admin/personell_register_show.php<?php echo URL_APPEND ?>&from=such&personell_nr=<?php echo $row['personell_nr']?>&target=personell_search">
	<?php  
		switch($row['sex']){
...
<!--     <td  class="v13" colspan=2><font>&nbsp;<?php echo formatDate2Local($row['date_birth'],$date_format); ?></td>
 -->    <td  class="v13" colspan=2><nobr>&nbsp;<?php echo $pers_obj->getNameJobFunction($row['job_function_title']); ?></nobr></td>
 <?php if ($row['chucvu_nr']==0){$chucvu='Không';}
		elseif($row['chucvu_nr']==1){$chucvu=$LDGiamDoc;}
		elseif($row['chucvu_nr']==2){$chucvu=$LDPhoGiamDoc;}
		elseif($row['chucvu_nr']==3){$chucvu=$LDTruongKhoa;}
		elseif($row['chucvu_nr']==4){$chucvu=$LDPhoKhoa;}
 echo'<td  class="v13" colspan=2><nobr>&nbsp;'.$chucvu.'</nobr></td>';
 ?>
...
?>
	 ...
	 <td>
	  <?php echo $pagen->makeSortLink($LDChucVu,'chucvu_nr',$oitem,$odir,$append);  ?></td>
...
<?php
					...
					while($row=$search_result->FetchRow())
					{
                        ...
                        echo "</td>";	
						echo"<td >";
						echo'<select name="chucvu" id="chucvu" onclick="javascript:adddoc(\''.$row['nr'].'\')">';
	echo'<option value="0" >Không</option>';
	
	$job_type=$pers_obj->getTypeChucVu(1);
	while($type_result=$job_type->FetchRow()){
	 echo'<option value="'.$type_result['nr'].'">'.$$type_result['LD_var'].'</option>';
	...
	echo '	<td id="doctor">&nbsp;
					...
					<a href="doctors-list-add.php'.URL_APPEND.'&nr='.$row['nr'].'&dept_nr='.$dept_nr.'&mode=save&retpath='.$retpath.'&ipath='.$ipath.'&chucvu_nr=0" title="'.$LDAddDoctorToList.'">
