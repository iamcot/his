...
<td rowspan=6 align="center" class="photo_id"><img <?php echo $img_source; ?> hspace=5></td>
</tr>
<!--
<tr bgcolor="white">
<td class="adm_item">&nbsp;
...
<tr bgcolor="white">
<td class="adm_item">&nbsp;<?php echo $LDChucVu ?>:
</td>
<td colspan=2   class="adm_input">
<?php if($job_post==0) echo 'Không';
	   elseif($job_post==1) echo $LDGiamDoc;
		 elseif($job_post==1) echo $LDPhoGiamDoc;
		  elseif($job_post==1) echo $LDTruongKhoa;
		   elseif($job_post==1) echo $LDPhoKhoa;
 ?>
</td>
</tr>
<tr bgcolor="white">
...
<tr bgcolor="white">
<td class="adm_item">&nbsp;<?php echo $LDDepartmentNow ?>:
</td>
<?php 
$sql="Select LD_var from care_department where nr=(select location_nr from care_personell_assignment where personell_nr='".$full_pnr."' and date_start=(select max(date_start)from care_personell_assignment where personell_nr='".$full_pnr."') and status NOT IN ('void','hidden','deleted','inactive') )";
$buf=$db->Execute($sql);
if($buf->RecordCount()){
$buf2=$buf->FetchRow();

echo'<td colspan=2 class="adm_input">&nbsp;'.$$buf2['LD_var'].'</td>';
}else{
echo'<td colspan=2 class="adm_input">&nbsp;</td>';
}
?>

</tr>
<?php 
$sql="select date_end from care_personell_assignment where personell_nr='".$full_pnr."' ";
$buf=$db->Execute($sql);
if(($buf->RecordCount())>1){

while($buf2=$buf->FetchRow()){
if($buf2['date_end'] != '0000-00-00'){
$sql="Select d.LD_var,pa.date_start,pa.date_end 
		from care_department as d,care_personell_assignment as pa
		where d.nr=(select location_nr from care_personell_assignment where personell_nr='".$full_pnr."' and date_end='".$buf2['date_end']."')
...
<td colspan=2 class="adm_input">&nbsp;<?php echo $salary_grading; ?>
</td>
</tr>
<tr bgcolor="white">
<td class="adm_item">&nbsp;<?php echo $LDHeSoChucVu ?>:
</td>
<td colspan=2 class="adm_input">&nbsp;<?php echo $heso_chucvu; ?>
</td>
</tr>
<tr bgcolor="white">
