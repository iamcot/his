...
while(list($jid,$date)=each($tdate)) {
	if(!in_array($jid,$notShow)) continue;
	$fpdf->Cell(0,5,"Data - ".$jid.": " .formatDate2Local($date,$date_format)." ".$ttime[$jid],0,1,'L'); 
}
...
while(list($group,$pm)=each($requestData)) {
	...
	$fpdf->Ln(6);
	$fpdf->MultiCell(0,5,$gName->fields['name'],0,'L');
	...
	$fpdf->MultiCell(0,5,$laboratoryTable,0,'L');
...
$fpdf->Ln(14);
$fpdf->Cell2Col(80,5,$LDDoctor,$LDTechnician);
$fpdf->Cell(0,5,$presPrescriber,0,1,'L');
$fpdf->Ln(10);
$fpdf->Cell2Col(80,5,"______________________ ",$access->login_id);
