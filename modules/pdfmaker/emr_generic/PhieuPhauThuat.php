...
$fpdf->Cell(0,5,'Số: '.$buf2[nr],0,0,'R');
$fpdf->Ln(); 

$fpdf->Cell(150,5,$cell,0,0,'L');
$fpdf->Ln(); 
...
$function=array("Bác sĩ phẫu thuật","Phụ mổ","Bác sĩ gây mê","Bác sĩ gây mê phụ","Y tá vòng trong","Y tá vòng ngoài");
for($i=0;$i<6;$i++){
...
if($personell_info[0]){
    for($i=0;$i<sizeof($personell_info[0]);$i++){
        ...
        else $fpdf->Cell(80,5," Bác sĩ phẩu thuật: ".trim($personell_info[0][$i],'\x'),0,0,'L');
    }    
}else{
...
$fpdf->Cell(45,5,' Người thực hiện:  '.$_SESSION['sess_user_name'],0,0,'C');
$fpdf->Ln();
if($op_info['op_start']){
$fpdf->SetX(55);
...
if($op_info['op_end']){
$fpdf->SetX(56);
$fpdf->Cell(0,5," Giờ kết thúc: ".$op_info['op_end'],0,0,'C');
}else{
$fpdf->SetX(104);
$fpdf->Cell(0,5,' Giờ kết thúc:.........................................................',0,0,'C');
}
if($personell_info[4]){
    for($i=0;$i<sizeof($personell_info[4]);$i++){
        if(trim($personell_info[4][$i],'\x')=='') continue;
            ...
            $fpdf->Ln();
            $fpdf->SetX(20);
            $fpdf->Cell(0,5,"Dụng cụ viên ".$i." :  ".trim($personell_info[4][$i],'\x'),0,0,'L');
        }
    }
}else{
    $fpdf->Ln();
...
if($personell_info[2]){
    for($i=0;$i<sizeof($personell_info[2]);$i++){
        if(trim($personell_info[2][$i],'\x')=='') continue;
            ...
            $fpdf->Ln();
            $fpdf->SetX(18);
            $fpdf->Cell(0,5,"Bác sĩ gây mê ".$i." :   ".trim($personell_info[2][$i],'\x'),0,0,'L');
        }
    }
}else{
    $fpdf->Ln();
