	...
	$idpic=$root_path.'uploads/photos/registration/'.$encounter['photo_filename'];
		...
		while(list($x,$v)=each($types)){
			...
			if(!stristr($filter,$nr)){
				...
				$fpdf->SetFont('','B',11);
				$fpdf->Cell(0,5,$title,0,1,'L',true);
				...
				if($notes=& getNotes($nr)){
					...
					while($report=$result->FetchRow()){
						...
						if(!empty($report['notes'])){
							$fpdf->Cell(40,5,$LDNotes.": ",0,0,'L');
							$fpdf->MultiCell(0,5,$report['notes']."\n",0,'L');
						}
						if(!empty($report['short_notes'])){
							$fpdf->Cell2Col(40,5,$LDShortNotes.":",$report['short_notes']);
						}
						if(!empty($report['aux_notes'])){
							$fpdf->Cell2Col(40,5,$LDShortNotes.":",$report['aux_notes']);
						}
						$fpdf->Ln(5);
					}
				}else{
