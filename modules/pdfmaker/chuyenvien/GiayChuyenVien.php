...
$fpdf->Cell(0,0,str_pad('',26,'__',STR_PAD_RIGHT),0,1,'C');
...
$fpdf->SetFont('','');
$fpdf->Cell(50,5,'Số lưu trữ: '.$encounter['encounter_nr'],0,1,'L');
$fpdf->Cell(135, 0, str_pad('', 20,'__', STR_PAD_RIGHT), 0, 0, 'L');
$fpdf->Ln(3);
$fpdf->Cell(135, 5, 'Số: ...../20...../GCT', 0, 0, 'L');

$fpdf->Ln(); 
$fpdf->SetFont('DejaVu','B',18);
$fpdf->Ln(15);
$fpdf->Cell(0,7,'GIẤY CHUYỂN TUYẾN',0,0,'C');
$fpdf->Ln(15);
...
//Thông tin bệnh nhân
$fpdf->Cell(0,6,'  '.$cell1.' chúng tôi trân trọng giới thiệu:',0,1,'L');
if($encounter['sex']=='m'){
    $sex=$LDMale;
}else{
    $sex=$LDFemale;
...
if(!empty($encounter['ngoaikieu'])){
	$fpdf->Cell(0,5,"  Quốc tịch: ".$encounter['ngoaikieu'].str_pad("", (64-strlen($encounter['ngoaikieu'])), " ", STR_PAD_RIGHT),0,1,'L');
}else{
	$fpdf->Cell(0,5,'  Quốc tịch: '.str_pad("", 70, " ", STR_PAD_RIGHT),0,1,'L');
}
...
$fpdf->Cell(0,5, '- Tuyến trước chuyển đến: (nếu có)'.''.str_pad(" ", 113, " ", STR_PAD_RIGHT),0,1,'L');
$fpdf->Ln();
//Bệnh án
$fpdf->SetFont('','B');
...
if($result=$db->Execute($sql)){
    ...
    while($row=$result->FetchRow()){
        $deptnr_ok=false;
		if($i==1){
			$fpdf->Cell(10,6,'- Kết quả xét nghiệm, cận lâm sàng: ',0,1,'L');
		}
    ...
    if(!$xnstr){
		//$num_thisline = 0;  
		$fpdf->Cell(0,5,"- Kết quả xét nghiệm, cận lâm sàng: ".str_pad(" ", 132, " ", STR_PAD_RIGHT),0,1,'L');
		$fpdf->Cell(0,5,"   ".str_pad(".", 158, ".", STR_PAD_RIGHT),0,1,'L');
	}
}
