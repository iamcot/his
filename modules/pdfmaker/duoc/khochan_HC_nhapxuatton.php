...
$pdf->SetAuthor($cell);
$pdf->SetTitle('Báo cáo nhập xuất tồn HC kho chẵn');
...
$header='<table width="100%">
		...
		<tr><td></td><td align="center">	
				...
				<font size="13"><b>HÓA CHẤT'.$titlereport.'</b></font></td><td></td>
		</tr>
		</table>';
...
$html='	<table cellpadding="2" border="1">
				<tr bgColor="#F2F2F2">
					...
					<th rowspan="2" align="center" width="95">Tên Hóa Chất</th>
...
switch($flag){
	case 'tonkho': 
			$listReport = $Pharma->HC_TonKhoChan($month, $year, $cond_typeput);
			break;
	case 'xuatnhapton':	
			$listReport = $Pharma->Khochan_baocaohoahchat_nhapxuatton($month, $year, $cond_typeput);
			break;	
	default: break;		
}
...
if(is_object($listReport)){
	...
	while($rowReport = $listReport->FetchRow())	{
		$baocaotruoc = $Pharma->Khochan_hoachat_tontruoc($rowReport['product_encoder'], $cond_typeput);
		//if ($i==1)
...
if(is_object($listReport) && $list_tonthangtruoc = $Pharma->HC_TonKhoChan($lastmonth, $lastyear, $cond_typeput.$dongtayy)){
...
$pdf->Output('KhoChan_BaocaoXNThoachat.pdf', 'I');
