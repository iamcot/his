...
switch($type){
	case 'medicine':
		...
		break;
	case 'medipot':
		$report_show = $Pharma->getPayOutMedInfo($report_id);
		$medicine_in_pres = $Pharma->getDetailPayOutMedInfo($report_id);
		break;
	case 'chemical':
		$report_show = $Pharma->getChemicalPayOutInfo($report_id);
		$medicine_in_pres = $Pharma->getDetailChemicalPayOutInfo($report_id);
		break;		
}
...
if($report_show!=false){
	if ($report_show['health_station']==0)
		...
		$sql1= "select * from care_pharma_health_station where health_station='".$report_show['health_station']."' ";
		if($re_sup=$db->Execute($sql1)){
			if($count1=$re_sup->RecordCount()){
				$hst=$re_sup->FetchRow();
				$placeto=$LDTramYTe.' '.$hst['village'];
			}
		}
	}
...
$pdf->SetAuthor($cell);
$pdf->SetTitle('Phiếu Xuất Kho');
$pdf->SetMargins(5, 8, 1);
...
$pdf->Write(0, 'PHIẾU XUẤT KHO', '', 0, 'C', true, 0, false, false, 0);
$pdf->SetFont('dejavusans', '', 10);
...
if (!$report_show || !$medicine_in_pres){
	...
	$pdf->Ln();
	$pdf->Write(0, "Họ và tên người nhận hàng: ".$placeto, '', 0, 'L', 1, 0, false, false, 0);
	$pdf->Write(0, "Lý do xuất kho: Xuất thuốc".$bhyt.'  '.$report_show['note'], '', 0, 'L', 1, 0, false, false, 0);
	$pdf->Write(0, 'Xuất tại: kho Dược '.$cell, '', 0, 'L', 1, 0, false, false, 0);
	$pdf->Ln();
	...
	$html='	<table cellpadding="2" border="1"  >
					<tr align="center" >
						...
						<td rowspan="2" width="30"><b>Mã số</b></td>
						<td rowspan="2" width="45"><b>'.$LDUnit.'</b></td>
						<td colspan="2" width="80"><b>'.$LDNumberOf.'</b></td>
						<td rowspan="2"><b>'.$LDCost.'</b></td>
						<td rowspan="2" width="70"><b>'.$LDTotalCost.'</b></td>
						<td rowspan="2"><b>'.$LDLotID.'</b></td>
						<td rowspan="2" width="70"><b>'.$LDExpDate1.'</b></td>
					</tr>
					<tr align="center">
	...
	for($i=1;$i<=$medicine_count;$i++) { 			
		...
		$html= $html.'<tr>
						...
						<td align="center">'.$rowIssue['lotid'].'</td>
						<td align="center">'.formatDate2Local($rowIssue['exp_date'],'dd/mm/yyyy').'</td>
					</tr>';					
		$total += ($rowIssue['number_voucher']*$rowIssue['price']);
	}
	
	$html = $html.'<tr><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td align="right"> - </td><td></td><td></td></tr>
					<tr><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td align="right"> - </td><td></td><td></td></tr>
					<tr>
						<td></td><td> <b>Tổng cộng</b> </td><td></td><td></td><td></td><td></td><td></td>
						<td align="right"><b>'.number_format($total).'</b></td><td></td><td></td>
					</tr>
				</table>';
