...
while(strlen($itemcd)!=1) {
	...
	$valueupdate = $_POST['update'.$oneitem];
	list($name, $cost, $discount) = explode("#", $valueupdate);
	
	if($name!=$no[$j] || $cost!=$no1[$j] || $discount!=$no2[$j])
	{
		$eComBill->updateServiceItem($no[$j],$no1[$j],$no2[$j],$oneitem);
		
		$history='Update '.$date_pres.' '.$_SESSION['sess_user_name'];
		$eComBill->createBillArchiveItem($oneitem,$no[$j],$no1[$j],$no2[$j],$history);
	}
	$j=$j+1;
}

$billmenu="billingmenu.php";
header('Location:'.$billmenu.URL_REDIRECT_APPEND.'&target='.$target);
exit;
?>
