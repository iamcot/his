...
$Encounter->loadEncounterData($patientno);		//is_transfer
...
if($chkcnt+$countpres>0)    {
    ...
    echo "<tr>";
    echo "<td width=100%><font color=#330066 size=3 face=Arial><strong>" . $LDBilling . "</strong></font></td>";
    echo "</tr>";
    echo "</table>";
    echo "<center><h4>".$LDBillPending." <a href=\"patient_due_first.php".URL_APPEND.'&patientno='.$patientno.'&full_en='.$full_en.'&target='.$target."&billid=currentbill\">".$LDCurrentBill. "</a></center></h4>";
...
if($cntergebnis){
    ...
    $ybf="7".$ybf."000000";
    $final_bill_no=(int)$ybf;       // mã hóa đơn
}
...
if(is_object($resultfinalqry)){
    ...
    $total_discount = $buffer['total_discount'];
}else{
    $totalbill=0;
}
...
if(is_object($bill_query)) {
    while ($bill_result=$bill_query->FetchRow()) {
        ...
        //$alldiscount += $bill_result['bill_discount'];
...
if(is_object($payment_query)) {
    ...
    $smarty->assign('LDTotalPaymentValue', $paymenttotal);
...
# Load info of payment, bill, finalbill --26/9/11---------------------------------------------------------------------
...
$smarty->assign('LDAmountAfterDiscount', $LDAmountPreviouslyReceived);


//include($root_path.'classes/money/baohiem.php');
//$tienbaohiem = TienBaoHiem($Encounter->encounter['insurance_nr'], $Encounter->encounter['insurance_start'], $Encounter->encounter['insurance_exp'], $last_totalbill, date("Y-m-d"), $Encounter->encounter['is_traituyen']);

//$smarty->assign('discount', '<input type="text" name="discount" size="10" style="text-align:right;" value="'.$tienbaohiem.'" >');
...
$smarty->assign('sHiddenInputs','<input type="hidden" name="totalbill" value="'. $totalbill .'">
								...
								<input type="hidden" name="totpayment" value="'. $totpayment .'">
