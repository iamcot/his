<?php
    ...
    $product_encoder=$med->getAllMedInDept(87,63,$odir,$oitem);
    //phân trang		  
    ...
    */
    ...
    # Title in toolbar
    $smarty->assign('sToolbarTitle', "$LDManageMed :: $LDListMed ");
    ...
    # Window bar title
    $smarty->assign('sWindowTitle',"$LDManageMed :: $LDListMed");
...
if ($linecount) echo str_replace("~nr~",$totalcount,$LDSearchFound).' '.$LDShowing.' '.$pagen->BlockStartNr().' '.$LDTo.' '.$pagen->BlockEndNr().'.';
	...
	if ($linecount) { 
	echo '<table border=0 cellpadding=2 cellspacing=1 width=60%> <tr class="wardlisttitlerow">';
...
?>
    <td align="center">
        ...
        <?php 
            $oitem=='name_sub';
            //echo $pagen->SortLink($LDGroupMed,'name_sub',$odir,TRUE);
			echo $LDGroupMed;
        ?>
        </b>
    </td>    
    <td align="center">
        ...
        <?php
            $oitem=='product_name';
            //echo $pagen->SortLink($LDNameMed,'product_name',$odir,TRUE);
			echo $LDNameMed;
        ?>
        </b>
    </td>
...
<?php
	while($zeile=$ergebnis->FetchRow()){
                ...
                $encoder=$med->getMedInfo($zeile['product_encoder'],$dept_nr,$ward_nr);
                $detail=$med->getMedInfoDetail($zeile['product_encoder'],$dept_nr,$ward_nr);
                echo '<td >';
                echo '&nbsp;'.ucfirst($detail['name_sub']).'</td>';
		echo '<td >';
                echo '&nbsp;'.ucfirst($encoder['product_name']).'</td>';
                $number=$med->getnumberMed($zeile['product_encoder'],$dept_nr,$ward_nr);
		echo '<td align="center">';
                echo '&nbsp;'.ucfirst($number['number']).'</td>';
                $unit=$med->getunitMed($zeile['product_encoder']);
		echo '<td align="center">';
...
<form name="managepharma" action="<?php echo $root_path.'modules/nursing/nursing-manage-medicine.php?'?>">
    <input type="submit" value="<?php echo $LDManageActiveMed?>" />
