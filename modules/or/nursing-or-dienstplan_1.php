...
if($_SESSION['sess_login_username']!='admin'){
    ...
    if($role['location_nr']!=87 && strpos($role['dept_nr'], '"87"')==''){
        header("Location:../../language/".$lang."/lang_".$lang."_invalid-access-warning.php"); 
        exit;
    }elseif($role['location_nr']==87){
        $dept_nr=$role['location_nr'];
    }else{
        $dept_nr=87;
    }
}else{
...
$dutyplan=&$pers_obj->getNOCDutyplan($dept_nr,ROLE_NR_NURSER,$pyear,$pmonth);
...
switch($retpath)
{
    case "menu": $rettarget=$root_path.'main/op-doku_1.php'.URL_APPEND; break;
    case "qview": $rettarget=$root_path.'main/op-doku_1.php'.URL_APPEND; break;
...
function popinfo(l)
{
    ...
    ww=400;
    wh=400;
    urlholder="nursing-or-dienstplan-popinfo.php<?php echo URL_REDIRECT_APPEND ?>&nr="+l+"&dept_nr=<?php echo $dept_nr ?>&ward_nr=<?php echo $ward_nr ?>&route=validroute&user=<?php echo $aufnahme_user.'"' ?>;
...
# Previous month
$sBuffer = '<a href="'.$thisfile.URL_APPEND.'&retpath='.$retpath.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr.'&pmonth=';
