...
$breakfile="doctors-dienstplan.php".URL_APPEND."&dept_nr=$dept_nr&ward_nr=$ward_nr&pmonth=$pmonth&pyear=$pyear&retpath=$retpath&pmonth=$pmonth&pyear=$pyear";
...
if($dblink_ok)
	{	
		if($mode=='save')
		{
		 ...
		 {
             $role_nr = 15;//bac si
		 	if($dutyplan=&$pers_obj->getDOCDutyplan(87,$role_nr,$pyear,$pmonth)){
...
# Prepare page title
 ...
 if(isset($$LDvar) && $$LDvar) $sTitle = $sTitle.$$LDvar." :: $LDMakeDutyPlan :: $LDDoctors";
   else $sTitle = $sTitle.$dept_obj->FormalName()." :: $LDMakeDutyPlan :: $LDDoctors";
...
function popselect(elem,mode,nr_t,nr_d,nr_s,nr_s_t,nr_s_d)
{
    ...
    urlholder="doctors-dienstplan-poppersonselect.php?elemid="+elem + "&dept_nr=87&ward_nr=63&month="+tmonth+"&year="+tyear+ "&mode=" + mode+ "&nr_t="+ nr_t + "&nr_d="+ nr_d + "&nr_s="+ nr_s + "&nr_s_t="+ nr_s_t + "&nr_s_d="+ nr_s_d + "&retpath=qview<?php echo "&lang=$lang"; ?>";
...
function cal_update()
{
	var filename="doctors-dienstplan-planen.php?<?php echo "sid=$sid&lang=$lang" ?>&retpath=<?php echo $retpath ?>&dept_nr=87&ward_nr=63&pmonth="+document.dienstplan.month.value+"&pyear="+document.dienstplan.jahr.value;
...
<?php 
 ...
 $smarty->append('JavaScript',$sTemp);
  ...
  $smarty->assign('LDStandbyPerson',$LDStayIn);
