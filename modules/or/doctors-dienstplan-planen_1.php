...
if($_SESSION['sess_user_name']!='admin'){
    $role= $access->checkNameRole($user_name);
    $rolename= $role->Fetchrow();
    if($rolename['location_nr']!=39){
...
$breakfile="doctors-dienstplan_1.php".URL_APPEND."&dept_nr=$dept_nr&pmonth=$pmonth&pyear=$pyear&retpath=$retpath";
...
if($dblink_ok)
	{	
		if($mode=='save')
		{
					...
					if($dpoc_nr=$pers_obj->DOCDutyplanExists($dept_nr,ROLE_NR_DOCTOR,$pyear,$pmonth)){
...
function popselect(elem,mode,nr_t,nr_d,nr_s,nr_s_t,nr_s_d)
{
	...
	urlholder="doctors-dienstplan-poppersonselect.php?elemid="+elem + "&dept_nr=<?php echo $dept_nr ?>&month="+tmonth+"&year="+tyear+ "&mode=" + mode+ "&nr_t="+ nr_t + "&nr_d="+ nr_d + "&nr_s="+ nr_s + "&nr_s_t="+ nr_s_t + "&nr_s_d="+ nr_s_d + "&retpath=qview<?php echo "&lang=$lang"; ?>";
...
for ($i=1,$n=0,$wd=$firstday;$i<=$maxdays;$i++,$n++,$wd++)
{
	...
	$smarty->assign('sInput2','<input type="hidden" name="hr'.$n.'" value="'.$r_pnr['hr'.$n].'">
                <input type="text" name="r'.$n.'" size="30" onFocus=this.select() value="'.$relems['r'.$n].'">');
