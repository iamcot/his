...
$breakfile=$root_path.'modules/or_admin/or_list_config.php'.URL_APPEND	;
...
if(!empty($mode)&&!$inputerror){
	...
	{	
		case 'create': 
		{
			if(($_POST['room_nr']=="") || ($_POST['dept_nr']=="") || ($_POST['date_create']=="") || !is_numeric($_POST['room_nr'])){
				echo $LDAlertInvalidOrRoomCreating;
				exit;
			} else 
		...
		case 'update':
		{ 
			if(($_POST['room_nr']!=$room_nr) && ($_POST['dept_nr']!=$dept_nr) && ($OR_obj->ORNrExists($_POST['room_nr']))){
				$error_msg=$LDORNrExists;
				$inputerror=TRUE;
				exit;
			}
...
$witem=array('nr', 'ward_id', 'name');
# Load the active wards available
$wardsarray=$ward_obj->getAllWardsItemsObjectofDept($witem, $dept_nr); 
...
<!-- 
function checkRoomNr(room_nr){
	if(isNaN(room_nr.value)){
	...
	$.ajax({
		...
		success: function(result)
		{
			if(result == 'dberror'){
				...
				document.getElementById("sltwardid").innerHTML = result;
			}
		}
	});
}
...
<table border=0>
  ...
  <tr>
	...
	?>
	<input type="text" name="room_nr" size=4 maxlength=4 value=<?php echo  $newORnr; ?> onblur="checkRoomNr(this)"> <a href="javascript:newORnr()"><img <?php echo createComIcon($root_path,'l_arrowgrnsm.gif','0') ?>> <?php echo $LDClkNextNr ?></a>
...
<tr>
    <td align=right bgColor="#eeeeee"><font color=#ff0000><b>*</b><?php echo $LDOwnerDept; ?>: </td>
    <td bgColor="#f9f9f9"><select name="dept_nr" onchange="changeDept(this.value, <?php echo ($ward_nr?$ward_nr:-1); ?>)">
	<option value=""> </option>';
	<?php
...
<tr>
    ...
    <td bgColor="#f9f9f9">
		<select name="ward_nr" id="sltwardid">
		<option value=""> </option>
	<?php
		while($row=$wardsarray->FetchRow()){
			echo "<option value='".$row[nr]."' ".($row[nr]==$ward_nr?"selected":"")."> [".$row[ward_id]."] - ".$row[name]." </option>";
		}
	?>
        </select>
</td>
