...
$patmenu="../show_chemical.php".URL_REDIRECT_APPEND."&full_en=".$_SESSION['sess_full_en']."&encounter_nr=".$_POST['encounter_nr']."&lang=".$_POST['lang']."&target=$target&pid=".$_SESSION['sess_pid'];
...
switch($mode){
		...
		case 'create':
				//prescription_info 
				$pres_id = $pres_obj->getLastIDChemicalPres();
				if ($pres_id==false)
					$id=1;
				else
				...
				$pres_obj->usePrescriptionChemical('prescription_info');
				$pres_obj->insertDataFromArray($pharma_prescription_info);
				
				//prescription sub
				$pres_obj->usePrescriptionChemical('prescription');
				$n = $_POST['theValue'];
				for ($j=1; $j<=$n; $j++)
				{
		...
		//echo $pres_obj->getLastQuery();
					$pres_obj->usePrescriptionChemical('prescription');
					$pres_obj->deleteAllChemicalInPres($id);
					$n = $_POST['theValue'];
					for ($j=1; $j<=$n; $j++)
					{
						if($_POST['medicinea'.$j]!=""){
		...
		case 'delete':
				$id = $_POST['idpres'];
				$pres_obj->deleteAllChemicalInPres($id);
				$pres_obj->deleteChemicalPres($id);				
				if(!$no_redirect){
					header("Location:".$patmenu);
					exit;
				}
