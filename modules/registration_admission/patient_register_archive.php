	...
	if($mode=='paginate'){
		...
		$s2='';
							...
							if(isset($pid) && $pid)
							{
								 ...
								 {
								 		$s2.=" pid $sql_LIKE '%$pid' OR pid $sql_LIKE '%$pid%'";
								 }
								 else
								 {
								       $s2.=" pid = ".$pid;
							...
							{
							     if($s2) $s2.=" AND (name_last $sql_LIKE '$name_last%' OR name_last $sql_LIKE '%$name_last%')"; else $s2.=" name_last $sql_LIKE '$name_last%' OR name_last $sql_LIKE '%$name_last%'";
							}
							...
							if(isset($name_first) && $name_first)
								if($s2) $s2.=" AND (name_first $sql_LIKE '%$name_first%' OR name_first $sql_LIKE '%$name_first')"; else $s2.=" name_first $sql_LIKE '$name_first%' OR name_first $sql_LIKE '%$name_first'";
							...
							//	$sql="select nr from care_address_citytown where name='".."'";
								if($s2) $s2.=" AND (SELECT nr)addr_citytown_nr $sql_LIKE (select nr from care_address_citytown where name='$addr_citytown_name')"; else $s2.=" addr_citytown_nr $sql_LIKE (select nr from care_address_citytown where name='$addr_citytown_name')";
								}
							...
							if(isset($civil_status) && $civil_status)
								...
								if($s2) $s2.=" AND citizenship $sql_LIKE '$citizenship'"; else $s2.=" citizenship $sql_LIKE '$citizenship'";
