...
function printout(pn){
	urlholder="<?php echo $root_path ?>modules/pdfmaker/thuongtich/GiayChungNhanThuongTich.php<?php echo URL_APPEND; ?>&enc="+pn;
...
if(file_exists($root_path.'cache/barcodes/en_'.$encounter_nr.'.png')) {
	...
	$smarty->assign('referrer_diagnosis','<textarea name="referrer_diagnosis" id="referrer_diagnosis" cols="40" rows="5">'.$stemp.'</textarea>'.$sBuf);
	//end
	...
	$smarty->assign('LDThuongtichvao',$LDThuongtichvao);
	$smarty->assign('thuongtichvao','<input name="thuongtichvao" type="text" style="width:96%;" value="'.$thuongtichvao.'">');
	$smarty->assign('LDThuongtichra',$LDTtHienTai);//$LDThuongtichra);
	$smarty->assign('thuongtichra','<input name="thuongtichra" type="text" style="width:96%;" value="'.$thuongtichra.'">');
...
*/
$smarty->assign('sMainBlockIncludeFile','registration_admission/admit_input_cntt.tpl');
