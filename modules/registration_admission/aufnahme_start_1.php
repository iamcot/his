		...
		if (($mode=='save') || ($forcesave!='')) {
			...
			if(!$error) {
				...
				if($update || $encounter_nr) {
					...
					$_POST['is_transfer']=$encounter_obj->isAdmitted($pid);
					//echo $encounter_obj->isPIDCurrentlyAdmitted($pid);
					$_POST['insurance_start']=@formatDate2STD($_POST['insurance_start'],$date_format);
					$_POST['insurance_exp']=@formatDate2STD($_POST['insurance_exp'],$date_format);	
					...
					if($encounter_obj->insertDataFromInternalArray()) {
						...
						$encounter_obj->_discharge($encounter_obj->isAdmitted($pid),"'1','2','4','5'",9,date("Y-m-d"),date("H:i:s"));
//echo $encounter_obj->isPIDCurrentlyAdmitted($pid);
			$sql="UPDATE care_encounter SET					
					transfer_date='".date("Y-m-d")."',
					transfer_time='".date("H:i:s")."',
					flag=1
				WHERE pid='".$pid."' AND encounter_nr='".$encounter_obj->isAdmitted($pid)."' AND is_discharged=0
			";
			//echo $sql;
			$db->Execute($sql);
		...
		//echo $encounter_obj->isAdmitted($pid);
...
function chkform(d) {
	...
	if(d.encounter_class_nr[0]&&d.encounter_class_nr[1]&&!d.encounter_class_nr[0].checked&&!d.encounter_class_nr[1].checked){
		...
		d.doctor_nr.focus();
		return false;
	}else if(d.lidovaovien.value==""){
		alert("<?php echo $LDPlsLidovaovien; ?>");
		d.lidovaovien.focus();
		return false;
...
function getref(pn){
	urlholder="<?php echo $root_path ?>modules/drg/drg-icd10-search_for_diag.php?sid=<?php echo "$sid&lang=$lang" ?>&pn="+pn;
...
$(function(){
 ...
 $("#madk_kcbbd").mask("**-***");
	...
	var str =document.getElementById("referrer_diagnosis").value;
	if (window.XMLHttpRequest)
	  {// code for IE7+, Firefox, Chrome, Opera, Safari
	  xmlhttp=new XMLHttpRequest();
	  }
	...
	xmlhttp.onreadystatechange=function()
	  ...
	  if (xmlhttp.readyState==4 && xmlhttp.status==200)
		{
		document.getElementById("referrer_diagnosis").innerHTML=xmlhttp.responseText;
		}
	  }
	xmlhttp.open("GET","rpc.php?diagnosis_code="+thisValue+"&data="+str,true);
	xmlhttp.send();
	}
//end
...
$smarty->assign('LDPlsEnterRefererDiagnosis',$LDPlsEnterRefererDiagnosis);
	...
	//	$sTemp = '<select name="admit_type" style="width:96%;">';
	//	while( $typeResults = $enc_type->FetchRow()) {
	//	if($typeResults['type_nr'] == $admit_type ) $selected="selected";else $selected=" ";
	//		$sTemp .= '<option value="'.$typeResults['type_nr'].'" '.$selected.'>'.$$typeResults['LD_var'].'</option>';
	//	}
	//	$sTemp .= "</select>";
	//} else {
	//	$enc_type = $encounter_obj->getEncounterType();
	...
	if(!$triage) {
		...
		if($triage == 'white') 
		{ 
		$smarty->assign('sAdmitTriage','<label class="triageWhite"><input type = "radio" name ="triage" value="white" checked>'.$sAdmitTriageWhite.'</label>
		...
		$smarty->assign('sAdmitTriage','<label class="triageWhite"><input type = "radio" name ="triage" value="white" >'.$sAdmitTriageWhite.'</label>
										<label class="triageGreen"><input type = "radio" name ="triage" value="green" checked>'.$sAdmitTriageGreen.'</label>			
										<label class="triageYellow"><input type = "radio" name ="triage" value="yellow">'.$sAdmitTriageYellow.'</label>
										<label class="triageRed"><input type = "radio" name ="triage" value="red">'.$sAdmitTriageRed.'</label>
									'); 
		...
		$smarty->assign('sAdmitTriage','<label class="triageWhite"><input type = "radio" name ="triage" value="white">'.$sAdmitTriageWhite.'</label>
										<label class="triageGreen"><input type = "radio" name ="triage" value="green">'.$sAdmitTriageGreen.'</label>			
										<label class="triageYellow"><input type = "radio" name ="triage" value="yellow" checked>'.$sAdmitTriageYellow.'</label>
										<label class="triageRed"><input type = "radio" name ="triage" value="red">'.$sAdmitTriageRed.'</label>
									'); 
		}elseif ($triage == 'red') { 
		$smarty->assign('sAdmitTriage','<label class="triageWhite"><input type = "radio" name ="triage" value="white">'.$sAdmitTriageWhite.'</label>
										<label class="triageGreen"><input type = "radio" name ="triage" value="green">'.$sAdmitTriageGreen.'</label>			
										<label class="triageYellow"><input type = "radio" name ="triage" value="yellow">'.$sAdmitTriageYellow.'</label>
										<label class="triageRed"><input type = "radio" name ="triage" value="red" checked>'.$sAdmitTriageRed.'</label>
									'); }
	...
	//edit 21-11  vy 
		...
		$sTemp = '';		
		$sTemp = $sTemp.'<select id="current_dept_nr" style="width:96%;"name="current_dept_nr" onchange="showWard()">';			
	...
	if($errordiagnose) $smarty->assign('LDDiagnosis',"<font color=red>$LDDiagnosis</font>");
	else $smarty->assign('LDDiagnosis',$LDDiagnosis);
	$sBuf='<div style="float:left;width:14%;">
			<input type="text" size="8" id="inputString" value="" onblur="fill(this.value);" >			
			<input type="button" value="ICD10" onclick="getref('.$encouter_nr.')" >		
			</div>';
	$stemp='';
	if(isset($referrer_diagnosis)) $stemp.=$referrer_diagnosis."\n";
	$smarty->assign('referrer_diagnosis','<textarea name="referrer_diagnosis" id="referrer_diagnosis" style="width:85%;" rows="3">'.$stemp.'</textarea>'." ".$sBuf);
	//end
	...
	$smarty->assign('referrer_name','<input name="referrer_name" type="text" style="width:90%;" value="'.$referrer_name.'">');
	$smarty->assign('LDTherapy',$LDTherapy);
	$smarty->assign('referrer_recom_therapy','<input name="referrer_recom_therapy" type="text" style="width:96%;" value="'.$referrer_recom_therapy.'">');
	$smarty->assign('LDSpecials',$LDSpecials);
	$smarty->assign('referrer_notes','<input name="referrer_notes" type="text" style="width:96%;" value="'.$referrer_notes.'">');
	
	//add 9-11 start
	if($errorvaovien)$smarty->assign('LDLidovaovien',"<font color=red>$LDLidovaovien</font>");
	else $smarty->assign('LDLidovaovien',$LDLidovaovien);
	$smarty->assign('lidovaovien','<input name="lidovaovien" type="text" style="width:96%;" value="'.$lidovaovien.'">');
	if($errorqtbly)$smarty->assign('LDQuatrinhbenhly',"<font color=red>$LDQuatrinhbenhly</font>");
	else $smarty->assign('LDQuatrinhbenhly',$LDQuatrinhbenhly);
	...
	$sTemp = '<select id="insurance_class_nr" name="insurance_class_nr" style="width:96%;">';
	...
	$smarty->assign('tienluong','<textarea style="width:98%;" name="tienluong" rows="1">'.$tienluong.'</textarea>');
	$smarty->assign('LDTTBA',$LDTTBA);
	$smarty->assign('ttba','<textarea style="width:98%;" name="tomtat_benhan" rows="1">'.$tomtat_benhan.'</textarea>');
	...
	$smarty->assign('insurance_nr','<input name="insurance_nr" type="text" style="width:96%;" value="'.$insurance_nr.'" id="inputisurance"> ');
	//add 3-1-2012
	$smarty->assign('LDMaDKKCB',$LDMaDKKCB);
	$smarty->assign('madk_kcbbd','<input name="madk_kcbbd" id="madk_kcbbd" type="text" maxlenght="5" style="width:96%" value="'.$madk_kcbbd.'" onblur="checkKCB(this.value)">');
	$smarty->assign('LDTinhtrang',$LDTinhtrang);
	if($is_traituyen==0){
	$stemp='<select id="traituyen" name="is_traituyen" style="width:96%">
	...
	$smarty->assign('sTinhtrangInput',$stemp);
	$smarty->assign('sCanBoTrungCao','<input type="checkbox" name="is_cbtc"> CBTC');
	$smarty->assign('sTNGT','<input type="checkbox" name="is_tngt">  TNGT');
	//add 9-11
	$smarty->assign('LDInsuranceStart',$LDInsuranceStart);
	$smarty->assign('sInsStartDayInput',$calendar->show_calendar($calendar,$date_format,'insurance_start',$insurance_start));
	//if(!empty($insurance_start)) $sTemp = @formatDate2STD($insurance_start,$date_format);
	//$smarty->assign('sInsStartDayInput','<input type="text" name="insurance_start"  value="'.$sTemp.'" style="width:96%;"  onBlur="IsValidDate(this,\''.$date_format.'\')" onKeyUp="setDate(this,\''.$date_format.'\',\''.$lang.'\')">');
	//if(!empty($insurance_exp)) $sTemp = @formatDate2STD($insurance_exp,$date_format);
	//$smarty->assign('sInsExpDayInput','<input type="text" name="insurance_exp"  value="'.$sTemp.'" style="width:96%;"  onBlur="IsValidDate(this,\''.$date_format.'\')" onKeyUp="setDate(this,\''.$date_format.'\',\''.$lang.'\')">');
	$smarty->assign('LDInsuranceExp',$LDInsuranceExp);
	
	$smarty->assign('sInsExpDayInput',$calendar->show_calendar($calendar,$date_format,'insurance_exp',$insurance_exp));
	$smarty->assign('LDNoicap',$LDNoicap);
	$smarty->assign('insurance_loca','<input name="insurance_loca" type="text" style="width:96%;" value="'.$insurance_loca.'"> ');
	$sTemp = '';
	...
	$smarty->assign('insurance_firm_name','<input name="insurance_firm_name" type="text" style="width:90%;" value="'.$sTemp.'">'.$sBuffer);
