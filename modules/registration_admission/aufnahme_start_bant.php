...
$DRG_obj=new DRG($encounter_nr); // Create a drg object
...
if($pid!='' || $encounter_nr!=''){
	...
	if ($pid)
	{
		...
		if(!$update&&$encounter_nr=$encounter_obj->isAdmitted($pid)){
...
function printout(pn){
	urlholder="<?php echo $root_path ?>modules/pdfmaker/benhangoaitru/BenhAnNgoaiTru.php<?php echo URL_APPEND; ?>&enc="+pn;
	popwin=window.open(urlholder,pn,"menubar=no,width=1024,height=768,resizable=yes,scrollbars=yes");
...
if(file_exists($root_path.'cache/barcodes/en_'.$encounter_nr.'.png')) {
	...
	$sBuf='<input type="button"  value="ICD10" onclick="getref('.$encouter_nr.')">';
	$stemp='';
	if(isset($referrer_diagnosis)) $stemp.=$referrer_diagnosis."\n";
	$smarty->assign('referrer_diagnosis','<textarea name="referrer_diagnosis" id="referrer_diagnosis" style="width:85%;" rows="5">'.$stemp.'</textarea>'.$sBuf);
	...
	$smarty->assign('quatrinhbenhly','<input name="quatrinhbenhly" type="text" style="width:96%;" value="'.$quatrinhbenhly.'">');
	$smarty->assign('LDKhambenhtt',$LDKhambenhtt);
	$smarty->assign('khambenhtoanthan','<input name="khambenhtoanthan" type="text" style="width:96%;" value="'.$khambenhtoanthan.'">');
	$smarty->assign('LDKhambenhbp',$LDKhambenhbp);
	$smarty->assign('khambenhbophan','<input name="khambenhbophan" type="text" style="width:96%;" value="'.$khambenhbophan.'">');
	$smarty->assign('LDKetqualamsang',$LDKetqualamsang);
	$smarty->assign('ketquacanlamsang','<input name="ketquacanlamsang" type="text" style="width:96%;" value="'.$ketquacanlamsang.'">');
	$smarty->assign('LDChandoanbenhchinh',$LDChandoanbenhchinh);
	$smarty->assign('benhchinh','<input name="benhchinh" type="text" style="width:96%;" value="'.$benhchinh.'">');
	$smarty->assign('LDChandoanbenhphu',$LDChandoanbenhphu);
	$smarty->assign('benhphu','<input name="benhphu" type="text" style="width:96%;" value="'.$benhphu.'">');
	$smarty->assign('LDTinhtrangravien',$LDTinhtrangravien);
	$smarty->assign('tinhtrangravien','<input name="tinhtrangravien" type="text" style="width:96%;" value="'.$tinhtrangravien.'">');
	$smarty->assign('LDHuongdieutri',$LDHuongdieutri);
	$smarty->assign('huongdieutritiep','<input name="huongdieutritiep" type="text" style="width:96%;" value="'.$huongdieutritiep.'">');
	...
	$smarty->assign('insurance_firm_name','<input name="insurance_firm_name" type="text" style="width:96%;" value="'.$sTemp.'">'.$sBuffer);
*/
	...
	$smarty->assign('pbSave','<input  type="image" '.createLDImgSrc($root_path,'savedisc.gif','0').' onclick="printout('.$encounter_nr.')" title="'.$LDSaveData.'" align="absmiddle">');
/*
...
*/
$smarty->assign('sMainBlockIncludeFile','registration_admission/admit_input_bant.tpl');
