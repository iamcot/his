					...
					if($encounter_obj->insertDataFromInternalArray()) {
					 ...
					 $logs->writeline_his($_SESSION['sess_login_userid'], $thisfile, $encounter_obj->getLastQuery(), date('Y-m-d H:i:s'));
						...
						$sql="INSERT care_encounter_measurement (msr_date, msr_time,encounter_nr, msr_type_nr, 	value,unit_nr,unit_type_nr,
								...
								WHERE pm.pid = '".$_SESSION['sess_pid']."' AND pm.STATUS=0  ORDER BY msr_date DESC, msr_time DESC";
						$db->execute($sql);
...
if(!isset($pid) || !$pid){
	...
	if(isset($encounter_nr) && $encounter_nr) 	$smarty->assign('sAdmitDate',@formatDate2Local($encounter_date,$date_format));
	 else $smarty->assign('sAdmitDate',$calendar->show_calendar($calendar,$date_format,'dat_reg',date('d/m/Y')));

	$smarty->assign('LDAdmitTime',$LDAdmitTime);
	$smarty->assign('LDInDate','Ngày nhập liệu');

	if(isset($encounter_nr) && $encounter_nr)  $smarty->assign('sAdmitTime',@convertTimeToLocal(formatDate2Local($encounter_date,$date_format,0,1)));
	else $smarty->assign('sAdmitTime','<input name="time_reg" id="time_reg" type="text" value="'.date('H:i').'" size="5">');
...
$smarty->assign('LDPlsEnterRefererDiagnosis',$LDPlsEnterRefererDiagnosis);
	...
	$smarty->assign('benhphu',' '.$sBuf.'<input type="text" name="benhphu" id="benhphu" style="width:85%;" value="'.$benhphu.'"  ondblclick="this.value=\'\'" autocomplete="off" >');
 ...
 //add 0310 - cot
	...
	$smarty->assign('sCanBoTrungCao','<input type="checkbox" onchange="check()" name="is_cbtc"> CBTC');
