<?php 
    ...
    if($text=='chutoa_notes'){
        $check_name=explode(";",$chutoa_notes);
        $flag=1;
    }else if($text=='thuki_notes'){
        $check_name=explode(";",$thuki_notes);
        $flag=2;
    }else{
        $check_name=explode(";",$thanhvien_notes);
        $flag=3;
    }
    html_rtl($lang); 
?>
...
<body onLoad="if (window.focus) window.focus()">
    <font face=arial>
        <form name="classif" id="classif" onSubmit="return process(this,'<?php echo $flag;?>')">
                                    ...
                                    while($row=$dept_nr->FetchRow()){
                                        ...
                                        if($list_personell=$personell->getAllOfDept($row['nr'])){
                                            ...
                                            while($info_a_personell=$list_personell->FetchRow()){
                                                ...
                                                echo ' name="c" id="c'.($rows1+$i).'" value="'.$full_pnr.';"';
                                                
                                                if(gettype($temp_check)=='integer' && $temp_check>=0 && $temp_check<sizeof($check_name))
                                                    echo ' checked>';
                                                else
                                                    echo ' >';
...
<script>
    ...
    function process(d,flag) {
	if(<?php echo $rows1 ?>==0) 
            return false;         
	clo=false;
	if(flag==3){
            wd=window.opener.document.report.thanhvien_notes;     
		
        }else if(flag==2){
            wd=window.opener.document.report.thuki_notes;
        }else{
            wd=window.opener.document.report.chutoa_notes;
        } 
