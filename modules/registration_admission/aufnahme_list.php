...
function Cond($item,$k){
	...
	if(empty($_POST[$item])) return false;
		...
		if($item == 'current_ward_nr') {
			...
			$buf=" ($tab.$item $sql_LIKE '%$_POST[$item]%' OR $tab.$item $sql_LIKE '%$_POST[$item]') ";
			if(!empty($where)) $where.=' AND '.$buf;
			else $where=$buf;
		}
	}
...
if (isset($mode) && ($mode=='search'||$mode=='paginate')){
	...
	if($mode=='paginate'){
		...
		$select="SELECT p.name_last,p.name_first,p.date_birth,p.addr_zip, p.sex,e.encounter_nr,e.encounter_class_nr,e.is_discharged,e.encounter_date,e.insurance_class_nr,e.insurance_nr,e.discharge_date FROM ";
		...
		if($insurance_firm_name){
			$from="  care_person AS p, care_encounter AS e";
				$where="e.insurance_firm_id=(select firm_id from care_insurance_firm where name ='$insurance_firm_name')";
		}
		*/
