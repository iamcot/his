<?php
    ...
    */
	...
	# Prepare additional info saving
        ...
        if(!empty($_POST['date_oivo'])) 
            $_POST['date_oivo']=@formatDate2STD($_POST['date_oivo'],$date_format);
        if(!empty($_POST['time_oivo'])) 
            $_POST['time_oivo']=@convertTimeToStandard($_POST['time_oivo']);
...
//                }
    ...
    if($current_encounter){
        $pregs=&$obj->getEncounterKhambenh_trong($current_encounter);
    }
    if($pregs){
    ...
    $page_title=$LDIn;
    $subtitle=$LDIn;
    $_SESSION['sess_file_return']="show_admit_general.php".URL_APPEND."&pid=".$_SESSION['sess_pid']."&encounter_nr=".$_SESSION['sess_en']."&target=search&flag=KS_s";
