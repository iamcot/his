...
$smarty->assign('sSelectKham','Khám ngoại');
}
//start gjergji
...
$smarty->assign('sAdmitClassInput',"<font color=$fcolor>$eclass</font>");
	...
	$smarty->assign('sWardInput','<a href="'.$root_path.'modules/nursing/'.strtr('nursing-station-pass.php'.URL_APPEND.'&rt=pflege&edit=1&station='.$current_ward_name.'&mode=ward&location_id='.$current_ward_name.'&dept_nr='.$current_dept_nr.'&ward_nr='.$current_ward_nr,' ',' ').'">'.$current_ward_name.'</a>');
...
$smarty->assign('tienluong',$tienluong);
...
$smarty->assign('madk_kcbbd',$madk_kcbbd);
$smarty->assign('LDTinhtrang',$LDTinhtrang);
if($is_traituyen=='0') $smarty->assign('sTinhtrangInput','Không rõ');
elseif($is_traituyen=='1') $smarty->assign('sTinhtrangInput','Đúng tuyến');
elseif($is_traituyen=='2') $smarty->assign('sTinhtrangInput','Trái tuyến');
...
$smarty->assign('sInsStartDayInput','');
}else{
...
ob_start();
	...
	require('./gui_bridge/default/gui_patient_encounter_showdata_options_pdf.php');
	$sTemp = ob_get_contents();
