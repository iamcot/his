...
if($mode=='show'){
	...
	$smarty->assign('sDocsBlockIncludeFile','registration_admission/form-yhct.tpl');
...
if(($mode=='show'||$mode=='details')&&!$enc_obj->Is_Discharged()){
	...
	if($mode=='details'){
	$smarty->assign('sNewRecLink','<a href="'.$root_path.'modules/registration_admission/edit_yhct_ngoaitru.php'.URL_APPEND.'&pid='.$_SESSION['sess_pid'].'&encounter_nr='.$_SESSION['sess_en'].'&target='.$target.'&nr='.$nr.'">Cập nhật</a>&nbsp;<a href="'.$thisfile.URL_APPEND.'&pid='.$_SESSION['sess_pid'].'&encounter_nr='.$_SESSION['sess_en'].'&target='.$target.'&mode=new">'.$LDEnterNewRecord.'</a>');
	}else{
