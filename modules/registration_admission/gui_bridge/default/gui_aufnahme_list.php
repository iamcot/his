...
<table border=0 cellpadding=0 cellspacing=0>
	<tr class="wardlisttitlerow">
	  ...
	  <td><b>
	  <?php 
	  	if($oitem=='insurance_nr') $flag=TRUE;
			else $flag=FALSE; 
		echo $pagen->SortLink($LDInsuranceNr,'insurance_nr',$odir,$flag); 
			 ?></b></td>
	</tr>
...
<?php
	...
	while($result=$ergebnis->FetchRow()){
		...
		$sql="SELECT name FROM care_class_insurance WHERE class_nr='".$result['insurance_class_nr']."'";
		
		$temp=$db->Execute($sql);
		$temp->RecordCount();
		$temp2=$temp->FetchRow();

		echo'<td align=right>&nbsp; &nbsp;<a href="'.$buf.'" title="'.$LDClk2Show.'">'.@formatDate2Local($result['encounter_date'],$date_format).'</a>('.$temp2['name'].')</td>
		<td align=right>&nbsp; &nbsp;'.$result['insurance_nr'].'</td></tr>';
		} else{
		echo'<td align=right>&nbsp; &nbsp; Đã xuất viện '.formatDate2Local($result['discharge_date'],$date_format).' </td>
		</tr>';
		}
		...
		echo '
		<tr>
			<td colspan=7><font face=arial size=2>'.$pagen->makePrevLink($LDPrevious).'</td>
...
<p>
<a href="
<?php if($_COOKIE['ck_login_logged'.$sid]) echo $breakfilen;
