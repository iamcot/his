<script language="javascript">
	...
	urlholder="<?php echo $root_path ?>modules/pdfmaker/giaytokhac/Giaycamdoan.php<?php echo URL_APPEND ?>&enc_nr=<?php echo $_SESSION['sess_en'] ?>&date_s="+date+"&time_s="+time;
...
<table border=0 cellpadding=4 cellspacing=1 width=100% class="frame">
    <tr bgcolor="#f6f6f6">
        ...
        <td <?php echo $tbg; ?> align="center"><FONT color="darkred"><b><?php echo $LDQuanhe; ?></b></FONT></td>
    ...
    if($rows){
        ...
        while($pregrancy=$pregs->FetchRow()){
            if($i%4==0){
                ...
                $time=$pregrancy['time'];
                $nr_dad=$pregrancy['nr'];
                $pregs1=&$obj->_getNotesKhac("notes.encounter_nr='$pn' AND notes.type_nr=types.nr AND types.sort_nr=29 AND notes.date='$date' AND notes.time='$time'", "ORDER BY notes.type_nr DESC");
                $date_array=array();
                if($pregs1){
