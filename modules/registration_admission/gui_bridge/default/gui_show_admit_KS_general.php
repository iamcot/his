...
# If person is dead show a black cross and assign death date
    ...
    if($flag=='KS_s'){
        $t='<img '.createComIcon($root_path,'bul_arrowgrnlrg.gif','0','absmiddle').'>';
        $smarty->assign('sListLinkIcon','<img '.createComIcon($root_path,'bul_arrowgrnlrg.gif','0','absmiddle').'>');
        $smarty->assign('sListLinkIcon1',$t);
        $smarty->assign('sListRecLink','<a href="show_admit_obstetrics.php'.URL_APPEND."&pid=".$pid.'&target='.$target.'">'.$LDKhamtoanthan.'</a>');
        $smarty->assign('sListRecLink1','<a href="show_admit_obstetrics_beside.php'.URL_APPEND."&pid=".$pid.'&target='.$target.'">'.$LDBeside.'</a>');
        $smarty->assign('sListRecLink2','<a href="show_admit_obstetrics_in.php'.URL_APPEND."&pid=".$pid.'&target='.$target.'">'.$LDIn.'</a>');
    }else{
        $t='';
        $smarty->assign('sListLinkIcon','<img '.createComIcon($root_path,'bul_arrowgrnlrg.gif','0','absmiddle').'>');
        $smarty->assign('sListLinkIcon1',$t);
        $smarty->assign('sListRecLink','<a href="show_admit_obstetrics_phu.php'.URL_APPEND."&pid=".$pid.'&target='.$target.'">'.$LDKhamtoanthan.'</a>');
        $smarty->assign('sListRecLink1','<a href="show_admit_obstetrics_chuyenkhoa.php'.URL_APPEND."&pid=".$pid.'&target='.$target.'">'.$LDChuyenkhoa.'</a>');
    }
...
$smarty->assign('sMainBlockIncludeFile','medocs/main_KS_general.tpl');
