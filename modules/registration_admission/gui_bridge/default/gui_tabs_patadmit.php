...
if(!isset($notabs)||!$notabs){
	...
	if($target=="stats") $img='stats-blu.gif'; //echo '<img '.createLDImgSrc($root_path,'archive_green.gif','0').'  alt="'.$LDArchive.'">';
	else{$img='stats-gray.png'; }
	$pbBuffer='<a href="aufnahme_stats.php'.URL_APPEND.'&target=stats&currDay='.date("d").'&currMonth='.date("m").'&currYear='.date("Y").'"><img '.createLDImgSrc($root_path,$img,'0').' alt="'.$LDArchive.'" title="'.$LDArchive.'" ';
	if($cfg['dhtml']) $pbBuffer.='class="fadeOut" ';
	$pbBuffer.=' align=middle></a>';
	$smarty->assign('pbStats',$pbBuffer);

	if($target=="report") $img='admitreport-blue.png'; //echo '<img '.createLDImgSrc($root_path,'archive_green.gif','0').'  alt="'.$LDArchive.'">';
		else{$img='admitreport-gray.png'; }
	$pbBuffer='<a href="aufnahme_report.php'.URL_APPEND.'&target=list"><img '.createLDImgSrc($root_path,$img,'0').' alt="'.$LDArchive.'" title="'.$LDArchive.'" ';
	if($cfg['dhtml']) $pbBuffer.='class="fadeOut" ';
	$pbBuffer.=' align=middle></a>';
	$smarty->assign('pbReport',$pbBuffer);
