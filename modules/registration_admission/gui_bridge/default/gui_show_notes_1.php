...
if($row1=$status->FetchRow()){
    ...
    if($current_dept_nr!=null){
        $smarty->assign('current_dept_nr',$$current_dept_nr);
    }else{
...
<?php
						while(list($x,$v)=each($types)){
                                                    if($v['LD_var']=='LDConsultNotes' || $v['LD_var']=='LDOther' || $v['LD_var']=='LDDischargeSummary'){                                                    
?>
							<TR>
								<td align=center>
									<img
								...
								<TD vAlign=top >
									<a href="show_notes_1.php<?php echo URL_APPEND ?>&pid=<?php echo $pid ?>&target=<?php echo $target ?>&type_nr=<?php echo $v['nr'] ?>">
<?php
