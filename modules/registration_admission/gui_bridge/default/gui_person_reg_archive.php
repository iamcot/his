...
$img_female=createComIcon($root_path,'spf.gif','0');
 ...
 while($result=$ergebnis->FetchRow()){
  ...
  echo '>
	...
	$sql="SELECT * FROM care_encounter WHERE pid='".$result['pid']."'AND encounter_date=(SELECT max(encounter_date) FROM care_encounter WHERE pid='".$result['pid']."') ";
	$temp=$db->Execute($sql);
	if($temp->RecordCount()){
	 $temp2=$temp->FetchRow();
	 if($temp2['is_discharged']==0){
	 $sql="SELECT name FROM care_class_insurance WHERE class_nr='".$temp2['insurance_class_nr']."'";
						$tem=$db->Execute($sql);
						$tem->RecordCount();
						$tem2=$tem->FetchRow();
		echo ' <td>&nbsp; &nbsp;'.@formatDate2Local($temp2['encounter_date'],$date_format).'('.$tem2['name'].')</td>';
	 } 
	 else{
		echo ' <td>&nbsp; &nbsp; Đã xuất viện ('.@formatDate2Local($temp2['discharge_date'],$date_format).')</td>';
	 }
	} else{
	echo ' <td>&nbsp; &nbsp; Chua tiep nhan</td>';
	}
	echo'
  </tr>
  <tr>
  <td colspan=8 height=1 class="thinrow_vspacer"><img src="../../gui/img/common/default/pixel.gif" border=0 width=1 height=1></td>
		...
		echo '			<tr><td colspan=7>'.$pagen->makePrevLink($LDPrevious).'</td>
