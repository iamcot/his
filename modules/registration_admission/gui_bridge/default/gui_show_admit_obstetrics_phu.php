...
<?php
		...
		while(list($z,$x)=each($fields)){
			if($x=='nr'||$x=='encounter_nr'||$x=='history'||$x=='modify_id'||$x=='modify_time'||$x=='create_id'||$x=='create_time'||$x=='flag'||empty($pregbuf[$show_preg_enc][$x])) continue;
    ...
    <td  style="padding-left:15px;">
        <FONT color="#006600">
            <b>
                <?php
                    if($x=='date'){
                        echo $LD['date_khamngoai'];
                    }else if($x=='thantietnieusinhduc_notes'){
                        echo $LDThantietnieusinhduc;
                    }else
                       echo $LD[$x];                                         
                ?>
            </b>
