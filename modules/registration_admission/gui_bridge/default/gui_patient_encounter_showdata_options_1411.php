...
if(file_exists($root_path.'gui/img/common/default/post_discussion.gif')){
	...
	$TP_iconBubble = '<img '.createComIcon($root_path,'chart.gif','0').'>';
	$TP_iconTalk = '<img '.createComIcon($root_path,'discussions.gif','0').'>';
	$TP_iconEye = '<img '.createComIcon($root_path,'eye_s.gif','0').'>';
	$TP_iconOGuy = '<img '.createComIcon($root_path,'pharma.jpg','0').'>';
	$TP_iconDepot = '<img '.createComIcon($root_path,'storage.gif','0').'>';
	$TP_iconChemical = '<img '.createComIcon($root_path,'Chemical.jpg','0').'>';
	...
	$TP_iconBill = '<img '.createComIcon($root_path,'dollarsign.gif','0').'>';
}else{
	$TP_iconPost = '';
	$TP_iconFolder = '';
	$TP_iconBubble = '';
...
//if($encounter_class_nr==2){
	if($data_entry){
		$TP_PRESCRIPTIONS="<a href=\"show_prescription.php".URL_APPEND."&pid=$pid&target=$target&encounter_nr=".$_SESSION['sess_en']." &type=pres\">$LDPrescriptions</a>";
	}else{
		$TP_PRESCRIPTIONS="<font color='#333333'>$LDPrescriptions</font>";
	}
	//$TP_TREATMENT="<font color='#333333'>$LDSheetTreatment</font>";
//}else{
		//$TP_PRESCRIPTIONS ="<font color='#333333'>$LDPrescriptions</font>";
	if($data_entry){
		$TP_TREATMENT="<a href=\"show_prescription.php".URL_APPEND."&pid=$pid&target=$target&encounter_nr=".$_SESSION['sess_en']."&type=sheet\">$LDSheetTreatment</a>";
	}else{
		$TP_TREATMENT="<font color='#333333'>$LDSheetTreatment</font>";
	}
//}
...
if($data_entry){
	$TP_DEPOT="<a href=\"show_depot.php".URL_APPEND."&pid=$pid&target=$target&encounter_nr=".$_SESSION['sess_en']." \">$LDDepot</a>";
}else{
	$TP_DEPOT="<font color='#333333'>$LDDepot</font>";
}

if($data_entry){
	$TP_CHEMICAL="<a href=\"show_chemical.php".URL_APPEND."&pid=$pid&target=$target&encounter_nr=".$_SESSION['sess_en']." \">$LDChemical</a>";
}else{
	$TP_CHEMICAL="<font color='#333333'>$LDChemical</font>";
}

if($data_entry){
	$TP_NOTESREPORTS="<a href=\"show_notes_1.php".URL_APPEND."&pid=$pid&target=$target\">$LDGiayto</a>";
}else{
	$TP_NOTESREPORTS="<font color='#333333'>$LDGiayto</font>";
}
...
/////////////// add 16/11-Huỳnh ////////////////
if($data_entry&&$death_date==DBF_NODATE){
$TP_REQUEST_LAB="<a href=\"show_appointment_1.php".URL_APPEND."&pid=$pid&target=search&type_nr=4\">$LDRequestLAB</a>";
}else{
$TP_REQUEST_LAB="<font color='#333333'>$LDRequestLAB</font>";
}
if($data_entry&&$death_date==DBF_NODATE){
$TP_REQUEST_OP="<a href=\"request-OP-station-patientdaten-doconsil.php".URL_APPEND."&pn=".$_SESSION['sess_en']."&edit=1&status=&target=or&noresize=1&mode=\">$LDRequestOP</a>";
}else{
$TP_REQUEST_OP="<font color='#333333'>$LDRequestOP</font>";
}
if($data_entry&&$death_date==DBF_NODATE){
$TP_REQUEST_XQUANG="<a href=\"".$root_path."modules/nursing/nursing-station-patientdaten-doconsil-radio.php".URL_APPEND."&pn=".$_SESSION['sess_en']."&edit=1&status=&target=radio&user_origin=lab&noresize=1&mode=\">$LDRequestXquang</a>";
}else{
$TP_REQUEST_XQUANG="<font color='#333333'>$LDRequestXquang</font>";
}

# Links to medocs module
if($data_entry){
	$TP_MEDOCS="<a href=\"".$root_path."modules/medocs/show_medocs.php".URL_APPEND."&encounter_nr=".$_SESSION['sess_en']."&edit=$edit&from=$from&is_discharged=$is_discharged&target=$target\">$LDMedocs1</a>";
}else{
	$TP_MEDOCS="<font color='#333333'>$LDMedocs1</font>";
}
...
//Huynh-khoa san
if($data_entry){
    $TP_ADMIT="<a href=\"show_admit_general.php".URL_APPEND."&pid=$pid&pn=".$_SESSION['sess_en']."&target=$target\">".$LDKhambenh."</a>";	
}else{
	$TP_ADMIT="<font color='#333333'>".$LDVisit."</font>";
}

$TP_NOINHI="<a href=\"show_admit_noinhi.php".URL_APPEND."&pid=$pid&target=$target\">".$LDNoiNhi."</a>";

if($id_dept['id']==7 || $id_dept['id']==6 && $encounter_class_nr==1){

        //Hoi benh phu san
	if($data_entry&&$sex=='f') {
//		$TP_QUESTION_OBSTETRICS="<a href=\"show_pregnancy_question.php".URL_APPEND."&pid=$pid&target=$target\">$question_Obstetrics</a>";
                $TP_QUESTION_OBSTETRICS="<a href=\"show_obstetrics_general.php".URL_APPEND."&pid=$pid&pn=".$_SESSION['sess_en']."&target=$target\">$question_Obstetrics</a>";
	}else{
		$TP_QUESTION_OBSTETRICS="<font color='#333333'>$question_Obstetrics</font>";
	}
	# If the sex is female, show the pregnancies option link
        //tinh trang san phu sau sinh
	if($data_entry&&$sex=='f') {
		$TP_preg_BLK="<a href=\"show_pregnancy.php".URL_APPEND."&pid=$pid&pn=".$_SESSION['sess_en']."&target=$target\">$LDPregnancies</a>";
	}else{
		$TP_preg_BLK="<font color='#333333'>$LDPregnancies</font>";
	}
	//dac diem so rau
        if($data_entry&&$sex=='f') {
		$TP_sorau_BLK="<a href=\"show_placenta.php".URL_APPEND."&pid=$pid&pn=".$_SESSION['sess_en']."&target=$target\">$SorauInfo</a>";
	}else{
		$TP_sorau_BLK="<font color='#333333'>$SorauInfo</font>";
	}
        //dac diem tre so sinh
	if($data_entry){
		$TP_BIRTHDX="<a href=\"show_birthdetail.php".URL_APPEND."&pid=$pid&pn=".$_SESSION['sess_en']."&target=$target\">$LDBirthDetails</a>";
	}else{
