...
$s=$s_obj->BASIC_String();
		...
		if(is_numeric($suchwort)) {
			...
			$sql2=" WHERE ( enc.encounter_nr='$suchwort'  OR enc.encounter_nr $sql_LIKE '%$suchwort%' OR enc.encounter_nr $sql_LIKE '%$suchwort' )";
	    } else {
			...
			# Check the size of the comp
				...
				$sql2=" WHERE ( (CONCAT(reg.name_last,' ',reg.name_first) $sql_LIKE '$searchkey%')
									OR (CONCAT(reg.name_last,' ',reg.name_first) $sql_LIKE '".$s_obj->upper($searchkey)."%') OR (CONCAT(reg.name_last,' ',reg.name_first) $sql_LIKE '%".$s_obj->upper($searchkey)."%')";
...
/*    	if(isset($_SESSION['department_nr']) && $_SESSION['department_nr'] != '' ) {
		...
		$sql2 .= $cond;*/
		//end : gjergji	
			// AND enc.is_discharged=0 remove 1810 -cot
...
if($mode=='search'||$mode=='paginate'){
	...
	if ($linecount) {
		...
		while($zeile=$ergebnis->FetchRow()){
			...
			if($zeile['insurance_class_nr']==1){
			$smarty->assign('sInsuranceType','BHYT');
			}elseif($zeile['insurance_class_nr']==2){
			$smarty->assign('sInsuranceType','Thu Phí');
			}elseif($zeile['insurance_class_nr']==3){
			$smarty->assign('sInsuranceType','Miễn Phí');
			}elseif($zeile['insurance_class_nr']==4){
			$smarty->assign('sInsuranceType','Khác');
			}
			$dept_name=$dept->LDvar($zeile['current_dept_nr']);
			$smarty->assign('sDept',$$dept_name);
