<?php
    ...
    */
	...
	if($allow_update){
		...
		$obj->setWhereCondition('encounter_nr='.$_POST['encounter_nr'].' AND flag=2');
    ...
    if($current_encounter){
        $pregs=&$obj->getEncounterKhambenh_Phu($_SESSION['sess_en'],2);
    }
    if($pregs){
        $rows=$pregs->RecordCount();
    }    

    $page_title=$LDKhamtoanthan;
    $subtitle=$LDKhamtoanthan;
    $_SESSION['sess_file_return']="show_admit_general.php".URL_APPEND."&pid=".$_SESSION['sess_pid']."&encounter_nr=".$_SESSION['sess_en']."&target=search&flag=KS_p";
