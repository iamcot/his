...
if (isset ( $target )) {
	switch ($target) {
		...
		//add
		case 'referrer_dr' :
			define ( 'PNAME', 'referrer_dr' );
			$searchkey = str_replace(" ","%", $searchkey);
			$sql = "SELECT pn.nr, CONCAT(p.name_last, ' ',p.name_first) AS '".PNAME.".value', 0 AS use_frequency FROM care_personell AS pn, care_person AS p WHERE (pn.pid = p.pid) ";
			if ($mode == 'search') {
