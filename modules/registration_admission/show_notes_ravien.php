    ...
    */
    ...
    if(!isset($mode)){
		...
		$time=(empty($x_time))?date('H:i:s'):convertTimeToStandard($x_time);
                ...
                if($_POST['date_ravien'] && $_POST['time']){
                    $data_array.=" notes='".formatDate2STD($_POST['date_ravien'],$date_format).' '.convertTimeToStandard($_POST['time'])."' ";
                ...
                if($_POST['chandoan']){
                    ...
                    $data_array=$array[0]." notes='".$_POST['chandoan']."' ";
                ...
                if($_POST['loidan']){
                    $array=explode("notes=",$data_array);
                    $data_array=$array[0]." notes='".$_POST['loidan']."' ";           
		...
		$time=(empty($x_time))?date('H:i:s'):convertTimeToStandard($x_time);
                ...
                if($_POST['date_ravien'] && $_POST['time']){
                    $data_array['notes']=formatDate2STD($_POST['date_ravien'],$date_format).' '.convertTimeToStandard($_POST['time']);
                    $enc_obj->saveDischargeNotesFromArray1($data_array,33);
                    //insert log
                    $logs->writeline_his($_SESSION['sess_login_userid'], $thisfile, $enc_obj->getLastQuery(), date('Y-m-d H:i:s'));
                }else{
                    echo $enc_obj->getLastQuery()."<br>$LDDbNoSave";
                ...
                if($_POST['chandoan']){
                    $data_array['notes']=$_POST['chandoan'];
                    $enc_obj->saveDischargeNotesFromArray1($data_array,34);
                    //insert log
                    $logs->writeline_his($_SESSION['sess_login_userid'], $thisfile, $enc_obj->getLastQuery(), date('Y-m-d H:i:s'));
                }else{
                    echo $enc_obj->getLastQuery()."<br>$LDDbNoSave";
                ...
                if($_POST['loidan']){
                    $data_array['notes']=$_POST['loidan'];
                    $enc_obj->saveDischargeNotesFromArray1($data_array,35);
                    //insert log
                    $logs->writeline_his($_SESSION['sess_login_userid'], $thisfile, $enc_obj->getLastQuery(), date('Y-m-d H:i:s'));
                }else{
                    echo $enc_obj->getLastQuery()."<br>$LDDbNoSave";
    ...
    if($current_encounter){
        $pregs=&$obj->_getNotesKhac("notes.encounter_nr='$current_encounter' AND notes.type_nr=types.nr AND types.sort_nr=31", "ORDER BY nr ASC");
        if($pregs){
            $rows=$pregs->RecordCount();
        }
    }      

    $page_title=$LDInfoGiayto.'::'.$LDRavien;
    $subtitle=$LDRavien;
