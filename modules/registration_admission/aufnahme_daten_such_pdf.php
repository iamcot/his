			...
			if($ergebnis=$db->SelectLimit($sql,$pagen->MaxCount(),$pagen->BlockStartIndex()))
       		{
				if ($linecount=$ergebnis->RecordCount()) 
				{
					if(($linecount==1) && $numeric&&$mode=='search')
					{
						...
						header('Location:aufnahme_daten_zeigen_pdf.php'.URL_REDIRECT_APPEND.'&from=such&encounter_nr='.$zeile['encounter_nr'].'&target=search');
						exit;
					}
...
if($mode=='search'||$mode=='paginate'){
	...
	if ($linecount) {
		...
		while($zeile=$ergebnis->FetchRow()){
			...
			$sTarget = "<a href=\"aufnahme_daten_zeigen_pdf.php".URL_APPEND."&from=such&encounter_nr=$full_en&target=search\">";
