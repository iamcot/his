...
if (isset ( $target )) {
	switch ($target) {
		...
		case 'referrer_dr' :
			...
			$sql = "SELECT pn.nr AS '".PNID.".value', CONCAT(p.name_last, ' ',p.name_first) AS '".PNAME.".value', 0 AS use_frequency, pns.location_nr FROM care_personell AS pn, care_person AS p, care_personell_assignment AS pns WHERE (pn.pid = p.pid) AND (pn.job_function_title=4 OR pn.job_function_title=12) AND pn.nr=pns.personell_nr";
			if ($mode == 'search') {
...
function setValue(val) {
	...
	for($i3 = 1; $i3 < sizeof ( $var_parent_document ) - 1; $i3 = $i3 + 2) {
            if(isset($obj_name)){
                $str='=array_val[' . $indice . ']+"-MNV:"+array_val[' . $indice1 . '];';
            }else{
                $str='=array_val[' . $indice . '];';
            }
		echo 'mywin.document.' . $formname . '.' . $var_parent_document [$i3] . $str;
		$indice ++;
                $indice1++;
	}
	;
...
if ($linecount) {
	...
	echo '
						<td><font face=arial size=2 color="#336633"><b>' . $LDDept . '</b></td>';
	echo '
	...
	for($i = 0; $i < sizeof ( $sql_value ); $i ++) {
		...
		echo "</td>";
                $dept_name=$dept->checkNameDept($sql_value [$i][3]);
                echo "<td>&nbsp;" .$$dept_name['LD_var'] ."</td>";
		echo "<td><font face=arial size=2>";
