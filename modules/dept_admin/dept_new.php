...
if(!empty($mode)&&!$inputerror){
	...
	{	
		case 'create': 
		{
			...
			if($dept_obj->checkDepartmentExist($_POST['name_formal'], $_POST['id'])) {
				echo "$LDAlertDeptExist";
				exit;
			}
		...
		case 'update':
		{ 
			if($_POST['is_sub_dept'] == 0) $_POST['parent_dept_nr'] = 0;
...
<script src="<?php echo $root_path; ?>js/jquery.maskedinput-1.2.2.js"></script>
<script language="javascript">
    jQuery(function($){
		$("#workhours").mask("S: 99h99 - 99h99; C: 99h99 - 99h99");	
                $("#consult_hours").mask("S: 99h99 - 99h99; C: 99h99 - 99h99");	
	});
function chkForm(d){
	if(d.name_formal.value==""){
		...
		d.type.focus();
		return false;
	}else if((d.inphone1.value!="") && isNaN(d.inphone1.value)){
		alert("<?php echo $LDPhoneIsNaN ?>");
		d.inphone1.focus();
		return false;
	}else if((d.inphone2.value!="") && isNaN(d.inphone2.value)){
		alert("<?php echo $LDPhoneIsNaN ?>");
		d.inphone2.focus();
		return false;
	}else if((d.inphone3.value!="") && isNaN(d.inphone3.value)){
		alert("<?php echo $LDPhoneIsNaN ?>");
		d.inphone3.focus();
		return false;
	}else if((d.funk1.value!="") && isNaN(d.funk1.value)){
		alert("<?php echo $LDPhoneIsNaN ?>");
		d.funk1.focus();
		return false;
	}else if((d.funk2.value!="") && isNaN(d.funk2.value)){
		alert("<?php echo $LDPhoneIsNaN ?>");
		d.funk2.focus();
		return false;
	}else{
...
function issubDeptChange(issubdept){
	if(issubdept == 0) document.getElementById("slt_parent_dept_nr").disabled = true;
	else document.getElementById("slt_parent_dept_nr").disabled = false;
}

function checkFormalNameExist(formalname){
	if(formalname != '<?php echo $name_formal; ?>'){
		var xmlhttp;
		...
		xmlhttp.onreadystatechange=function()
		{
			if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				if(xmlhttp.responseText == "1") {
					alert("<?php echo $LDAlertFormalNameExist; ?>");
					document.getElementById("tbxnameformal").focus();
					return false;
				}
			}
		}
		xmlhttp.open("GET","<?php echo $root_path;?>modules/dept_admin/checkFormalNameExist.php?formalname="+formalname,true);
		xmlhttp.send();
	}
}

function checkDeptIDExist(deptid){
	if(deptid != '<?php echo $id; ?>'){
		var xmlhttp;
		if (window.XMLHttpRequest)
		{// code for IE7+, Firefox, Chrome, Opera, Safari
			xmlhttp=new XMLHttpRequest();
		...
		xmlhttp.onreadystatechange=function()
		{
			...
			{
				if(xmlhttp.responseText == "1") {
					alert("<?php echo $LDAlertDeptIDExist; ?>");
					document.getElementById("tbxdeptid").focus();
					return false;
				}
			}
		}
		xmlhttp.open("GET","<?php echo $root_path;?>modules/dept_admin/checkDeptIdExist.php?deptid="+deptid,true);
		xmlhttp.send();
	}
}        
</script>
...
<table  class="propertylist nursingtb">
  <tr>
    <td class=pblock align=right bgColor="#eeeeee" style="width:25%"><font color=#ff0000><b>*</b><?php echo $LDFormalName ?></font>: </td>
    <td class=pblock><input type="text" name="name_formal" size=40 maxlength=40 value="<?php echo $name_formal ?>" id="tbxnameformal" onblur="checkFormalNameExist(this.value)"><br>
</td>
  </tr> 
  <tr>
	...
	?>
	<input type="text" name="id" size=40 maxlength=40 value="<?php echo $id; ?>" id="tbxdeptid" onblur="checkDeptIDExist(this.value)">
	<?php
	}
	?>
</td>
...
<tr>
  ...
  <tr>
    ...
    <td class=pblock>	<input type="radio" name="is_sub_dept" value="1" <?php if($is_sub_dept) echo 'checked'; ?> onchange="issubDeptChange(this.value)"> <?php echo $LDYes ?> <input type="radio" name="is_sub_dept" value="0" <?php if(!$is_sub_dept) echo 'checked'; ?> onchange="issubDeptChange(this.value)"> <?php echo $LDNo ?>
</td>
  </tr> 
<tr>
    <td class=pblock align=right bgColor="#eeeeee"><font color=#ff0000><b>*</b></font><?php echo $LDParentDept; ?>: </td>
    <td class=pblock><select name="parent_dept_nr" id="slt_parent_dept_nr" <?php if(!$is_sub_dept) echo "disabled='disabled';"?>>
	<option value=""> </option>';
	<?php
  ...
  <tr>
    ...
    <td class=pblock><input type="text" id="workhours" name="work_hours" size=40 maxlength=40 value="<?php echo $work_hours ?>"><br>
</td>
  </tr> 

  <tr>
    <td class=pblock align=right bgColor="#eeeeee"><?php echo $LDConsultationHrs ?>: </td>
    <td class=pblock><input type="text" id="consult_hours" name="consult_hours" size=40 maxlength=40 value="<?php echo $consult_hours ?>"><br>
</td>
  </tr> 
...
<input type="submit" class="butadd"  value="">
<a href="javascript:history.back()" style="float:left;" class="butcancel"><img <?php echo createLDImgSrc($root_path,'cancel.gif','0') ?> border="0"></a>
<?php
}
?>
</form>
