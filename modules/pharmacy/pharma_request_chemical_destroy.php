    ...
    if(!isset($mode)) $mode='';
        ...
        if ($search==''){
            ...
            if (strrpos($search,'/') || strrpos($search,'-')){
                ...
                $condition=" WHERE status_finish='0' AND destroy_id LIKE '%".$search."%' ORDER BY destroy_id";
            $breakfile = $thisfile.URL_APPEND;
        }
        $list_report = $Cabinet->listAllChemicalDestroy($condition);
        if(is_object($list_report)){
            $batchrows = $list_report->RecordCount();		

            if($batchrows && (!isset($report_id) || !$report_id)){ 			// Check for the report_id = $destroy_id. If available get the patients data to show 
                $report_show = $list_report->FetchRow();		
                $report_id = $report_show['destroy_id'];
            }
...
<?php
        echo $LDDestroyChemicalNotFound;
	echo '<center><a href="'.$breakfile.'"><img '.createLDImgSrc($root_path,'back2.gif','0').'></a></center>';
	exit;
}
    /* Check for the report id = $destroy_id. If available get the patients data */
    if($batchrows && $report_id){

        //GET DATA PRESCRIPTION (get all medicine in this reportpaper)

        if($chemical_in_report = $Cabinet->getDetailChemicalDestroyInfo($report_id)){
    ...
    # Prepare title
    $sTitle = $LDPendingDestroyRequest;
...
<!-- 
	...
	var r=confirm("<?php echo $LDIsMedicineDestroy; ?>");
	if (r==true) {
		document.form_test_request.action="includes/inc_destroy_chemical_statusfinish.php?report_id="+ report_id +"&user_origin=<?php echo $user_origin; ?>";
...
<table border=0>
    <tr valign="top">
        <td>
        <?php 
            include('includes/inc_destroy_request_lister_fx.php');
        ?>
        </td>
        <td>
                ...
                <tr>
                    <td>
                        <font size="3" color="#5f88be">
                            <b><?php echo $LDRequestChemicalDestroy; ?></b>
                    </td>
                    <td align="right">
                                ...
                                <tr>	<!-- ***************      HIEN THI NOI DUNG (MEDICINE) TRONG PHIEU LINH THUOC      ***************    -->
                                    ...
                                    <?php 
                                        if(($edit || $read_form) && $chemical_count){ 
                                            if($report_show = $Cabinet->getChemicalDestroyInfo($report_id))
                                                require('includes/inc_destroy_chemical_in_list.php');
                                        } 
                                    ?>
                                    </td>
                                </tr>
