...
{
	var win = 'includes/khochan_payout_chemical_showdetail.php<?php echo URL_APPEND; ?>' +'&report_id=' + issue_id+"&type=chemical";
...
$new_report_url='payout_chemical.php';
...
$smarty->assign('LDCreatorName',$LDPayOutPerson1);
$smarty->assign('LDDatetime',$LDDate);
$smarty->assign('LDTotal',$LDTotal);
$smarty->assign('LDExportTo',$LDPlace);
...
if ($search==''){
	...
	$condition="WHERE chemical_type_pay_out='1'  ORDER BY pay_out_id ";
		
	$total_items=$Pharma->countChemicalPayOutItems($condition);
	...
	$listItem = $Pharma->listSomeChemicalPayOutSplitPage($current_page,$number_items_per_page,$condition);
		
}else{
	if (strrpos($search,'/') || strrpos($search,'-')){
		$search = formatDate2STD($search,'dd/mm/yyyy');
		$condition=" WHERE chemical_type_pay_out='1' AND date_time LIKE '".$search."%' ORDER BY date_time DESC";
	}
	else
		$condition=" WHERE chemical_type_pay_out='1' AND pay_out_id LIKE '%".$search."%' ORDER BY pay_out_id";	
		$listItem = $Pharma->listAllChemicalPayOut($condition);	
	...
	if(is_object($listItem)){
		...
		for ($i=0;$i<$listItem->RecordCount();$i++)
		{
			...
			if ($rowItem['status_finish']){
				$editbutton="javascript:alert('".$LDCannotEditPayOut."')";
				$status_finish=$LDPayOutOk; $temp1='check-r.gif';
			}
			else{
				$editbutton='payout_chemical.php'.URL_APPEND.'&payout_id='.$rowItem['pay_out_id'].'&target=update';
