<?php
    ...
    if ($search==''){
        $condition=" WHERE status_finish='0' ".$health_station." AND chemical_type_pay_out='1' ORDER BY create_time";
    }
    else{
        if (strrpos($search,'/') || strrpos($search,'-')){
                $search = formatDate2STD($search,'dd/mm/yyyy');
                $condition=" WHERE status_finish='0'  ".$health_station." AND chemical_type_pay_out='1' AND create_time LIKE '".$search."%' ORDER BY create_time DESC";
        }
        else
                $condition=" WHERE status_finish='0' ".$health_station."  AND chemical_type_pay_out='1' AND pay_out_id LIKE '%".$search."%' ORDER BY pay_out_id";
        $breakfile = $thisfile.URL_APPEND;
    }
    $list_report = $Pharma->listAllChemicalPayOut($condition);
...
<?php
    ...
    if($batchrows && $report_id){
        ...
        if($chemical_in_report = $Pharma->getDetailChemicalPayOutInfo($report_id)){
    ...
    # Prepare title
    $sTitle = $LDPendingChemicalPayOutRequest;
...
{ 
	...
	if (r==true) {
		document.form_test_request.action="includes/inc_khochan_chemical_payout_statusfinish.php?report_id="+ report_id+"&user_origin=<?php echo $user_origin; ?>";
		...
		<table border="0" width="850"> <!-- ***************     SEARCH      ***************    -->
			<tr>
				<td>
                                    ...
                                    <b>
					<?php if($mode=='khole') echo $LDKhoLe.': '.$TypepayoutChemical; 
						else echo $LDKhoChan.': '.$TypepayoutChemical; ?>
                                    </b>
                                </td>
				<td align="right">
			...
			<tr>	<!-- ***************      HIEN THI NOI DUNG (MEDICINE) TRONG PHIEU LINH THUOC      ***************    -->
				<td align="center">
					<?php 
						...
						if($report_show = $Pharma->getChemicalPayOutInfo($report_id))
							require('includes/inc_khochan_payout_chemical_in_list.php');
					} ?>
				</td>
			</tr>    <!-- *************************************************************************************    -->
			<tr><td>&nbsp;<br></td></tr>
