...
$fileforward='includes/khochan_putin_medipot_save.php'.URL_APPEND;
...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDKhoChan.' :: '.$TypePutInMedipot);
 ...
 # Window bar title
 $smarty->assign('Name',$TypePutInMedipot);
...
function insertRow()
{
	...
	xmlhttp.open("GET","includes/khochan_putin_addmedicine.php?i="+idnum+'&stt_nr='+laststt,true);
	xmlhttp.send();
...
{
	var win = 'includes/khochan_search_medipot.php?id_number='+id_number;
...
function Medicine_AutoComplete(i){
			...
			var includeScript = "includes/khochan_putin_autocomplete_medipot.php?mode=auto&k="+i;
			new Ajax.Autocompleter(name_med,"hint",includeScript, {
					method: 'get',
					paramName: 'search',
...
{
	var process_file='includes/khochan_putin_autocomplete_medipot.php?mode=filldata';
	var name_med='medicine'+i;
...
$list_report_url='putin_medipot_list_medicine.php';
ob_start();
...
$smarty->assign('LDMedicineName',$LDMedipotName);
...
$smarty->assign('sTypePutInInput','<input type="text" value="'.$TypePutInMedipot.'" class="input1" readonly>');
...
if(!isset($target) || ($target=='new') || ($target=='create')){
		...
		for ($i=1;$i<=$maxid;$i++){
			if($flag) {
				...
				if($listReport = $Product->SearchCatalogMedipotKhoLe($condition)){
			...
			ob_start();
				$stt_nr=$i+1;
				require('includes/khochan_putin_addmedicine.php');
    ...
    $smarty->assign('sType',$LDTypeInt);	
		...
		$listReport = $Pharma->getDetailPutInMedInfo($putin_id);
		if(is_object($listReport)){
			...
			for ($i=1;$i<=$maxid;$i++){
				...
				ob_start();
				require('includes/khochan_putin_addmedipot.php');
				$sTempDiv = $sTempDiv.ob_get_contents();				
				ob_end_clean();							
			}	
	...
	$smarty->assign('sTotalInput','<input type="text" id="total_money" name="total_money" class="input1" value="'.str_replace(',','',number_format($rowReport['totalcost']/(1+$rowReport['vat']/100))).'"/>');
...
$smarty->assign('sMainBlockIncludeFile','pharmacy/put_in.tpl');
