...
$list_report = $Cabinet->listAllReturn($condition);
...
if($batchrows && $report_id){
	...
	if($medicine_in_report = $Cabinet->getDetailReturnInfo($report_id)){
...
{ 
	...
	var r=confirm("<?php echo $LDIsMedicineReturn; ?>");
	if (r==true) {
		document.form_test_request.action="includes/inc_return_medipot_statusfinish.php?report_id="+ report_id +"&user_origin=<?php echo $user_origin; ?>";
...
include('includes/inc_return_medipot_request_lister_fx.php');
...
?></td> <!-- ************************************************************************    -->
	...
	<form name="form_test_request" method="post" >
		<table border="0" width="850"> <!-- ***************     SEARCH      ***************    -->
			<tr>
				<td><font size="3" color="#5f88be"><b><?php echo $LDRequestMedipotReturn; ?></b></td>
			...
			<tr>	<!-- ***************      HIEN THI NOI DUNG (MEDICINE) TRONG PHIEU LINH THUOC      ***************    -->
				<td align="center">
					<?php if(($edit || $read_form) && $medicine_count){ 
						if($report_show = $Cabinet->getReturnInfo($report_id))
							require('includes/inc_return_medipot_in_list.php');
					} ?>
				</td>
			</tr>    <!-- *************************************************************************************    -->
			<tr><td>&nbsp;<br></td></tr>
