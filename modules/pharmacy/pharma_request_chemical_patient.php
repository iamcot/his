...
if(!$mode) {	//$mode='' : load all pres	
	if (!$typeInOut || $typeInOut=='all')
		$list_pres = $Pres->getAllChemicalPresByTypePatient('allpatient','0','0');
	elseif ($typeInOut=='inpatient')
		$list_pres = $Pres->getAllChemicalPresByTypePatient('inpatient','0','0');
	else
		$list_pres = $Pres->getAllChemicalPresByTypePatient('outpatient','0','0');
...
<?php
		echo $LDPresChemicalNotFound.'<br/>';
...
if($batchrows && $pres_id){
	...
	if( $enc_obj->loadEncounterData($pn)) {
		...
		if( $enc_obj->is_loaded){
			...
			if($chemical_in_pres = $Pres->getAllChemicalInPres($pres_id)){
...
{ 
	...
	if (r==true) {
		document.form_test_request.action="includes/inc_pres_chemical_statusfinish.php?pres_id="+ pres_id+"&radiovalue=<?php echo $radiovalue; ?>&user_origin=<?php echo $user_origin; ?>";
    ...
    function RefreshList(radio)
{
  ...
  for (i = 1; i <= n; i++)
  {
	if(document.getElementsByName('sumcost'+i).length)
		total = total + document.form_test_request['sumcost'+x].value*1;
  }
...
<?php 
							if(($edit || $read_form) && $chemical_count)
								$pres_show = $Pres->getPrescriptionChemicalInfo($pres_id);
...
?>					
						<table border=0 cellpadding=5 width="100%">	
						   ...
						   <tr bgcolor="#f6f6f6">
							...
							<tr>
								<td><FONT SIZE=-1 color="#000066"><?php echo $LDDangToa; ?></td>
							...
							<tr>
								<td><FONT SIZE=-1   color="#000066"><?php echo $LDGotChe; ?></td>
								<td>
									<?php
											echo $tempfinish.' ';?>
					...
					<td colspan="3">
						<table border=0 cellpadding=5 width="100%">	
							<tr bgcolor="#f6f6f6">
								...
								<td width="24%"><FONT SIZE=-1   color="#000066"><?php echo $LDTongTien; ?></td>
		...
		<tr>	
			<td align="center">
				<?php if(($edit || $read_form) && $chemical_count){ 
						if($pres_show)
							require('includes/inc_chemical_in_pres.php');
				} ?>
			</td>
		</tr>    <!-- *************************************************************************************    -->
...
<input type="hidden" id="countpres" name="countpres" value="<?php echo $chemical_count; ?>">
		...
		<td> 
			...
			<table border="0" > <!-- ***************     SEARCH      ***************    -->
				...
				<tr>
					...
					<td align="right"><input OnClick="search()" type="image" <?php echo createComIcon($root_path,'Search.png','0','',TRUE); ?> ></td>
				</tr>
				<tr>
