...
<?php
    ...
    */
    ...
    $breakfilecatalogue=$root_path.'modules/pharmacy/catalogue_chemical.php'.URL_APPEND;
    ...
    $_SESSION['sess_user_origin']='chemical';
    ...
    */
    ...
    $this_file="catalogue_chemical.php";
    $process_file='catalogue_process.php';
    $ajax_file="catalogue_chemical_ajax.php";
	...
	$img_remove=createComIcon($root_path,'pharma_remove.jpg','0');
...
<script language="javascript">
    ...
    //----------Hoa chat-----------
    function Chemical_AutoComplete()
    {
        $.ajax({
        type: "GET",
        url: process_file+"?catalogue=chemical&key=autocomplete",
        cache: false,
        async: false,
        data: "quick="+document.getElementById("group_of_chemical_input").value,
        success: function(data) {
                    var a = data.split("@#");

                    $( "#group_of_chemical_input" ).autocomplete({
                                    source: a
                            });
                    }
        });
    }
    function Fill_Data_GroupName_Chemical()
    {
            var temp=document.getElementById("group_of_chemical_input").value;
            //temp=temp.replace('+','@*');
            temp=temp.split('+').join('@*');

            $.ajax({
                    type: "GET",
                    url: "catalogue_chemical_ajax.php?catalogue=chemical_filldata&key=autocomplete",
                    cache: false,
                    async: false,
                    data: "quick="+temp,
                    success: function(data) {
                            var a= data.split("@#");
                            document.getElementById("chemical_group_name").innerHTML=a[1];
...
function OnChange_GroupNameSub()
{
	var temp=document.getElementById("pharma_group_name_input").value;
	var xmlhttp;
    ...
    xmlhttp.onreadystatechange=function(){
		if (xmlhttp.readyState==4 && xmlhttp.status==200){
			document.getElementById('pharma_group_name_sub').innerHTML= xmlhttp.responseText;
		}
	}
	xmlhttp.open("GET","catalogue_group_sub.php?groupid="+temp,true);
	xmlhttp.send();  
}
    ...
    {
//        alert(">>>"+last_page+"<<<<--->>>>"+current_page+"<<<<<<--->>>>>"+current_page+"<<<<<");
        $.ajax({
        type: "GET",
    ...
    //hàm biến đổi khi nhấn nút cancel	//cancel($catalogue,page,$_product_encoder,$_product_name,$_type_name_of_medicine,$_generic_drug,$_content,$_component,$_using_type,$_effects,$_description,$_caution,$_price,$_supplier,$_pharma_generic_drug_id,$_pharma_group_name,$_unit_name_of_medicine,$_unit_of_price_name,$_sodangky,$_hangsx,$_nuocsx,$_note,'',$_in_use)
    function cancel(catalogue, page, obj, value1, value2, value3, value4, value5, value6, value7, value8, value9, value10, value11, value12, value13, value14, value15, value16, value17, value18, note, history, status)
    {
        document.getElementById("save").innerHTML='<input type="hidden"/>';
        document.getElementById("cancel").innerHTML='<input type="hidden"/>';
		document.getElementById("remove").innerHTML='<input type="hidden"/>';
        if (catalogue=='supplier')
        {
            ...
            document.getElementById("edit").innerHTML='<input type="image" '+'<?php echo $img_edit ?>' + ' onclick="edit('+"'"+catalogue+"','"+page+"','"+obj+"','"+value1+"','"+value2+"','"+value3+"','"+value4+"','"+value5+"','"+value6+"','"+value7+"','"+value8+"','"+value9+"','"+value10+"','"+value11+"','"+value12+"','"+value13+"','"+value14+"','"+value15+"','"+value16+"','"+value17+"','"+value18+"','"+note+"','"+history+"','"+status+"'"+')" title="'+'<?php echo $LDEdit ?>'+'"/>';
    //        document.getElementById("disable").innerHTML='<input type="button" value="'+'<?php echo $LDDisable ?>'+'"/>';
        ...
        if (catalogue=='generic_drug')
        {
			...
			document.getElementById("generic_drug").innerHTML=value1;
			document.getElementById("pharma_group_name_sub").innerHTML=value10;
        ...
        if ((catalogue=='medicine') || (catalogue=='vnmedicine'))
        {
			...
			document.getElementById("sodangky").innerHTML=value16;
			document.getElementById("hangsx").innerHTML=value17;
			document.getElementById("nuocsx").innerHTML=value18;
        ...
        if (catalogue=='chemical')
        {
            ...
            document.getElementById("caution").innerHTML=value9;
            document.getElementById("product_encoder").innerHTML=obj;
            document.getElementById("supplier").innerHTML='<a href="<?php echo $this_file ?>?catalogue=supplier&lang=<?php echo $lang ?>&flag=1&obj_primary=' +value11+ '"><b>' + value11 + '</b></a>';
            document.getElementById("price").innerHTML=value10;
            document.getElementById("unit_name_of_chemical").innerHTML=value14;
            document.getElementById("currency").innerHTML=value15;
            ...
            document.getElementById("chemical_group_name").innerHTML=value2;
    ...
    //save($catalogue,page,$_product_encoder,$_product_name,$_type_name_of_medicine,$_generic_drug,$_content,$_component,$_using_type,$_effects,$_description,$_caution,$_price,$_supplier,$_pharma_generic_drug_id,$_pharma_group_name,$_unit_name_of_medicine,$_unit_of_price_name,'','','',$_note,'',$_in_use)

    function save(catalogue, page, obj, value1, value2, value3, value4, value5, value6, value7, value8, value9, value10, value11, value12, value13, value14, value15, value16, value17, value18, note, history, status)
    {
        if (catalogue=="generic_drug")
        {
            var hospital_th='';
        ...
        if (catalogue=="chemical")
        {
                var temp_i = document.getElementById("unit_name_of_chemical_input").selectedIndex;
                var select_unit=document.getElementById("unit_name_of_chemical_input").options[temp_i].value;
                var select_unit_text=document.getElementById("unit_name_of_chemical_input").options[temp_i].text;

                temp_i = document.getElementById("unit_of_price").selectedIndex;
                var select_unitprice=document.getElementById("unit_of_price").options[temp_i].value;
                var select_unitprice_text=document.getElementById("unit_of_price").options[temp_i].text;
                
                var temp_i = document.getElementById("group_of_chemical_input").selectedIndex;
                var select_group=document.getElementById("group_of_chemical_input").options[temp_i].value;
                var select_group_text=document.getElementById("group_of_chemical_input").options[temp_i].text;
        }
                ...
                if (xmlhttp.responseText!='fail')	//if success
                    {
                        ...
                        {                       
                            if (catalogue=="supplier")
                            {
                                document.getElementById("edit").innerHTML='<input type="image" '+'<?php echo $img_edit ?>' + ' onclick="edit('+"'"+catalogue+"','"+page+"','"+obj+"','"+document.getElementById("supplier_name_input").value+"','"+document.getElementById("type_name_of_supplier_input").value+"','"+document.getElementById("address_input").value+"','"+document.getElementById("telephone_input").value+"','"+document.getElementById("fax_input").value+"','"+xmlhttp.responseText+"','"+value7+"','"+value8+"','"+value9+"','"+value10+"','"+value11+"','"+value12+"','"+value13+"','"+value14+"','"+value15+"','"+value16+"','"+value17+"','"+value18+"','"+document.getElementById("note_supplier_input").value+"','"+history+"','"+status+"'"+')" title="'+'<?php echo $LDEdit ?>'+'"/>';
                            ...
                            if (catalogue=="generic_drug")
                            {
								...
								var cbxtext=temptext.split('@@@');
								
								document.getElementById("edit").innerHTML='<input type="image" '+'<?php echo $img_edit ?>' + ' onclick="edit('+"'"+catalogue+"','"+page+"','"+obj+"','"+value1+"','"+document.getElementById("pharma_group_name_input").value+"','"+document.getElementById("drug_id_input").value+"','"+document.getElementById("using_type_input").value+"','"+document.getElementById("effects_input").value+"','"+document.getElementById("description_input").value+"','"+cbxtext[0]+"','"+value8+"','"+cbxtext[1]+"','"+value10+"','"+value11+"','"+value12+"','"+value13+"','"+value14+"','"+value15+"','"+value16+"','"+value17+"','"+hospital_text+"','"+document.getElementById("note_input").value+"','"+history+"','"+status+"'"+')" title="'+'<?php echo $LDEdit ?>'+'"/>';								
                                
								document.getElementById("pharma_group_name").innerHTML=cbxtext[0];
								document.getElementById("pharma_group_name_sub").innerHTML=cbxtext[1];
                                ...
                                document.getElementById("generic_drug").innerHTML=document.getElementById("generic_drug_input").value;
                            ...
                            if (catalogue=="medicine" || catalogue=="vnmedicine")
                            {
                                                            ...
                                                            var temp_product_name = document.getElementById("product_name_input").value;
															...
															var temp_sodangky = document.getElementById("sodangky_input").value;
                                                            var temp_hangsx = document.getElementById("hangsx_input").value;
                                                            var temp_nuocsx = document.getElementById("nuocsx_input").value;
															
                                                            var temp_note = document.getElementById("note_input").value;

                                                            document.getElementById("edit").innerHTML='<input type="image" '+'<?php echo $img_edit ?>' + ' onclick="edit('+"'"+catalogue+"','"+page+"','"+obj+"','"+temp_product_name+"','"+select_type_text+"','"+temp_generic_drug+"','"+temp_content+"','"+temp_component+"','"+temp_using_type+"','"+temp_effects+"','"+temp_description+"','"+temp_caution+"','"+temp_price+"','"+temp_supplier+"','"+temp_ger_drug_id+"','"+temp_group_name+"','"+select_unit_text+"','"+ select_unitprice_text+"','"+temp_sodangky+"','"+temp_hangsx+"','"+temp_nuocsx+"','"+temp_note+"','"+history+"','"+status+"'"+')" title="'+'<?php echo $LDEdit ?>'+'"/>';
								...
								document.getElementById("sodangky").innerHTML=temp_sodangky;
								document.getElementById("hangsx").innerHTML=temp_hangsx;
								document.getElementById("nuocsx").innerHTML=temp_nuocsx;
                                ...
                                document.getElementById("product_name").innerHTML=temp_product_name;
...
//                                var temp_product_name = value1; //document.getElementById("product_name").value;
                                ...
                                var temp_content=''; 	 
                                var temp_group_name=value2;
                                var temp_description = document.getElementById("description_input").value;
                                var temp_caution = document.getElementById("caution_input").value;
                                var temp_price = document.getElementById("price_input").value;
                                var temp_currency = document.getElementById("currency").value;
                                var temp_supplier = document.getElementById("supplier_input").value;
                                var temp_product_name = document.getElementById("product_name_input").value;
                                //var temp_product_encoder = document.getElementById("product_encoder_input").value;
                                ...
                                document.getElementById("edit").innerHTML='<input type="image" '+'<?php echo $img_edit ?>' + ' onclick="edit('+"'"+catalogue+"','"+page+"','"+obj+"','"+temp_product_name+"','"+select_group_text+"','','','','','','"+temp_description+"','"+temp_caution+"','"+temp_price+"','"+temp_supplier+"','"+temp_ger_drug_id+"','"+temp_group_name+"','"+select_unit_text+"','"+ select_unitprice_text+"','"+temp_sodangky+"','"+temp_hangsx+"','"+temp_nuocsx+"','"+temp_note+"','"+history+"','"+status+"'"+')" title="'+'<?php echo $LDEdit ?>'+'"/>';
                                ...
                                document.getElementById("price").innerHTML=temp_price;
                                document.getElementById("supplier").innerHTML=temp_supplier;
                                document.getElementById("unit_name_of_chemical").innerHTML=select_unit_text; 
                                document.getElementById("currency").innerHTML=select_unitprice_text;                         
                                document.getElementById("note").innerHTML=temp_note;
                                ...
                                document.getElementById("product_name").innerHTML=temp_product_name;
                                //document.getElementById("product_encoder").innerHTML=temp_product_encoder;
                                document.getElementById("chemical_group_name").innerHTML=select_group_text;//document.getElementById("group_of_chemical_input").value;
                                //alert(xmlhttp.responseText);
                            ...
                            document.getElementById("save").innerHTML='<input type="hidden"/>';
							document.getElementById("remove").innerHTML='<input type="hidden"/>';
                            load_page(catalogue, page, page, "<?php echo $records_in_page ?>", "<?php echo $quick?>");
                        }
        ...
        if ('<?php echo $mode ?>'=='new')
        {
            ...
            if (catalogue=="generic_drug")
            {				
                xmlhttp.open("GET",process_file+"?catalogue="+catalogue+"&obj="+obj+"&value1="+document.getElementById("generic_drug_input").value+"&value2="+document.getElementById("pharma_group_name_input").value+"&value3="+document.getElementById("drug_id_input").value+"&value4="+document.getElementById("using_type_input").value+"&value5="+document.getElementById("description_input").value+"&value6="+document.getElementById("effects_input").value+"&value7="+value7+"&value8="+value8+"&value9="+document.getElementById("pharma_group_name_sub_input").value+"&value10="+value10+"&note="+document.getElementById("note_input").value+"&hospital_th="+hospital_th+"&history="+history+"&key=new",true);
            }
        ...
        else
        {
            ...
            if (catalogue=="generic_drug")
            {
                            xmlhttp.open("GET",process_file+"?catalogue="+catalogue+"&obj="+obj+"&value1="+value1+"&value2="+document.getElementById("pharma_group_name_input").value+"&value3="+document.getElementById("drug_id_input").value+"&value4="+document.getElementById("using_type_input").value+"&value5="+document.getElementById("description_input").value+"&value6="+document.getElementById("effects_input").value+"&value7="+document.getElementById("generic_id_input").value+"&value8="+value8+"&value9="+document.getElementById("pharma_group_name_sub_input").value+"&value10="+value10+"&note="+document.getElementById("note_input").value+"&hospital_th="+hospital_th+"&history="+history+"&key=save",true);
            }
            if (catalogue=="medicine" || catalogue=="vnmedicine")
            {			
                ...
                xmlhttp.open("GET",process_file+"?catalogue="+catalogue+"&obj="+obj+"&value1="+document.getElementById("product_name_input").value+"&value2="+temp_ger_drug_id+"&value3="+temp_content+"&value4="+document.getElementById("component_input").value+"&value5="+document.getElementById("using_type_input").value+"&value6="+select_type+"&value7="+select_unit+"&value8="+document.getElementById("caution_input").value+"&value9="+document.getElementById("supplier_input").value+"&value10="+document.getElementById("price_input").value+"&value11="+select_unitprice+"&value12="+status+"&value13="+document.getElementById("description_input").value+"&value14="+document.getElementById("effects_input").value+"&value15="+document.getElementById("note_input").value+"&value16="+document.getElementById("sodangky_input").value+"&value17="+document.getElementById("hangsx_input").value+"&value18="+document.getElementById("nuocsx_input").value+"&history="+history+"&key=save",true);
            }
            if (catalogue=="medipot") //chua lam
            {			
                xmlhttp.open("GET",process_file+"?catalogue="+catalogue+"&obj="+obj+"&value1="+document.getElementById("product_name_input").value+"&value2=&value3=&value4=&value5=&value6=&value7="+select_unit+"&value8="+document.getElementById("caution_input").value+"&value9="+document.getElementById("supplier_input").value+"&value10="+document.getElementById("price_input").value+"&value11="+select_unitprice+"&value12="+status+"&value13="+document.getElementById("description_input").value+"&value14=&value15="+document.getElementById("note_input").value+"&value16="+document.getElementById("sodangky_input").value+"&value17="+document.getElementById("hangsx_input").value+"&value18="+document.getElementById("nuocsx_input").value+"&history="+history+"&key=save",true);
            }
            if (catalogue=="chemical") //chua lam
            {	 
                //alert(process_file+"?catalogue="+catalogue+"&obj="+obj+"&value1="+value1+"&value2="+value2+"&value7="+value7+"&value8="+value8+"&value9="+document.getElementById("description_input").value+"&value10="+document.getElementById("caution_input").value+"&value11="+document.getElementById("price_input").value+"&value12="+document.getElementById("supplier_input").value+"&value13="+value13+"&value14="+value14+"&value15="+select_unit+"&value16="+select_unitprice+"&note="+document.getElementById("note_input").value+"&history=&key=save");
                xmlhttp.open("GET",process_file+"?catalogue="+catalogue+"&obj="+obj+"&encoder_1="+obj+"&value1="+document.getElementById("product_name_input").value+"&value2="+document.getElementById("group_of_chemical_input").value+"&value7="+value7+"&value8="+document.getElementById("description_input").value+"&value9="+document.getElementById("caution_input").value+"&value10="+document.getElementById("price_input").value+"&value11="+document.getElementById("supplier_input").value+"&value12="+value12+"&value13="+value13+"&value14="+select_unit+"&value15="+select_unitprice+"&value16="+document.getElementById("sodangky_input").value+"&value17="+document.getElementById("hangsx_input").value+"&value18="+document.getElementById("nuocsx_input").value+"&note="+document.getElementById("note_input").value+"&history=&key=save<?php echo '&page='?>"+page,true);                
            }            
        }
    ...
    //HÀM SELECT
    function select_obj(catalogue,obj,page)
    {   
                    $.ajax({
                        type: "GET",
                        url:process_file+"?catalogue="+catalogue+"&key=select_obj&obj="+obj,
                        data:'',
                    ...
                    if (catalogue=='supplier')
                        {
                            ...
                            document.getElementById("edit").innerHTML='<input type="image" '+'<?php echo $img_edit ?>' + ' onclick="edit('+"'supplier','"+page+"','"+obj+"','"+myhash["supplier_name"]+"','"+myhash["type_of_supplier"]+"','"+myhash["address"]+"','"+myhash["telephone"]+"','"+myhash["fax"]+"','"+myhash["type_name_of_supplier"]+"','','','','','','','','','','','','','"+myhash["note"]+"','','"+'1'+"'"+')" title="'+'<?php echo $LDEdit ?>'+'"/>';

                        }
                    else if (catalogue=='generic_drug')
                        {
							...
							document.getElementById("pharma_group_name_sub").innerHTML=myhash["pharma_group_name_sub"];
                            ...
                            document.getElementById("edit").innerHTML='<input type="image" '+'<?php echo $img_edit ?>' + ' onclick="edit('+"'generic_drug','"+page+"','"+obj+"','"+myhash["generic_drug"]+"','"+myhash["pharma_group_id"]+"','"+myhash["drug_id"]+"','"+myhash["using_type"]+"','"+myhash["effects"]+"','"+myhash["description"]+"','"+myhash["pharma_group_name"]+"','"+myhash["generic_drug_id"]+"','"+myhash["pharma_group_id_sub"]+"','"+myhash["pharma_group_name_sub"]+"','','','','','','','','"+temp_th+"','"+myhash["note"]+"','','"+'1'+"'"+')" title="'+'<?php echo $LDEdit ?>'+'"/>';
                        }

                    else if ((catalogue=='medicine')||(catalogue=='vnmedicine'))
                        {
							...
							document.getElementById("sodangky").innerHTML=myhash["sodangky"];
							document.getElementById("nuocsx").innerHTML=myhash["nuocsx"];
							document.getElementById("hangsx").innerHTML=myhash["hangsx"];

                            document.getElementById("edit").innerHTML='<input type="image" '+'<?php echo $img_edit ?>' + ' onclick="edit(\''+catalogue+"','"+page+"','"+obj+"','"+myhash["product_name"]+"','"+myhash["type_name_of_medicine"]+"','"+myhash["generic_drug"]+"','"+myhash["content"]+"','"+myhash["component"]+"','"+myhash["using_type"]+"','"+myhash["effects"]+"','"+myhash["description"]+"','"+myhash["caution"]+"','"+myhash["price"]+"','"+myhash["supplier"]+"','"+myhash["pharma_generic_drug_id"]+"','"+myhash["pharma_group_name"]+"','"+myhash["unit_name_of_medicine"]+"','"+myhash["short_name"]+"','"+myhash["sodangky"]+"','"+myhash["hangsx"]+"','"+myhash["nuocsx"]+"','"+myhash["note"]+"','','"+'1'+"'"+')" title="'+'<?php echo $LDEdit ?>'+'"/>';
                            ...
                            if(catalogue=='medicine'){
								...
								document.getElementById("pharma_group_name").innerHTML=myhash["generic_drug"];
								document.getElementById("generic_drug").innerHTML=myhash["pharma_group_name"];
							}
                        }
                    else if (catalogue=='medipot')
                        {
                            document.getElementById("product_name").innerHTML=myhash["product_name"];
                            document.getElementById("caution").innerHTML=myhash["caution"];
                            document.getElementById("description").innerHTML=myhash["description"];
                            ...
                            document.getElementById("edit").innerHTML='<input type="image" '+'<?php echo $img_edit ?>' + ' onclick="edit('+"'medipot','"+page+"','"+obj+"','"+myhash["product_name"]+"','"+myhash["type_name_of_medicine"]+"','','','','','"+myhash["effects"]+"','"+myhash["description"]+"','"+myhash["caution"]+"','"+myhash["price"]+"','"+myhash["supplier"]+"','','"+myhash["pharma_group_name"]+"','"+myhash["unit_name_of_medicine"]+"','"+myhash["short_name"]+"','"+myhash["sodangky"]+"','"+myhash["hangsx"]+"','"+myhash["nuocsx"]+"','"+myhash["note"]+"','','"+'1'+"'"+')" title="'+'<?php echo $LDEdit ?>'+'"/>';
                        }
                    else if (catalogue=='chemical')
                        {
                            document.getElementById("product_name").innerHTML=myhash["product_name"];
                            document.getElementById("caution").innerHTML=myhash["caution"];
                            document.getElementById("description").innerHTML=myhash["description"];
                            document.getElementById("supplier").innerHTML='<a href="'+'<?php echo $this_file ?>' + '?catalogue=supplier&lang='+'<?php echo $lang ?>'+'&obj_primary=' +myhash["supplier"] + '"><b>' + myhash["supplier"] + '</b></a>';
                            document.getElementById("product_encoder").innerHTML=obj;
                            document.getElementById("unit_name_of_chemical").innerHTML=myhash["unit_name_of_chemical"];
                            document.getElementById("price").innerHTML=myhash["price"];
                            document.getElementById("currency").innerHTML=myhash["short_name"] ;
                            document.getElementById("note").innerHTML=myhash["note"];     
                            ...
                            if($("#chemical_group_name"))
                                $("#chemical_group_name").html(myhash["chemical_group_name"]);
                            if($("#group_of_chemical_input"))
                                setSelect(document.getElementById("group_of_chemical_input"),"group_of_chemical_input",myhash["chemical_group_name"]);
//                            document.getElementById("chemical_group_name").innerHTML=myhash["chemical_group_name"];
                           document.getElementById("edit").innerHTML='<input type="image" '+'<?php echo $img_edit ?>' + ' onclick="edit('+"'chemical','"+page+"','"+obj+"','"+myhash["product_name"]+"','"+myhash["chemical_group_name"]+"','','','','','"+myhash["effects"]+"','"+myhash["description"]+"','"+myhash["caution"]+"','"+myhash["price"]+"','"+myhash["supplier"]+"','','"+myhash["chemical_group_name"]+"','"+myhash["unit_of_chemical"]+"','"+myhash["unit_name_of_chemical"]+"','"+myhash["sodangky"]+"','"+myhash["hangsx"]+"','"+myhash["nuocsx"]+"','"+myhash["note"]+"','','"+'1'+"'"+')" title="'+'<?php echo $LDEdit ?>'+'"/>';
                        }
                    }});
 //               }
//            }
...
//        xmlhttp.send();
   ...
   function setSelect(selectbox,id,text){
...
function load_catalogue(){
    ...
    GLOBAL $LDTitleVnMedicine; GLOBAL $LDTitleChemical; GLOBAL $LDTitleMedipot; GLOBAL $LDSearchInputText;
    ...
    $sCatalogueInfo=load_catalogue1($catalogue, $quick, $attribute, $group_key, $records_in_page, 1, &$records, &$flag);
    //lấy trang 1 của danh mục + tổng số kết quả
                ...
                if ($records>0)
                    {
                        ...
                        if($flag!=1){
                            ...
                            $_obj=new Product();
                            if ($result=$_obj->GetFirstChemicalCatalogue($quick, $group_key, $attribute))
             ...
             if ($records>0)
                        {
                            ...
                            if($flag!=1){
                                if ($obj_primary==''){
                                    if($temp=$_obj->GetFirstGenericDrugCatalogue($quick, $group_key, $attribute))
										$obj_info= $temp->FetchRow();
								}	
                                else {
									if($temp=$_obj->GetGenericDrugInfo($obj_primary, $username))
										$obj_info=$temp->FetchRow();
								}	
                            }else{
                                $_obj=new Product();
                     ...
                     if ($records>0)
                        {
                            if ($mode=='') $mode='view';
                            if ($obj_primary==''){
                                 if($temp = $_obj->GetFirstMedicineCatalogue($quick, $group_key, $attribute))
									$obj_info=$temp->FetchRow();
							}	 
                            else {
								if($temp = $_obj->GetMedicineInfo($obj_primary, $username))
									$obj_info= $temp->FetchRow();
							}
                        }
                }
                            ...
                            if ($records>0)
                                {
                                    ...
                                    else if ($result=$_obj->GetChemicalInfo($obj_primary, $username))
                                        $obj_info=$result->FetchRow();
                                }
                        }
                }
    ...
    if ($sCatalogueInfo=='')
     ...
     else
     {
         if ($records%$records_in_page==0) $total_pages=$records/$records_in_page;
            ...
            $LDCurrent='<a href="javascript:load_page('."'$catalogue'".','.$total_pages.',1,'.$records_in_page.','."'$quick'".')"><b>[1]</b></a>';
         if ($total_pages>1) 
             $LDLast='<a href="javascript:load_page('."'$catalogue'".','.$total_pages.','.$total_pages.','.$records_in_page.','."'$quick'".')">'.$total_pages.'</a>';
         else
         {
             $LDLast='';
             $LDNext='';
         ...
         if ($total_pages>=3)  $LDNext='<a href="javascript:load_page('."'$catalogue'".','.$total_pages.',2,'.$records_in_page.','."'$quick'".')">2</a>';
         else $LDNext='';
         if ($total_pages>=4)  $LDNext2='<a href="javascript:load_page('."'$catalogue'".','.$total_pages.',3,'.$records_in_page.','."'$quick'".')">3</a>';
         else $LDNext2='';
         if ($total_pages>=5)  $LDNext3='<a href="javascript:load_page('."'$catalogue'".','.$total_pages.',3,'.$records_in_page.','."'$quick'".')">4</a>';
         else $LDNext3='';
         if ($total_pages>=6)  $LDNext4='<a href="javascript:load_page('."'$catalogue'".','.$total_pages.',3,'.$records_in_page.','."'$quick'".')">5</a>';
         else $LDNext4='';
         if ($total_pages>=7)  $LDNext5='<a href="javascript:load_page('."'$catalogue'".','.$total_pages.',3,'.$records_in_page.','."'$quick'".')">6</a>';
         else $LDNext5='';
     }
...
{    
    ...
    if ($catalogue=='generic_drug' || $catalogue=='medicine' || $catalogue=='vnmedicine')
    {
         if ($group_key=='')
         $dropdown_menu_temp='<ul id="nav"><li class="top"><a href="'.strtr($this_file.URL_APPEND.'&catalogue='.$catalogue."&attribute=$attribute&quick=$quick",'','').'" id="dropdown menu" class="top_link"><span class="down">'.$Name.'</span></a> <ul class="sub">';
         $_obj=new Product();
         if ($catalogue=='vnmedicine')
			$type_info=&$_obj->GetPharmaGroupNameVn();
		 else
    ...
    if ($catalogue=='chemical')
    {
         ...
         $dropdown_menu_temp='<ul id="nav"><li class="top"><a href="'.strtr($this_file.URL_APPEND.'&catalogue='.$catalogue."&attribute=$attribute&quick=$quick",'','').'" id="dropdown menu" class="top_link"><span class="down">'.$Name.'</span></a> <ul class="sub">';
         $_obj=new Product();
         $type_info=&$_obj->GetChemicalGroupName();
         //đưa dữ liệu ra chuỗi
         if (is_object($type_info))
         {
               while($type=$type_info->FetchRow())
              {
                   if ($type["chemical_group_id"]!=$group_key)
                        $dropdown_menu=$dropdown_menu.'<li>
                                                            <a href="'.strtr($this_file.URL_APPEND.'&catalogue='.$catalogue."&attribute=$attribute&quick=$quick&group_key=".$type["chemical_group_id"],'','').'">'.$type["chemical_group_name"].'
                                                            </a>
                                                       </li>';
                   else
                        $dropdown_menu_temp='<ul id="nav">
                                                <li class="top">
                                                    <a href="'.strtr($this_file.URL_APPEND.'&catalogue='.$catalogue."&attribute=$attribute&quick=$quick&group_key=".$type["chemical_group_id"],'','').'" id="dropdown menu" class="top_link">
                                                        <span class="down">'.$type["chemical_group_name"].'</span>
                                                    </a>
                                                <ul class="sub">';
              }
         }
...
{
    GLOBAL $dropdown_catalogue; 
    GLOBAL $catalogue;
    GLOBAL $LDMedicineCatalogueTxt; 
    GLOBAL $LDVNMedicineCatalogueTxt; 
    GLOBAL $LDMedipotCatalogueTxt; 
    GLOBAL $LDChemicalCatalogueTxt; 
    GLOBAL $LDGenericDrugCatalogueTxt; 
    GLOBAL $LDSupplierCatalogueTxt;
    ...
    //------------------------------------------------
    if ($catalogue=='chemical'){
        $dropdown_catalogue_temp='<ul id="nav2">
                                    <li class="top">
                                        <a href="'.strtr($this_file.URL_APPEND."&catalogue=$catalogue",'','').'" id="dropdown menu" class="top_link">
                                            <span class="down">'.$LDChemicalCatalogueTxt.'</span>
                                        </a>
                                  <ul class="sub">';
    }else
        $dropdown_catalogue=$dropdown_catalogue.'<li><a href="'.strtr($this_file.URL_APPEND.'&catalogue=chemical','','').'">'.$LDChemicalCatalogueTxt.'</a></li>';
    //------------------------------------------------
...
{    
    ...
    GLOBAL $LDByGenericDrugName;    GLOBAL $LDMedicine;    GLOBAL $LDVNMedicine;    GLOBAL $LDBySupplier;    
    GLOBAL $LDByEncoder;    GLOBAL $LDByComponent;
    if ($catalogue=='supplier')
    {
        if ($attribute=='')//''=Tất cả
...
//smarty-> gán giá trị cho các template 
 ...
 # Window bar title
 $smarty->assign('Name',$LDCatalogue);
...
// Append javascript to JavaScript block
 ...
 $smarty->assign('LDCancel','<p id="cancel"/>');
  $smarty->assign('LDRemove','<p id="remove"/>');
 //chỉ chạy với $catalogue=='supplier'
 ...
 $smarty->assign('LDSearchInput','<form style="margin: 0px; padding: 0px;" effects="'.strtr($this_file.'?'.$_GET["ntid"].''.$_GET["lang"].''.$_GET["mode"].''.$_GET["catalogue"].''.$_GET["quick"],'','').'" method="get">
                                        <input type="text" style="width:100%;"  name="quick" id="quick" onfocus="if(this.value=='."'".$LDSearchInputText."'".') this.value='."''".'" onblur="if(this.value=='."''".') this.value='."'".$LDSearchInputText."'".'" value="'.$LDSearchInput.'"'.'>');
 ...
 $smarty->assign('LDSearchButton','     <input type="image" '.$img_search.' name="search_button" title="'.$LDSearchButton.'">
...
//--------------------------------------------------------------------------------------------------------
 if ($catalogue=='supplier')
 {
     if($flag!=1){
        ...
        if ($group_key!='')
        {
         ...
         $smarty->assign('LDOption', load_dropdownmenu($Type_Chemical_Drug));
        $Title=$LDTitleMedicine;
        $smarty->assign('LDTitle2',$LDTitle2Medicine);
        $smarty->assign('LDArticleName',$LDChemicalName);//Tên Hóa chất
        $smarty->assign('LDStatus',''.$LDStatus); //Trạng thái
        ...
        $smarty->assign('LDNote',''.$LDNote);//Ghi chú		
        $smarty->assign('LDHangSX',''.$LDHangSX);//Hangsx
        $smarty->assign('LDNuocSX',''.$LDNuocSX);//Nuoc sx	
        if ($group_key!='')
        {
            $_obj=new Product();
            $type_info=&$_obj->GetChemicalGroupName($group_key)->FetchRow();
            $group_name=$type_info["chemical_group_name"];
            $smarty->assign('LDGroupName', $group_name);
        }
     }     
 }
  ...
  if ($catalogue=='generic_drug')
 {
      if($flag!=1){
			...
			$smarty->assign('LDGroupSub',$LDGroupSub);//nhóm thuốc con
            $smarty->assign('LDStatus',$LDStatus);//trạng thái
 ...
 if ($catalogue=='vnmedicine')
 {
    $smarty->assign('LDOption', load_dropdownmenu($Type_Generic_Drug));
     ...
     $smarty->assign('LDStatus',''.$LDStatus); //Trạng thái
	 $smarty->assign('LDNhomYHCT',''.$LDGroupNameMed);//Nhóm thuốc
	 $smarty->assign('LDGroupNameMed',''.$LDGroupSub);//Nhóm thuốc con
	 //$smarty->assign('LDContent',''.$LDContent);//Hàm lượng
     $smarty->assign('LDComponent',''.$LDComponent);//Thành phần
     $smarty->assign('LDType_of_Medicine',''.$LDType_of_Medicine);//Dạng thuốc
     $smarty->assign('LDUsing_Type',$LDUsing_Type);//Đường dùng
     ...
     {
         $_obj=new Product();
         $type_info=&$_obj->GetPharmaGroupNameVn($group_key)->FetchRow();
         $group_name=$type_info["pharma_group_name"];
         $smarty->assign('LDGroupName', $group_name);
     }
 }
...
if ($mode == 'save') //Lưu vào CSDL
{
    ...
    {
        $_obj=new Product();
        $obj_primary=$_obj->AddGenericDrug($generic_drug, $pharma_group_id, $pharma_group_id_sub, $drug_id, $generic_drug_id, $using_type, $hospital_5th, $hospital_6th, $hospital_7th, $hospital_8th, $description, $effects, $note, $in_use, $user);
    ...
    if ($catalogue=='medicine')
    {
         if ($price=='') $price=0;
         $_obj=new Product();
         $obj_primary=$_obj->AddMedicine($product_encoder, $product_name, $generic_drug, $content, $component, $using_type, $type_of_medicine, $unit_name_of_medicine_input, $caution, $supplier, $note, $price, $unit_of_price, $effects, $in_use, $description, $user, $sodangky,$hangsx, $nuocsx);
		 $logs->writeline_his($_SESSION['sess_login_userid'], $thisfile, $_obj->getLastQuery(), date('Y-m-d H:i:s'));
    }
    //-------------VNMEDICINE-----------------------
    if ($catalogue=='vnmedicine')
    {
        $_obj=new Product();		
        $obj_primary=$_obj->AddVnMedicine($product_encoder, $product_name, $generic_drug, $content, $component, $using_type, $type_of_medicine, $unit_name_of_medicine_input, $caution, $supplier, $note, $price, $unit_of_price, $effects, $in_use, $description, $user, $sodangky, $hangsx, $nuocsx);
		$logs->writeline_his($_SESSION['sess_login_userid'], $thisfile, $_obj->getLastQuery(), date('Y-m-d H:i:s'));
    }
    //-------------MEDIPOT-----------------------
    if ($catalogue=='medipot')
    {
        ...
        $obj_primary=$_obj->AddMedipot($product_encoder, $product_name, $group_of_medipot_input, $unit_name_of_medicine_input, $caution, $supplier, $note, $price, $unit_of_price, $in_use, $description, $user, $sodangky, $hangsx, $nuocsx);
		$logs->writeline_his($_SESSION['sess_login_userid'], $thisfile, $_obj->getLastQuery(), date('Y-m-d H:i:s'));
    }
    //------------CHEMICAL-------------------------
    if ($catalogue=='chemical')
    {
        $_obj=new Product();
        //echo $product_encoder.' '.$product_name.' '.$group_of_chemical_input.' '.$unit_name_of_chemical_input.' '.$caution.' '.$supplier.' '.$note.' '.$price.' '.$unit_of_price.' '.$in_use.' '.$description.' '.$user;
        $obj_primary=$_obj->AddChemical($product_encoder, $product_name, $group_of_chemical_input, $unit_name_of_chemical_input, $caution, $supplier, $note, $price, $unit_of_price, $in_use, $description, $user, $sodangky, $hangsx, $nuocsx);
		$logs->writeline_his($_SESSION['sess_login_userid'], $thisfile, $_obj->getLastQuery(), date('Y-m-d H:i:s'));
    }
	...
	echo '<script language="JavaScript">
			self.location="'.$breakfilecatalogue.'&catalogue='.$catalogue.'";
		</script>';
...
//     $smarty->assign('LDProducts',$LDProducts);
    ...
    if($catalogue=='supplier')
    {
        if($flag==''){
            ...
            $smarty->assign('LDEdit','<p id="edit"><input type="image" '.$img_edit.' onclick="edit('."'supplier','".$total_pages."','".$_supplier."','".$_supplier_name."','".$_type_of_supplier."','".$_address."','".$_telephone."','".$_fax."','".$_type_name_of_supplier."','','','','','','','','','','','','','".$_note."','','".$_in_use."'".')" title="'.$LDEdit.'"/></p>');
            ...
            if ($obj_info!='')
            {
                ...
                $_chemical_generic_drug_id=$obj_info["chemical_generic_drug_id"];
                
                $_chemical_group_name=$obj_info["chemical_group_name"];
                ...
                $_unit_of_chemical=$obj_info["unit_of_chemical"];
                $_unit_name_of_chemical=$obj_info["unit_name_of_chemical"];
                $_price=$obj_info["price"];
                $_unit_of_price=$obj_info["unit_of_price"];
                $_unit_of_price_name=$obj_info["short_name"];
                $_note=$obj_info["note"];
				
				$_sodangky = $obj_info["sodangky"];
				$_nuocsx = $obj_info["nuocsx"];
				$_hangsx = $obj_info["hangsx"];
            }
            ...
            //Thuốc gốc
            $smarty->assign('sGenericInput',''.'<p id="generic_drug"><a href="'.$this_file.'?catalogue=generic_drug&lang='.$lang.'&flag=1&obj_primary='.$_chemical_generic_drug_id.'"><b>'.$_generic_drug.'</b></a></p>');

                    //Thuốc gốc
            $smarty->assign('sGroupNameInput',''.'<p id="pharma_group_name">'.$_chemical_group_name.'</p>');
            ...
            //Dạng thuốc
            $smarty->assign('sType_of_MedicineInput',''.'<p id="type_name_of_chemical">'.$_type_name_of_chemical.'</p>');

            //Đường dùng
            $smarty->assign('sUsing_TypeInput','<p id="using_type">'.$_using_type.'</p>');
			
			//Hang sx
			$smarty->assign('sHangSXInput','<p id="hangsx">'.$_hangsx.'</p>');
			
			//Nuoc sx
			$smarty->assign('sNuocSXInput','<p id="nuocsx">'.$_nuocsx.'</p>');
            ...
            //Số đăng kí
            $smarty->assign('sRegNoInput',''.'<p id="sodangky">'.$_sodangky.'</p>'.'<input type="hidden" id="product_encoder" value="'.$_product_encoder.'">');

            //Nhà cung cấp
            $smarty->assign('sSupplierInput',''.'<p id="supplier"><a href="'.$this_file.'?catalogue=supplier&lang='.$lang.'&flag=1&obj_primary='.$_supplier.'"><b>'.$_supplier.'</b></a></p>');
            ...
            //Đơn vị tính
            $smarty->assign('sUnitInput',''.'<p id="unit_name_of_chemical">'.$_unit_name_of_chemical.'</p>');

            //Ghi chú
            $smarty->assign('sNoteInput',''.'<p id="note">'.$_note.'</p>');

            $smarty->assign('LDEdit','<p id="edit"><input type="image" '.$img_edit.' onClick="edit('."'$catalogue','".$total_pages."','".$_product_encoder."','".$_product_name."','".$_type_name_of_medicine."','".$_generic_drug."','".$_content."','".$_component."','".$_using_type."','".$_effects."','".$_description."','".$_caution."','".$_price."','".$_supplier."','".$_pharma_generic_drug_id."','".$_pharma_group_name."','".$_unit_name_of_medicine."','".$_unit_of_price_name."','','','','".$_note."','','".$_in_use."'".')" title="'.$LDEdit.'"/></p>');
            $smarty->assign('LDDelete','<p id="disable"><input type="button" value="'.$LDDisable.'"/></p>');
    ...
    //-------------GENERIC_DRUG-----------------
      if($catalogue=='generic_drug')
     {
           if ($obj_info!='')
        {
			...
			$_group_sub=$obj_info["pharma_group_name_sub"];
            $_group_id=$obj_info["pharma_group_id"];
			$_group_id_sub=$obj_info["pharma_group_id_sub"];
            $_using_type=$obj_info["using_type"];
            $_effects=$obj_info["effects"];
            $_description=$obj_info["description"];
        ...
        //Nhóm thuốc con
         $smarty->assign('sGroupInputSub','<p id="pharma_group_name_sub">'.$_group_sub.'</p>');		 
		...
		//$smarty->assign('sHospitalInput','<p id="hospital"><input type="checkbox" name="hospital_5th">'.$LDhospital_5th.'</input> <input type="checkbox" name="hospital_6th">'.$LDhospital_6th.'</input> <input type="checkbox" name="hospital_7th">'.$LDhospital_7th.'</input> <input type="checkbox" name="hospital_8th">'.$LDhospital_8th.'</input> </p>');
         ...
         $smarty->assign('LDEdit','<p id="edit"><input type="image" '.$img_edit.' onclick="edit('."'generic_drug','".$total_pages."','".$_pharma_generic_drug_id."','".$_generic_drug."','".$_group_id."','".$_drug_id."','".$_using_type."','".$_effects."','".$_description."','".$_group."','$_generic_id','".$_group_id_sub."','".$_group_sub."','','','','','','','','$_hospital','".$_note."','','".$_in_use."'".')" title="'.$LDEdit.'"/></p>');
//         $smarty->assign('LDDelete','<p id="disable"><input type="button" value="'.$LDDisable.'"/></p>');
     ...
     if($catalogue=='medicine' || $catalogue=='vnmedicine' || $catalogue=='medipot')
     {
          if ($obj_info!='')
          {
              ...
              $_product_name=$obj_info["product_name"];
              $_in_use=$obj_info["in_use"];
              $_pharma_group_name=$obj_info["pharma_generic_drug_id"];
			  
			  if($catalogue=='vnmedicine'){
				  $_generic_drug=$obj_info["pharma_group_name"];	
				  $_pharma_group_name=$obj_info["generic_drug"];				  
			  }else{
				  $_generic_drug=$obj_info["generic_drug"];	
         ...
         //Thuốc gốc
		 if($catalogue=='vnmedicine')
			$smarty->assign('sGenericInput',''.'<p id="generic_drug">'.$_generic_drug.'</p>');	
		 else
		...
		$smarty->assign('sNuocSXInput','<p id="nuocsx">'.$_nuocsx.'</p>');
         ...
         //Ghi chú
         $smarty->assign('sNoteInput',''.'<p id="note">'.$_note.'</p>');
         $smarty->assign('LDEdit','<p id="edit"><input type="image" '.$img_edit.' onClick="edit('."'$catalogue','".$total_pages."','".$_product_encoder."','".$_product_name."','".$_type_name_of_medicine."','".$_generic_drug."','".$_content."','".$_component."','".$_using_type."','".$_effects."','".$_description."','".$_caution."','".$_price."','".$_supplier."','".$_pharma_generic_drug_id."','".$_pharma_group_name."','".$_unit_name_of_medicine."','".$_unit_of_price_name."','".$_sodangky."','".$_hangsx."','".$_nuocsx."','".$_note."','','".$_in_use."'".')" title="'.$LDEdit.'"/></p>');
         $smarty->assign('LDDelete','<p id="disable"><input type="button" value="'.$LDDisable.'"/></p>');
         $smarty->assign('sSubBlockIncludeFile','pharmacy/form_medicine.tpl');
     }
     //--------------CHEMICAL--------------------
     if($catalogue=='chemical')
     {
         if ($obj_info!='')
          {
              ...
              $_content=$obj_info["content"];
              $_component=$obj_info["component"];
              $_type_of_chemical=$obj_info["type_of_chemical"];
              $_type_name_of_chemical=$obj_info["type_name_of_chemical"];
              $_using_type=$obj_info["using_type"];
              $_effects=$obj_info["effects"];
              $_description=$obj_info["description"];
              $_caution=$obj_info["caution"];
         ...
         $smarty->assign('LDGroupNameMed',''.$LDGroupNameChemical); //Nhóm Hoa chat
		 //nhóm hóa chất
         $smarty->assign('sGroupNameInput',''.'<p id="chemical_group_name">'.$_chemical_group_name.'</p>');
		...
		$smarty->assign('sNuocSXInput','<p id="nuocsx">'.$_nuocsx.'</p>');		 
         ...
         //echo 
         $smarty->assign('LDEdit','<p id="edit"><input type="image" '.$img_edit.' onClick="edit('."'$catalogue','".$total_pages."','".$_product_encoder."','".$_product_name."','".$_type_name_of_chemical."','".$_generic_drug."','".$_content."','".$_component."','".$_using_type."','".$_effects."','".$_description."','".$_caution."','".$_price."','".$_supplier."','".$_chemical_generic_drug_id."','".$_chemical_group_name."','".$_unit_name_of_chemical."','".$_unit_of_price_name."','".$_sodangky."','".$_hangsx."','".$_nuocsx."','".$_note."','','".$_in_use."'".')" title="'.$LDEdit.'"/></p>');
 ...
 ?>
<script>
    function edit(catalogue, page, obj, value1, value2, value3, value4, value5, value6, value7, value8, value9, value10, value11, value12, value13, value14, value15, value16, value17, value18, note, history, status)
    {
        var xmlhttp;
        //document.write("Minh");
        if (window.XMLHttpRequest)
    ...
    //             document.write(xmlhttp.responseText);
                if (catalogue=='supplier')
                    {                    
                        document.getElementById("type_name_of_supplier").innerHTML= xmlhttp.responseText;
                        document.getElementById("cancel").innerHTML='<input type="image" '+'<?php echo $img_cancel ?>' + ' onClick="cancel('+"'"+catalogue+"','"+page+"','"+obj+"','"+value1+"','"+document.getElementById("type_name_of_supplier_input").value+"','"+value3+"','"+value4+"','"+value5+"','"+value6+"','"+value7+"','"+value8+"','"+value9+"','"+value10+"','"+value11+"','"+value12+"','"+value13+"','"+value14+"','"+value15+"','"+value16+"','"+value17+"','"+value18+"','"+note+"','"+history+"','"+status+"'"+')" title="'+'<?php echo $LDCancel ?>'+'"/>';
                        ...
                        document.getElementById("save").innerHTML='<input type="image" '+'<?php echo $img_save ?>' + ' onClick="save('+"'"+catalogue+"','"+page+"','"+obj+"','"+value1+"','"+value2+"','"+value3+"','"+value4+"','"+value5+"','"+value6+"','"+value7+"','"+value8+"','"+value9+"','"+value10+"','"+value11+"','"+value12+"','"+value13+"','"+value14+"','"+value15+"','"+value16+"','"+value17+"','"+value18+"','"+note+"','"+history+"','"+status+"'"+')" title="'+'<?php echo $LDSave ?>'+'"/>';
                        //document.getElementById("history").innerHTML='<input type="text" name="supplier_name" value="'+history+'" style="width:100%" maxlength=300/>';
						
						document.getElementById("remove").innerHTML='<input type="image" <?php echo $img_remove ?>  title="<?php echo $LDRemove ?>"  onClick="remove(\''+catalogue+'\',\''+page+'\',\''+obj+'\')" />';
//                        document.getElementById("supplier").innerHTML='<input type="text" id="supplier_input" name="supplier" value="'+catalogue+'" style="width:100%" maxlength=300>';
                    }
                    if (catalogue=='generic_drug')
                    {                    
                        var temp = xmlhttp.responseText;
						slb = temp.split('@@@');
						document.getElementById("pharma_group_name").innerHTML= slb[0];
						document.getElementById("pharma_group_name_sub").innerHTML= slb[1];
                        ...
                        document.getElementById("save").innerHTML='<input type="image" '+'<?php echo $img_save ?>' + ' onClick="save('+"'"+catalogue+"','"+page+"','"+obj+"','"+value1+"','"+value2+"','"+value3+"','"+value4+"','"+value5+"','"+value6+"','"+value7+"','"+value8+"','"+value9+"','"+value10+"','"+value11+"','"+value12+"','"+value13+"','"+value14+"','"+value15+"','"+value16+"','"+value17+"','"+value18+"','"+note+"','"+history+"','"+status+"'"+')" title="'+'<?php echo $LDSave ?>'+'"/>';
                        document.getElementById("generic_drug").innerHTML='<input type="text" id="generic_drug_input" name="generic_drug" value="'+value1+'" style="width:100%" maxlength=300>';
						
                        document.getElementById("cancel").innerHTML='<input type="image" '+'<?php echo $img_cancel ?>' + ' onClick="cancel('+"'"+catalogue+"','"+page+"','"+obj+"','"+value1+"','"+value2+"','"+value3+"','"+value4+"','"+value5+"','"+value6+"','"+value7+"','"+value8+"','"+value9+"','"+value10+"','"+value11+"','"+value12+"','"+value13+"','"+value14+"','"+value15+"','"+value16+"','"+value17+"','"+value18+"','"+note+"','"+history+"','"+status+"'"+')" title="'+'<?php echo $LDCancel ?>'+'"/>';
                        //document.getElementById("history").innerHTML='<input type="text" name="supplier_name" value="'+history+'" style="width:100%" maxlength=300/>';
                    ...
                    if ((catalogue=='medicine') || (catalogue=='vnmedicine'))
                    {
                        ...
                        document.getElementById("price").innerHTML='<input type="text" id="price_input" name="price_input" value="'+value10+'" style="width:100%" maxlength=300>';
						document.getElementById("sodangky").innerHTML='<input type="text" id="sodangky_input" name="sodangky_input" value="'+value16+'" style="width:100%" maxlength=300>';
						document.getElementById("hangsx").innerHTML='<input type="text" id="hangsx_input" name="hangsx_input" value="'+value17+'" style="width:100%" maxlength=300>';
						document.getElementById("nuocsx").innerHTML='<input type="text" id="nuocsx_input" name="nuocsx_input" value="'+value18+'" style="width:100%" maxlength=300>';
                        ...
                        document.getElementById("cancel").innerHTML='<input type="image" '+'<?php echo $img_cancel ?>' + ' onClick="cancel('+"'"+catalogue+"','"+page+"','"+obj+"','"+value1+"','"+value2+"','"+value3+"','"+value4+"','"+value5+"','"+value6+"','"+value7+"','"+value8+"','"+value9+"','"+value10+"','"+value11+"','"+value12+"','"+value13+"','"+value14+"','"+value15+"','"+value16+"','"+value17+"','"+value18+"','"+note+"','"+history+"','"+status+"'"+')" title="'+'<?php echo $LDCancel ?>'+'"/>';
                        document.getElementById("product_name").innerHTML='<input type="text" id="product_name_input" name="product_name" value="'+value1+'" style="width:100%" maxlength=300>';
                    ...
                    {
                        var data= xmlhttp.responseText.split("@#");
                        document.getElementById("chemical_group_name").innerHTML=data[3];//'<?php  echo load_group_Chemical($_chemical_generic_drug_id)?>';                        
                        document.getElementById("description").innerHTML='<textarea id="description_input" name="description" style="width:100%" rows=2>'+value8+'</textarea>';
                        document.getElementById("caution").innerHTML='<textarea id="caution_input" name="caution" style="width:100%" rows=2>'+value9+'</textarea>';
                        document.getElementById("supplier").innerHTML='<input type="text" id="supplier_input" name="supplier_input"  onkeyup="Supplier_AutoComplete()" onFocus="Supplier_AutoComplete()" value="'+value11+'" style="width:100%" maxlength=300>';
                        document.getElementById("price").innerHTML='<input type="text" id="price_input" name="price_input" value="'+value10+'" style="width:100%" maxlength=300>';
						...
						document.getElementById("nuocsx").innerHTML='<input type="text" id="nuocsx_input" name="nuocsx_input" value="'+value18+'" style="width:100%" maxlength=300>';
                        document.getElementById("currency").innerHTML=data[1];
                        document.getElementById("unit_name_of_chemical").innerHTML=data[0];
                        document.getElementById("note").innerHTML='<textarea id="note_input" name="note" style="width:100%" rows=2>'+note+'</textarea>';
                        document.getElementById("product_name").innerHTML='<input type="text" id="product_name_input" name="product_name" value="'+value1+'" style="width:100%" maxlength=300>';                      
                        //document.getElementById("product_encoder").innerHTML='<input type="text" id="product_encoder_input" name="product_encoder" value="'+obj+'" style="width:100%" maxlength=300>';
                        document.getElementById("save").innerHTML='<input type="image" '+'<?php echo $img_save ?>' + ' onClick="save('+"'"+catalogue+"','"+page+"','"+obj+"','"+value1+"','"+value2+"','"+value3+"','"+value4+"','"+value5+"','"+value6+"','"+value7+"','"+value8+"','"+value9+"','"+value10+"','"+value11+"','"+value12+"','"+value13+"','"+value14+"','"+value15+"','"+value16+"','"+value17+"','"+value18+"','"+note+"','"+history+"','"+status+"'"+')" title="'+'<?php echo $LDSave ?>'+'"/>';
        ...
        if ((catalogue=='medicine') || (catalogue=='vnmedicine') || (catalogue=='medipot') || (catalogue=='chemical'))
		...
		if (catalogue=='generic_drug')
			xmlhttp.open("GET",process_file+"?catalogue="+catalogue+"&obj="+obj+"&key=select_box&select="+value2+"__"+value9,true);
        xmlhttp.send();
    }
	
function remove(catalogue, page, obj){
	var r=confirm("<?php echo $LDWouldDelete; ?>");
	if (r==true) {
		var xmlhttp;
		...
		xmlhttp.onreadystatechange=function() {
			if (xmlhttp.readyState==4 && xmlhttp.status==200){
				if(xmlhttp.responseText!='fail'){
					alert('<?php echo $LDRemoveSuccessful; ?>');
					//alert(xmlhttp.responseText);
					self.location="<?php echo $breakfilecatalogue.'&catalogue='.$catalogue; ?>";
				}
				else{
					alert('<?php echo $LDCannotRemove ?>');
					return false;
				}
			}
		}
		
		xmlhttp.open("GET","catalog_remove_item.php?catalogue="+catalogue+"&obj="+obj,true);
		xmlhttp.send();	
	}
}	
</script>
...
//           
        ...
        if($catalogue=='generic_drug')
       {
             ...
             $smarty->assign('sGenericIdInput','<p id="generic_id"><input type="text" name="generic_drug_id" value="" style="width:100%" maxlength=300/></p>');
			 $tempsbx = explode('@@@',$selected_box);
             $smarty->assign('sGroupInput','<p id="pharma_group_name">'.$tempsbx[0].'</p>');
			 $smarty->assign('sGroupInputSub','<p id="pharma_group_name_sub">'.$tempsbx[1].'</p>');
       ...
       if($catalogue=='medicine' || $catalogue=='vnmedicine' || $catalogue=='medipot')
       {
             ...
             $smarty->assign('sRegNoInput',''.'<p id="sodangky"><input type="text" name="sodangky" value="" style="width:100%" maxlength=300/></p><input id="product_encoder" name="product_encoder" type="hidden" value="">');
			 $smarty->assign('sHangSXInput',''.'<p id="hangsx"><input type="text" name="hangsx" value="" style="width:100%" maxlength=300/></p>');
			 $smarty->assign('sNuocSXInput',''.'<p id="nuocsx"><input type="text" name="nuocsx" value="" style="width:100%" maxlength=300/></p>');
       ...
       {
            $curency_select=load_currency('');
            $_unit_name_of_chemical=load_unit_Chemical('');
            $_chemical_group_name=load_group_Chemical('');
            $smarty->assign('LDGroupNameMed',''.$LDGroupNameChemical); //Nhóm Hoa chat
            $smarty->assign('sGroupNameInput',''.$_chemical_group_name);
            $smarty->assign('sArticleNameInput','<form style="margin: 0px; padding: 0px;" action="'.$this_file.URL_APPEND.'" method="POST">
                                                    <p id="product_name">
                                                        <input type="text" name="product_name" value="" style="width:100%" maxlength=300/>
                                                    </p>');
            $smarty->assign('sStatusInput',''.'<p id="status">'.$_in_use.'</p>');
            $smarty->assign('sGenericInput',''.'<p id="chemical_drug">
                                                    <input type="text" id="chemical_drug_input" name="generic_drug"  onkeyup="Chemical_AutoComplete()" onFocus="Chemical_AutoComplete()" onBlur="Fill_Data_GroupName_Chemical()"  value="" style="width:100%" maxlength=300/>
                                                </p>');
            $smarty->assign('sContentInput',''.'<p id="content">
                                                    <input type="text" name="content" value="" style="width:100%" maxlength=300/>
                                                </p>');
            $smarty->assign('sType_of_MedicineInput',''.'<p id="type_name_of_chemical">'.$selected_box.'</p>');
            $smarty->assign('sUsing_TypeInput','<p id="using_type"><input type="text" name="using_type" value="" style="width:100%" maxlength=300/></p>');
            $smarty->assign('sEffectsInput','<p id="effects"><textarea name="effects" style="width:100%" rows=2></textarea></p>');
            $smarty->assign('sDescriptionInput',''.'<p id="description"><textarea name="description" style="width:100%" rows=2></textarea></p>');
            $smarty->assign('sCautionInput',''.'<p id="caution"><textarea name="caution" style="width:100%" rows=2></textarea></p>');
       ...
       $smarty->assign('LDCancel','<p id="cancel"/>
                                      <a href="'.strtr($this_file.URL_APPEND."&catalogue=$catalogue",'','').'">
                                          <img '.$img_cancel.' title="'.$LDCancel.'"/>
                                      </a>
                                   </p>'.'
