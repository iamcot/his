...
<script  language="javascript">
    ...
    {
        var win = 'includes/khochan_payout_showdetail.php<?php echo URL_APPEND; ?>' +'&report_id=' + issue_id+"&type=medicine";
...
if ($search==''){
    ...
    $condition="WHERE pharma_type_pay_out='1'  ORDER BY pay_out_id ";

    $total_items=$Pharma->countPayOutItems($condition);
    ...
    $listItem = $Pharma->listSomePayOutSplitPage($current_page,$number_items_per_page,$condition);

}else{
    if (strrpos($search,'/') || strrpos($search,'-')){
        $search = formatDate2STD($search,'dd/mm/yyyy');
        $condition=" WHERE pharma_type_pay_out='1' AND date_time LIKE '".$search."%' ORDER BY date_time DESC";
    }
    else
        $condition=" WHERE pharma_type_pay_out='1' AND pay_out_id LIKE '%".$search."%' ORDER BY pay_out_id";
    $listItem = $Pharma->listAllPayOut($condition);
...
if(is_object($listItem)){
    ...
    for ($i=0;$i<$listItem->RecordCount();$i++)
    {
        ...
        if ($rowItem['status_finish']){
            ...
            $editbutton='payout.php'.URL_APPEND.'&payout_id='.$rowItem['pay_out_id'].'&target=update';
