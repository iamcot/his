...
/* Get pending putin */
if($mode=='khole')
	$health_station="AND health_station='0' ";
else{
	$mode='khochan';
	$health_station="AND health_station<>'0' ";
}
...
if ($search==''){
	$condition=" WHERE status_finish='0' ".$health_station." AND pharma_type_pay_out='2' ORDER BY create_time";
}
else{
	if (strrpos($search,'/') || strrpos($search,'-')){
		$search = formatDate2STD($search,'dd/mm/yyyy');
		$condition=" WHERE status_finish='0'  ".$health_station." AND pharma_type_pay_out='2' AND create_time LIKE '".$search."%' ORDER BY create_time DESC";
	}
	else
		$condition=" WHERE status_finish='0' ".$health_station."  AND pharma_type_pay_out='2' AND put_in_id LIKE '%".$search."%' ORDER BY pay_out_id";
	$breakfile = $thisfile.URL_APPEND;
}
$list_report = $Pharma->listAllPayOutMed($condition);
if(is_object($list_report)){
		$batchrows = $list_report->RecordCount();		
		
		if($batchrows && (!isset($report_id) || !$report_id)){ 			// Check for the report_id = $pay_out_id. If available get the patients data to show 
			$report_show = $list_report->FetchRow();		
			$report_id = $report_show['pay_out_id'];
		}
...
/* Check for the report id = $pay_out_id. If available get the patients data */
if($batchrows && $report_id){
		
	//GET DATA PRESCRIPTION (get all medicine in this reportpaper)
			
	if($medicine_in_report = $Pharma->getDetailPayOutMedInfo($report_id)){
...
{ 
	...
	var r=confirm("<?php echo $LDGiveMedipotReport; ?>");
	if (r==true) {
		document.form_test_request.action="includes/inc_khochan_payout_medipot_statusfinish.php?report_id="+ report_id+"&user_origin=<?php echo $user_origin; ?>";
...
require('includes/inc_khochan_payout_request_lister_fx.php');
...
?></td> <!-- ************************************************************************    -->
	...
	<form name="form_test_request" method="post" >
		<table border="0" width="850"> <!-- ***************     SEARCH      ***************    -->
			<tr>
				<td><font size="3" color="#5f88be"><b>
					<?php if($mode=='khole') echo $LDKhoLe.': '.$TypePutInMedipot; 
						else echo $LDKhoChan.': '.$TypepayoutMedipot; ?>
					</b></td>
			...
			<tr>
				<td>
					<a href="#"><input type="image" <?php echo createLDImgSrc($root_path,'abschic.gif','0') ?> onclick="FinishPres(<?php echo $report_id; ?>)" title="<?php echo $LDFinishEntry; ?>"></a>
			...
			<tr>	<!-- ***************      HIEN THI NOI DUNG (MEDICINE) TRONG PHIEU LINH THUOC      ***************    -->
				<td align="center">
					<?php if(($edit || $read_form) && $medicine_count){ 
						if($report_show = $Pharma->getPayOutInfo($report_id))
							require('includes/inc_khochan_payout_medipot_in_list.php');
					} ?>
				</td>
			</tr>    <!-- *************************************************************************************    -->
			<tr><td>&nbsp;<br></td></tr>
