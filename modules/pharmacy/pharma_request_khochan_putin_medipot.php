...
/* Get pending putin */
if ($search==''){
	$condition=" WHERE status_finish='0' AND pharma_type_put_in='2' ORDER BY create_time";
}
else{
	if (strrpos($search,'/') || strrpos($search,'-')){
		$search = formatDate2STD($search,'dd/mm/yyyy');
		$condition=" WHERE status_finish='0' AND pharma_type_put_in='2' AND create_time LIKE '".$search."%' ORDER BY create_time DESC";
	}
	else
		$condition=" WHERE status_finish='0' AND pharma_type_put_in='2' AND put_in_id LIKE '%".$search."%' ORDER BY put_in_id";
	$breakfile = $thisfile.URL_APPEND;
}
$list_report = $Pharma->listAllPutInMed($condition);
...
if($batchrows && $report_id){
	...
	if($medicine_in_report = $Pharma->getDetailPutInMedInfo($report_id)){
...
{ 
	...
	if (r==true) {
		document.form_test_request.action="includes/inc_khochan_putin_medipot_statusfinish.php?report_id="+ report_id+"&user_origin=<?php echo $user_origin; ?>";
...
?></td> <!-- ************************************************************************    -->
	...
	<form name="form_test_request" method="post" >
		<table border="0" width="850"> <!-- ***************     SEARCH      ***************    -->
			<tr>
				<td><font size="3" color="#5f88be"><b><?php echo $LDKhoChan.': '.$TypePutInMedipot; ?></b></td>
				...
				<td align="center">
					<?php if(($edit || $read_form) && $medicine_count){ 
						if($report_show = $Pharma->getPutInInfo($report_id))
							require('includes/inc_khochan_putin_medipot_in_list.php');
					} ?>
				</td>
			</tr>    <!-- *************************************************************************************    -->
			<tr><td>&nbsp;<br></td></tr>
