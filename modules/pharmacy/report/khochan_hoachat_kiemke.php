...
$fileforward='khochan_kiemkehoachat_save.php'.URL_APPEND;
$urlsearch=$thisfile;
...
if($todate!=''){
	$check_sql = "SELECT * FROM care_chemical_khochan_ton_info WHERE todate<='".$todate."' ORDER BY todate DESC ";	
...
{
	if(flag==true){
	urlholder="<?php echo $root_path;?>modules/pdfmaker/duoc/khochan_kiemkeHC.php<?php echo URL_APPEND; ?>&type=medicine&select_type="+select_type+"<?php echo '&todate='.$todate; ?>";
...
<table border="0" cellSpacing="1" cellPadding="3" width="95%" bgColor="#C3C3C3">
	...
	if ($search==''){
		...
		if($flag)
			$listItem = $Product->ShowKhoChanHC_Ton($current_page, $number_items_per_page, $condition, $todate);
		else
			$listItem = $Pharma->Khochan_hoachat_nhapxuatton_theongay($condition, $cond_typeput, $last_report_date, $todate);
		...
		if($flag)
			$listItem = $Product->SearchKhoChanHC_Ton($condition, $todate);
		else
			$listItem = $Pharma->Khochan_hoachat_nhapxuatton_theongay($condition, $cond_typeput, $last_report_date, $todate);
		$breakfile = $thisfile;
	}
