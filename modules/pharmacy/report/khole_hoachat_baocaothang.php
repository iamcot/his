<?php
    ...
    */
    ...
    # Window bar title
    $smarty->assign('Name',$LDReportUseMonth);
...
{
	urlholder="<?php echo $root_path;?>modules/pdfmaker/duoc/khole_baocaosudungHC.php<?php echo URL_APPEND; ?>&type=chemical&select_type="+select_type+"&monthreport="+month+"&yearreport="+year;
	testprintpdf=window.open(urlholder,"BaoCaoSuDungThuoc","width=1000,height=760,menubar=yes,resizable=yes,scrollbars=yes");
}
    function selectTypeChemical() {
            var temp_i = document.getElementById("type_chemical").selectedIndex;
            document.getElementById("select_type").value = document.getElementById("type_chemical").options[temp_i].value;
            document.reportform.action='<?php echo $thisfile; ?>';
            document.reportform.submit();			
    }
-->
...
<?php
    ...
    $temp='<select id="type_chemical" name="type_chemical" onchange="selectTypeChemical()">
    ...
    switch($select_type){	
        case 0: $listReport = $CabinetPharma->reportChemicalMonth('','',$monthreport,$yearreport); break;
        case 1: $listReport = $CabinetPharma->reportChemicalKPMonth($monthreport,$yearreport); break;
        case 2: $listReport = $CabinetPharma->reportChemicalBHMonth($monthreport,$yearreport); break;
	    case 3: $listReport = $CabinetPharma->reportChemicalCBTCMonth($monthreport,$yearreport); break;	
        default: $listReport = $CabinetPharma->reportChemicalMonth('','',$monthreport,$yearreport); break;
    }
...
if(is_object($listReport)){
	...
	for($k=0; $k<=$n; $k++){
		...
		if ($flag){
			...
			echo	'<td align="center">'.$rowReport['product_encoder'].'</td>';	//Ma hoa chat
			echo	'<td align="left">'.$rowReport['product_name'].'</td>';	//Ten hoa chat
		...
		if ($rowReport['pres_id']){
			if($resulttemp=$CabinetPharma->getTypeChemicalPres($rowReport['pres_id'])){
	...
	echo 	'<td>'.$listitem['pres_in'].'</td>';
	if ($listitem['pres_in']*$price>0)
	echo 	'<td>'.($listitem['pres_in']*$price).'</td>';  else echo '<td></td>';
	echo 	'<td>'.$listitem['pres_out'].'</td>';
	if ($listitem['pres_out']*$price>0)
	echo 	'<td>'.($listitem['pres_out']*$price).'</td>'; else echo '<td></td>';
	echo 	'<td>'.$listitem['use'].'</td>';
	if ($listitem['use']*$price>0)
	echo 	'<td>'.($listitem['use']*$price).'</td>'; else echo '<td></td>';
	echo 	'<td>'.$listitem['dest'].'</td>';
	if ($listitem['dest']*$price>0)
	echo 	'<td>'.($listitem['dest']*$price).'</td>'; else echo '<td></td>';		
	echo	'<th>'.$total.'</th>';	//Tong cong		
	echo	'<th>'.($total*$price).'</th>';		//Tien
	echo '</tr>';
	*/
