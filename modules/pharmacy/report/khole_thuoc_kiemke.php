...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDKhoLe.' :: '.$LDPharmaReportInventory.' '.$title1);

 # href for help button
 $smarty->assign('pbHelp',"javascript:gethelp('submenu1.php','$LDIssuePaper')");

 # Window bar title
 $smarty->assign('title',$LDPharmaReportInventory.' '.$title1);
...
{
	urlholder="<?php echo $root_path;?>modules/pdfmaker/duoc/khole_kiemkethuoc.php<?php echo URL_APPEND; ?>&type=medicine&select_type="+select_type;
...
<table border="0" cellSpacing="1" cellPadding="3" width="95%" bgColor="#C3C3C3">
	<tr bgColor="#EDF1F4">
		...
		<th rowspan="2"><?php echo $LDNuocSx; ?></th>
	...
	if ($search==''){
		...
		if ($listItem = $Product->SearchCatalogKhoLe($condition)){		
		...
		if ($total_pages>1)
			$listItem = $Product->ShowCatalogKhoLe($current_page, $number_items_per_page, $condition);
		...
		elseif (is_numeric($search))
			$condition=$dongtayy." AND product_lot_id LIKE '%".$search."%' ";
		else
			$condition=$dongtayy." AND (product_name LIKE '".$search."%' OR product_name LIKE ' ".$search."%' )";
		$condition .= $cond_typeput;
		
		$listItem = $Product->SearchCatalogKhoLe($condition);
	...
	if(is_object($listItem)){
		...
		$sTemp='<tr bgColor="#ffffff"><td colspan="13">'.$LDItemNotFound.'</td></tr>';
		echo $sTemp;
	}
