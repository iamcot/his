...
{
	urlholder="<?php echo $root_path;?>modules/pdfmaker/duoc/khole_kiemkeVTYT.php<?php echo URL_APPEND; ?>&type=medipot&select_type="+select_type;
...
<table border="0" cellSpacing="1" cellPadding="3" width="95%" bgColor="#C3C3C3">
	...
	if ($search==''){
		...
		if ($listItem = $Product->SearchCatalogMedipotKhoLe($condition)){		
		...
		if ($total_pages>1)
			$listItem = $Product->ShowCatalogMedipotKhoLe($current_page, $number_items_per_page, $condition);
		...
		$listItem = $Product->SearchCatalogMedipotKhoLe($condition);
		$breakfile = $thisfile;
	}
