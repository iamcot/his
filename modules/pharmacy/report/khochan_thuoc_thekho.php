...
switch($type){
	case 'tayy':
		$title1=$LDthuocTayY; break;
	case 'dongy': 
		$title1=$LDthuocDongY; break;
	default: $dongtayy = ''; $title1=$LDthuoc; break;
}
...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDKhoChan.' :: '.$LDThekho.' - '.$title1);
...
function printOut(select_type,fromdate,todate)
{
	...
	urlholder="<?php echo $root_path;?>modules/pdfmaker/duoc/khochan_thuoc_thekho.php<?php echo URL_APPEND; ?>&select_type="+select_type+"&typedongtay=<?php echo $type;?>"+"&encoder="+encoder+"&fromdate="+fromdate+"&todate="+todate;
...
function Medicine_AutoComplete(){
			...
			var includeScript =  "khochan_autocomplete_medicine.php?type=<?php echo $type; ?>";
...
$smarty->assign('name','<input type="text" id="medicine" name="medicine" size="35" value="'.$medicine.'" onFocus="Medicine_AutoComplete()"><div id="hint"></div>');
...
$smarty->assign('content','<input type="text" id="content" name="content" size="9" value="'.$content.'">&nbsp;');
$smarty->assign('LDunit',$LDUnit.': ');
$smarty->assign('unit','<input type="text" id="unit" name="unit" value="'.$unit.'" size="14">');
...
$listReport = $Pharma->Khochan_thuoc_thekho($encoder, $fromdate, $todate, $cond_typeput);
	//Lay x
...
$result_ton = $Pharma->Khochan_thuoc_tontruoc($encoder, $cond_typeput." AND toninfo.todate<'$ngaynhapxuat_gannhat' ");	
	if($result_ton!=false){
...
$tongnhap_trc=0;
$tongxuat_trc=0;
$result_nhapxuat = $Pharma->Khochan_thuoc_tongnhapxuat_theongay($encoder, $ngayton_trc, $fromdate, $cond_typeput, 0);
...
//echo $ton_trc.' '.$tongnhap_trc.' '.$tongxuat_trc;
$i=0;
$toncuoiky = $tondauky;
$Tong_ton=$tondauky; $Tong_nhap=0; $Tong_xuat=0; 
if(is_object($listReport)){
	...
	while($rowReport = $listReport->FetchRow())	{
		if($rowReport['manhap']>0){	//NHAP
			...
			echo '</tr>';
			$i++;
			$Tong_nhap += $rowReport['number_voucher'];
		}else{	//XUAT
			$toncuoiky -= $rowReport['number_voucher'];
			echo '<tr bgcolor="#ffffff">';
			echo 	'<td>'.@formatDate2Local($rowReport['ngay'],'dd/mm/yyyy').'</td>';	
			...
			echo '</tr>';
			$i++;		
			$Tong_xuat += $rowReport['number_voucher'];
		}
	}		
 ...
 $sTempDiv = $sTempDiv.'<tr bgColor="#ffffff">
							<td colspan="6" align="center"><b>'.$LDKetLuan.'</b></td>
							<td align="right"><b>'.$Tong_ton.'</b></td>
							<td align="right"><b>'.$Tong_nhap.'</b></td>
							<td align="right"><b>'.$Tong_xuat.'</b></td>
							<td align="right"><b>'.$toncuoiky.'</b></td>
							<td></td>
						</tr>';
