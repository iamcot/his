...
$fileforward='khochan_kiemkevtyt_save.php'.URL_APPEND;
$urlsearch=$thisfile;
...
if($todate!=''){
	$check_sql = "SELECT * FROM care_med_khochan_ton_info WHERE todate<='".$todate."' ORDER BY todate DESC ";	
...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDKhoChan.' :: '.$LDPharmaReportInventory.' VTYT');
...
{
	if(flag==true){
	urlholder="<?php echo $root_path;?>modules/pdfmaker/duoc/khochan_kiemkeVTYT.php<?php echo URL_APPEND; ?>&type=medicine&select_type="+select_type+"<?php echo '&todate='.$todate; ?>";
...
function ChotKiemKe(update_id){	
	var name=prompt("<?php echo $LDChotKiemKeVao.' '.$ngaydaux.' '.$LDDen.' '.$ngaycuoix.' '.$LDChoThang ?>","<?php echo $thangbaocao; ?>");
	if (name!=null)
	{
...
switch($select_type){	
	...
	case 1: $cond_typeput = ' AND sub.typeput=1 '; break;
	case 2: $cond_typeput = ' AND sub.typeput=0 '; break;
	case 3: $cond_typeput = ' AND sub.typeput=2 '; break;
	default: $cond_typeput = ' AND sub.typeput=1 ';
}	
...
<table border="0" cellSpacing="1" cellPadding="3" width="95%" bgColor="#C3C3C3">
	<tr bgColor="#EDF1F4">
		...
		<th rowspan="2"><?php echo $LDMedipotName; ?></th>
	...
	if ($search==''){
		...
		if($flag)
			$listItem = $Product->ShowKhoChanVTYT_Ton($current_page, $number_items_per_page, $condition, $todate);
		else
			$listItem = $Pharma->Khochan_vtyt_nhapxuatton_theongay($condition, $cond_typeput, $last_report_date, $todate);
		...
		if($flag)
			$listItem = $Product->SearchKhoChanVTYT_Ton($condition, $todate);
		else
			$listItem = $Pharma->Khochan_vtyt_nhapxuatton_theongay($condition, $cond_typeput, $last_report_date, $todate);
		$breakfile = $thisfile;
	}
