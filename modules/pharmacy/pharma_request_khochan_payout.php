...
if ($search==''){
	$condition=" WHERE status_finish='0' ".$health_station." AND pharma_type_pay_out='1' ORDER BY create_time";
}
else{
	if (strrpos($search,'/') || strrpos($search,'-')){
		$search = formatDate2STD($search,'dd/mm/yyyy');
		$condition=" WHERE status_finish='0'  ".$health_station." AND pharma_type_pay_out='1' AND create_time LIKE '".$search."%' ORDER BY create_time DESC";
	}
	else
		$condition=" WHERE status_finish='0' ".$health_station."  AND pharma_type_pay_out='1' AND put_in_id LIKE '%".$search."%' ORDER BY pay_out_id";
	$breakfile = $thisfile.URL_APPEND;
}
$list_report = $Pharma->listAllPayOut($condition);
...
if($batchrows && $report_id){
	...
	if($medicine_in_report = $Pharma->getDetailPayOutInfo($report_id)){
...
{ 
	...
	if (r==true) {
		document.form_test_request.action="includes/inc_khochan_payout_statusfinish.php?report_id="+ report_id+"&user_origin=<?php echo $user_origin; ?>";
		...
		<table border="0" width="950"> <!-- ***************     SEARCH      ***************    -->
			<tr>
				<td><font size="3" color="#5f88be"><b>
					<?php if($mode=='khole') echo $LDKhoLe.': '.$TypePutInMedicine; 
						else echo $LDKhoChan.': '.$TypepayoutMedicine; ?>
					</b></td>
				...
				<td align="center">
					<?php if(($edit || $read_form) && $medicine_count){ 
						if($report_show = $Pharma->getPayOutInfo($report_id))
							require('includes/inc_khochan_payout_medicine_in_list.php');
					} ?>
				</td>
			</tr>    <!-- *************************************************************************************    -->
			<tr><td>&nbsp;<br></td></tr>
