...
$list_report = $Cabinet->listAllDestroy($condition);
...
if($batchrows && $report_id){
	...
	if($medicine_in_report = $Cabinet->getDetailDestroyInfo($report_id)){
...
{ 
	...
	var r=confirm("<?php echo $LDIsMedipotDestroy; ?>");
	if (r==true) {
		document.form_test_request.action="includes/inc_destroy_medipot_statusfinish.php?report_id="+ report_id +"&user_origin=<?php echo $user_origin; ?>";
...
include('includes/inc_destroy_medipot_request_lister_fx.php');
...
?></td> <!-- ************************************************************************    -->
	...
	<form name="form_test_request" method="post" >
		<table border="0" width="850"> <!-- ***************     SEARCH      ***************    -->
			<tr>
				<td><font size="3" color="#5f88be"><b><?php echo $LDRequestMedipotDestroy; ?></b></td>
			...
			<tr>	<!-- ***************      HIEN THI NOI DUNG (MEDICINE) TRONG PHIEU LINH THUOC      ***************    -->
				<td align="center">
					<?php if(($edit || $read_form) && $medicine_count){ 
						if($report_show = $Cabinet->getDestroyInfo($report_id))
							require('includes/inc_destroy_medipot_in_list.php');
					} ?>
				</td>
			</tr>    <!-- *************************************************************************************    -->
			<tr><td>&nbsp;<br></td></tr>
