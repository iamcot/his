...
{
	var win = 'includes/khochan_putin_showdetail.php<?php echo URL_APPEND; ?>' +'&report_id=' + issue_id+"&type=medicine";
	myWindow=window.open( win , 'View Details' , 'height=500,width=800' );
	myWindow.focus();
}
-->
</script>
...
$smarty->assign('LDTotal',$LDTotalNumber2);
...
if ($search==''){
	...
	$total_items=$Pharma->countPutInItems($condition);
	...
	$listItem = $Pharma->listSomePutInSplitPage($current_page,$number_items_per_page,$condition);
		
}else{
	if (strrpos($search,'/') || strrpos($search,'-')){
		$search = formatDate2STD($search,'dd/mm/yyyy');
		$condition=" WHERE pharma_type_put_in='1' AND date_time LIKE '".$search."%' ORDER BY date_time DESC";
	}
	else $condition=" WHERE pharma_type_put_in='1' AND put_in_id LIKE '%".$search."%' ORDER BY put_in_id";	
		
	$listItem = $Pharma->listAllPutIn($condition);	
...
$smarty->assign('pbSearch','<a href="#"><input type="image" '.createComIcon($root_path,'Search.png','0','',TRUE).' onclick="search()" ></a>');
