...
<!--
    ...
    {
	var win = 'includes/khochan_putin_chemical_showdetail.php<?php echo URL_APPEND; ?>' +'&report_id=' + issue_id+"&type=chemical";
...
<?php
    ...
    $new_report_url='putin_chemical.php';
    ...
    if ($search==''){
            ...
            $condition=" WHERE chemical_type_put_in='2' ORDER BY put_in_id ";
            ...
            if (strrpos($search,'/') || strrpos($search,'-')){
                    ...
                    $condition=" WHERE chemical_type_put_in='2' AND date_time LIKE '".$search."%' ORDER BY date_time DESC";
            }
            else $condition=" WHERE chemical_type_put_in='2' AND put_in_id LIKE '%".$search."%' ORDER BY put_in_id";	
    ...
    if(is_object($listItem)){
        ...
        for ($i=0;$i<$listItem->RecordCount();$i++)
        {
            ...
            if ($rowItem['status_finish']){
                $editbutton="javascript:alert('".$LDCannotEditInputChem."')";
                $status_finish=$LDInPutOk; $temp1='check-r.gif';
            }
            else{
                $editbutton='putin_chemical.php'.URL_APPEND.'&putin_id='.$rowItem['put_in_id'].'&target=update';
            ...
            $sTemp=$sTemp.'<tr bgColor="#ffffff" >
                                <td align="center"><input OnClick=viewDetail('.$rowItem['put_in_id'].') type="image" '.createComIcon($root_path,'info3.gif','0','',TRUE).'></td>
