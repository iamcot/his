...
$fileforward='includes/khochan_payout_medipot_save.php'.URL_APPEND;
...
function searchMedicine(id_number)
{
	...
	var win = 'includes/khochan_search_medipot_payout.php?id_number='+id_number+'&typeput='+typeput;
...
{
	var process_file='includes/khochan_payout_autocomplete_medipot.php?mode=filldata';
	var name_med='medicine'+i;
...
$list_report_url='payout_medipot_list_medicine.php';
ob_start();
...
$smarty->assign('sTypePutInInput','<input type="text" value="'.$TypepayoutMedipot.'" class="input1" readonly>');
...
if(!isset($target) || ($target=='new') || ($target=='create')){
	...
	$smarty->assign('sTypePut','<select name="typeput" id="typeput" class="input1" ><option value="0">'.$LDBH.'</option><option value="1" selected>'.$LDNoBH.'</option><option value="2">'.$LDCBTC.'</option></select>');
		...
		$listReport = $Pharma->getDetailPayOutMedInfo($payout_id);
		if(is_object($listReport)){
			...
			$create_id= $rowReport['receiver'];
			$date_time= $rowReport['date_time'];
    ...
    $smarty->assign('sTypePut','<select name="typeput" id="typeput" class="input1" onChange="alertselected(this)"><option value="0" '.$flag1.'>'.$LDBH.'</option><option value="1" '.$flag2.'>'.$LDNoBH.'</option><option value="2" '.$flag3.'>'.$LDCBTC.'</option></select>');	
}
