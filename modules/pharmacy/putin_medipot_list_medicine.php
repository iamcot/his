...
{
	var win = 'includes/khochan_putin_medipot_showdetail.php<?php echo URL_APPEND; ?>' +'&report_id=' + issue_id+"&type=medipot";
...
$new_report_url='putin_medipot.php';
...
if ($search==''){
	...
	$condition=" WHERE pharma_type_put_in='2' ORDER BY put_in_id ";
		
	$total_items=$Pharma->countPutInMedItems($condition);
	...
	$listItem = $Pharma->listSomePutInMedSplitPage($current_page,$number_items_per_page,$condition);
		
}else{
	if (strrpos($search,'/') || strrpos($search,'-')){
		$search = formatDate2STD($search,'dd/mm/yyyy');
		$condition=" WHERE pharma_type_put_in='2' AND date_time LIKE '".$search."%' ORDER BY date_time DESC";
	}
	else $condition=" WHERE pharma_type_put_in='2' AND put_in_id LIKE '%".$search."%' ORDER BY put_in_id";	
		
	$listItem = $Pharma->listAllPutInMed($condition);	
	...
	if(is_object($listItem)){
		...
		for ($i=0;$i<$listItem->RecordCount();$i++)
		{
			...
			if ($rowItem['status_finish']){
				$editbutton="javascript:alert('".$LDCannotEditInputMed."')";
				$status_finish=$LDInPutOk; $temp1='check-r.gif';
			}
			else{
				$editbutton='putin_medipot.php'.URL_APPEND.'&putin_id='.$rowItem['put_in_id'].'&target=update';
			...
			$sTemp=$sTemp.'<tr bgColor="#ffffff" >
								<td align="center"><a href="#"><input type="image" '.createComIcon($root_path,'info3.gif','0','',TRUE).' onclick="viewDetail('.$rowItem['put_in_id'].')" ></a></td>
