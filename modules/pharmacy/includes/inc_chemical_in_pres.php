...
//$chemical_count = $medicine_in_pres->RecordCount()
...
<table border=0 cellpadding=5 width="100%">
	...
	<tr>
		<td colspan="4" align="center"><br>
			<table bgcolor="#EEEEEE" width=100% cellpadding="5">
				<tr bgColor="#E1E1E1">
					...
					<td align="center"><u><?php echo $LDChemicalName; ?></u></td>
		...
		<?php for($i=1;$i<=$chemical_count;$i++) { 			
				$chemical_pres = $chemical_in_pres->FetchRow();

				$tonkhochan = $Pres->findInventoryKhoChan_HoaChat($chemical_pres['product_encoder'],$pres_show['typeput']);
				$totalcostchemical = $chemical_pres['sum_number']*$chemical_pres['cost'];						
				$strtext = $chemical_pres['desciption']; //howtouse count totalunits/per
					$strtext = explode("/", $strtext);
					$split_desciption = explode(" ", $strtext[0]);								
							
			echo '<tr bgcolor="#ffffff">
					<td width="3%">'.$i.'.<input type="hidden" name="medicine_nr['.$i.']" value="'.$chemical_pres['nr'].'"></td>
					<td width="30%">
						<!-- Ten thuoc / lieu luong-->
						<b>'.$chemical_pres['product_name'].'</b>
					</td>
			...
			echo	'<!-- So luong, don gia, thanh tien -->
					<td align="center" width="10%"><b>
						'.$chemical_pres['sum_number'].'</b>
					</td>
					<td align="center" width="10%">
						<input type="text" size=3 name="receive['.$i.']" value="'.$chemical_pres['sum_number'].'" onFocus="startCalc('.$i.');" onBlur="stopCalc();" '.$readonly.'>	
					</td>
					<td align="center" width="6%"><b>
						'.$chemical_pres['note'].'</b>
					</td>
					<td align="right" width="12%">
						<input name="cost'.$i.'" type="text" size=1 value="'.$chemical_pres['cost'].'" style="text-align:center;border-color:white;border-style:solid;" readonly>
					</td>
					<td align="right" width="13%">
						<input name="sumcost'.$i.'" type="text" size=1 value="'.$totalcostchemical.'" style="text-align:center;border-color:white;border-style:solid;" readonly>
					</td>
				</tr>';
	...
	<tr>
		<td colspan="4" ><FONT SIZE=1  > <?php if($bill || $pres_show['status_bill']) echo $LDNoteChemicalBill; else echo $LDNoteChemicalPres; ?></td>
	</tr>
