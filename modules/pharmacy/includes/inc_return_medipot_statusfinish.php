...
{ 
	if($medicine = $Cabinet->getMedicineInReturn($nr)){
		...
		$Product->updateMedipotAvaiProduct($encoder, $available_product_id, $number,'+', $typeput);
		...
		$Cabinet->insertArchive($dept_nr, $ward_nr, $encoder, $available_product_id, '0', $number, 0, 0, 0, $report_id, 0, $user_accept, $typeput);
...
$patmenu="../pharma_request_medipot_return.php".URL_REDIRECT_APPEND."&target=$target&pid=".$_SESSION['sess_pid']."&user_origin=".$user_origin;
