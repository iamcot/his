...
<!-- Begin
function sendValue(name,text,unit){
	var i = '<?php echo $id_number; ?>';
	window.opener.document.getElementById('medicine'+i).value = name;
	var a=text.split("_");
	window.opener.document.getElementById('encoder'+i).value = a[0];
	window.opener.document.getElementById('cost'+i).value = a[1];				
	window.opener.document.getElementById('lotid'+i).value = a[2];
	window.opener.document.getElementById('exp'+i).value = a[3];
	window.opener.document.getElementById('unit'+i).value = unit;
	window.close();
...
function searchItem(){
	...
	xmlhttp.open("GET","khochan_search_medicine_payout_getdb.php?name_med="+name_med+"&name_ger_med="+name_ger_med+"&group_med="+group_med+"&supplier="+supplier+"&typeput="+typeput,true);
