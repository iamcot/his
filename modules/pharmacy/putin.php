...
$fileforward='includes/khochan_putin_save.php'.URL_APPEND;
...
# Title in the toolbar
 $smarty->assign('sToolbarTitle',$LDKhoChan.' :: '.$TypePutInMedicine);
 ...
 # Window bar title
 $smarty->assign('Name',$TypePutInMedicine);
...
{
	var win = 'includes/khochan_search_medicine.php?id_number='+id_number;
...
function Medicine_AutoComplete(i){
			...
			var includeScript = "includes/khochan_putin_autocomplete_medicine.php?mode=auto&k="+i;
			new Ajax.Autocompleter(name_med,"hint",includeScript, {
					method: 'get',
					paramName: 'search',
...
function setSelectionId(div,li) {
			...
			document.getElementById('cost'+k).value = my_format_number(temp_cost[0]);
...
{
	var process_file='includes/khochan_putin_autocomplete_medicine.php?mode=filldata';
	var name_med='medicine'+i;
      ...
      if (xmlhttp.readyState==4 && xmlhttp.status==200)
        {
			...
			document.getElementById('cost'+i).value = my_format_number(a[1]);			
...
$list_report_url='putin_list_medicine.php';
ob_start();
...
$smarty->assign('sTypePutInInput','<input type="text" value="'.$TypePutInMedicine.'" class="input1" readonly>');
...
//    $stmt->closeCursor();
    ...
    $smarty->assign('sType',$LDTypeInt);
		...
		$listReport = $Pharma->getDetailPutInInfo($putin_id);
	...
	$smarty->assign('sTotalInput','<input type="text" id="total_money" name="total_money" class="input1" value="'.number_format($rowReport['totalcost']/(1+$rowReport['vat']/100)).'"/>');
