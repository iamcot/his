<?php
    ...
    if(!$mode) {	//$mode='' : load all issuepaper	
            ...
            $list_issue = $IssuePaper->listAllIssuePaperChemical($condition);

            if(is_object($list_issue)){
                    $batchrows = $list_issue->RecordCount();
                    if($batchrows && (!isset($issue_id) || !$issue_id)){ 
    ...
    if($batchrows && $issue_id){
            ...
            if($chemical_in_pres = $IssuePaper->getAllChemicalInIssuePaper($issue_id)){
                    if($chemical_count = $chemical_in_pres->RecordCount()){
                        $edit_form=1;
                    }		
            }else{
                $mode='';
...
<!-- 
	...
	var r=confirm("<?php echo $LDGiveChemicalIssue; ?>");
	if (r==true) {
		document.form_test_request.action="includes/inc_issuepaper_chemical_statusfinish.php?issue_id="+ issue_id+"&radiovalue=<?php echo $radiovalue; ?>&user_origin=<?php echo $user_origin; ?>";
    ...
    ?>
            ...
            <td>
                <form name="form_test_request" method="post" onSubmit="return FinishPres(<?php echo $issue_id; ?>)">
                    <input type="image" <?php echo createLDImgSrc($root_path,'abschic.gif','0') ?>  title="<?php echo $LDFinishEntry; ?>"> 
                        <img <?php echo createLDImgSrc($root_path,'printout.gif','0') ?> alt="<?php echo $LDPrintOut; ?>" OnClick="printOut()" />
                            <p>
                                <table border=0 bgcolor="#000000" cellpadding=1 cellspacing=0>
                                    <tr>
                                        <td>
                                            ...
                                            <tr>	<!-- ***************      HIEN THI NOI DUNG (CHEMICAL) TRONG PHIEU LINH HOA CHAT      ***************    -->
                                                    <td align="center">
                                                            <?php if(($edit || $read_form) && $chemical_count){ 
                                                                    if($issue_show = $IssuePaper->getChemicalIssuePaperInfo($issue_id))
                                                                            require('includes/inc_chemical_in_issue.php');
                                                            } ?>
                                                    </td>
                                            </tr>
                                            <tr><td>&nbsp;<br></td></tr>
                            ...
                            <input type="image" <?php echo createLDImgSrc($root_path,'abschic.gif','0') ?>  title="<?php echo $LDFinishEntry; ?>"> 
                            <img OnClick="printOut()" <?php echo createLDImgSrc($root_path,'printout.gif','0') ?> alt="<?php echo $LDPrintOut; ?>">
            ...
            <td> 
                <table>
                    <tr>
                        <td>
                            <input type="radio" name="typeprespatient" id="all" value="1" onClick="RefreshList(this)" <?php if (!$typeSumDepot || $typeSumDepot=='all') echo 'checked'; ?>>
                                <?php echo $LDAllIssue; ?>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <input type="radio" name="typeprespatient" id="depot" value="2" onClick="RefreshList(this)" <?php if ($typeSumDepot=='depot') echo 'checked'; ?>>
                                <?php echo $LDDepotIssue; ?>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <input type="radio" name="typeprespatient" id="sum" value="3" onClick="RefreshList(this)" <?php if ($typeSumDepot=='sum') echo 'checked'; ?>>
                                <?php echo $LDSumIssue; ?>
                        </td>
                    </tr>
                </table>
                ...
                <table border="0" > <!-- ***************     SEARCH      ***************    -->
                    ...
                    <tr>
                        ...
                        <td align="right">
                            <input OnClick="search()" type="image" <?php echo createComIcon($root_path,'Search.png','0','',TRUE); ?> />
                        </td>
                    </tr>
                    <tr>
                        <td align="center" colspan="2">
                            <FONT size=1>
                                <?php echo $LDSearchIssueGuide; ?>
                            </FONT>
                        </td>
                    </tr>
                </table> 
