...
if($search&&isset($keyword)&&($keyword)){
	...
	if($editmode){
		...
		$encounter=&$lab_obj->searchLimitEncounterLabResults($suchword,$pagen->MaxCount(),$pgx,$oitem,$odir);
	...
	if($linecount=$lab_obj->LastRecordCount()){
		...
		if(isset($totalcount) && $totalcount){
			...
			if($editmode){
				...
				@$lab_obj->searchEncounterLabResults($suchword);
			}
	...
	while($zeile=$encounter->FetchRow()){
		...
		if($zeile['encounter_nr']!=$prev_nr){
			...
			if($editmode){ 
				...
				echo'
					<a href="labor_datalist_noedit_blood.php'.URL_APPEND.'&encounter_nr='.$zeile['encounter_nr'].'&noexpand=1&nostat=1&user_origin=lab"  title="'.$LDClk2See.'">
					<button onClick="javascript:window.location.href=\'labor_datalist_noedit_blood.php'.URL_REDIRECT_APPEND.'&encounter_nr='.$zeile['encounter_nr'].'&noexpand=1&nostat=1&user_origin=lab\'"><img '.createComIcon($root_path,'update2.gif','0','absmiddle',FALSE).' alt="'.$LDClk2See.'"><font size=1> '.$LDLabReport;
			}
