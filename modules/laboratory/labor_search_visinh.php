...
if($search&&isset($keyword)&&($keyword)){

	# Load the date formatter 
	include_once($root_path.'include/core/inc_date_format_functions.php');
    
	include_once($root_path.'include/care_api_classes/class_lab_blood.php');
	
	$lab_obj=new LabBlood();
	...
	if($editmode){
		# Search
		$encounter=&$lab_obj->searchLimitEncounterBasicInfo($suchword,$pagen->MaxCount(),$pgx,$oitem,$odir);  
	}else{
		$encounter=&$lab_obj->searchLimitEncounterLabVsinhResults($suchword,$pagen->MaxCount(),$pgx,$oitem,$odir);
	...
	if($linecount=$lab_obj->LastRecordCount()){
		...
		if($mode!='paginate') $_SESSION['sess_searchkey']=$suchword;
		...
		if(isset($totalcount) && $totalcount){
			...
			if($editmode){
				@$lab_obj->searchEncounterBasicInfo($suchword);
			}else{
				@$lab_obj->searchEncounterLabVsinhResults($suchword);
			}
...
function checkForm(v) {
	if((v.keyword.value=="")||(v.keyword.value==" ")){
		v.keyword.value=" ";
	...
	<tr bgcolor="#DDE1EC">
		...
		<ul>
			...
			<FORM  method="post" name="sform"
				onSubmit="return checkForm(this)"><B><?php echo $LDSearchWordPrompt ?></B></font>
			<p><font size=3> <INPUT type="text" name="keyword" size="20"
				maxlength="40" value="" onblur="ChangeCase(this);" ></font> <input
...
?>	
			...
			<table border=0 cellpadding=3 cellspacing=1>
				<tr class="wardlisttitlerow">
					<td><b>
	  <?php echo $pagen->makeSortLink('Mã xét nghiệm','encounter_nr',$oitem,$odir,$append);  ?></b></td>
					<td><b>
...
*/
	...
	while($zeile=$encounter->FetchRow()){
			...
			if($editmode){ 
				...
				echo'
					<a href="labor_result_visinh1.php'.URL_APPEND.'&pn='.$zeile['encounter_nr'].'&batch_nr='.$zeile['batch_nr'].'&noexpand=1&nostat=1&user_origin=lab&target=lab"  title="'.$LDClk2See.'">
					<button onClick="javascript:window.location.href=\'labor_result_visinh1.php'.URL_REDIRECT_APPEND.'&pn='.$zeile['encounter_nr'].'&batch_nr='.$zeile['batch_nr'].'&noexpand=1&nostat=1&user_origin=lab&target=lab\'"><img '.createComIcon($root_path,'update2.gif','0','absmiddle',FALSE).' alt="'.$LDClk2See.'"><font size=1> '.$LDLabReport;
			}
