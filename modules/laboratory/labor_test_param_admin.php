...
$sTempNew = '<a href="javascript:newParam()"><img '.createLDImgSrc($root_path,'add_chem.png','0').'></a>';	
...
$smarty->assign('sParamGroup',$sTemp. $parametergruppe[$paramName['name']] );
...
while($tg=$tgroups->FetchRow()){
	...
	if(isset($parametergruppe[$tg['id']])&&!empty($parametergruppe[$tg['group_id']])) $sTemp = $sTemp.$parametergruppe[$tg['id']];
		else $sTemp = $sTemp.$parametergruppe[$tg['name']];
	$sTemp = $sTemp.'</option>';
	$sTemp = $sTemp."\n";
}
$sTemp = $sTemp.'</select>';
