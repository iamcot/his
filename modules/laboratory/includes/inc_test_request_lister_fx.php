...
if($tracker>1)
{
   ...
   $requests->MoveFirst();
?>
<a href="<?php echo $thisfile.URL_APPEND."&target=".$target."&subtarget=".$subtarget."&pn=".$test_request['encounter_nr']."&batch_nr=".$test_request['batch_nr']."&user_origin=".$user_origin."&dept_nr=".$dept_nr."&ward_nr=".$ward_nr."&tracker=".($tracker-1); ?>"><img <?php echo createComIcon($root_path,'uparrowgrnlrg.gif','0','left',TRUE) ?> alt="<?php echo $LDPrevRequest ?>"></a>
<?php
}
if($tracker<$batchrows)
{
   ...
   $test_request=$requests->FetchRow();
?>
<a href="<?php echo $thisfile.URL_APPEND."&target=".$target."&subtarget=".$subtarget."&pn=".$test_request['encounter_nr']."&batch_nr=".$test_request['batch_nr']."&user_origin=".$user_origin."&dept_nr=".$dept_nr."&ward_nr=".$ward_nr."&tracker=".($tracker+1); ?>"><img <?php echo createComIcon($root_path,'dwnarrowgrnlrg.gif','0','right',TRUE) ?>  alt="<?php echo $LDNextRequest ?>"></a>
<?php
}
echo '<b>Ngày gửi yêu cầu</b>';
...
while($test_request=$requests->FetchRow())
{
  ...
  if($buf_date!=$send_date)
  {
	 ...
	 $enc_obj->loadEncounterData($test_request['encounter_nr']);
  	 ...
  	 $info = $result['name_last']. " " . $result['name_first'] . "<br>" . formatDate2Local($result['encounter_date'],$date_format) . "<br>" . $result['pid'];
  } 
