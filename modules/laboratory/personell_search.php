...
if (isset ( $target )) {
	switch ($target) {
		...
		case 'referrer_dr' :
			...
			$sql = "SELECT pn.nr AS '".PNID.".value', CONCAT(p.name_last, ' ',p.name_first) AS '".PNAME.".value', 0 AS use_frequency FROM care_personell AS pn, care_person AS p ,care_personell_assignment as cpa 
					WHERE (pn.pid = p.pid) AND (pn.nr=cpa.personell_nr) AND cpa.role_nr=17  AND cpa.status NOT IN('deleted')";
			if ($mode == 'search') {
				$sql .= " AND ((pn.nr $sql_LIKE '%$searchkey%') OR (CONCAT(p.name_last, ' ',p.name_first) $sql_LIKE '$searchkey%') OR (CONCAT(p.name_last, ' ', p.name_first) $sql_LIKE '%$searchkey%'))";
		...
		case 'doctor_nr' :
			$searchkey = str_replace(" ","%", $searchkey);
			$sql = "SELECT pn.nr AS '".$obj_val.".value', CONCAT(p.name_last, ' ',p.name_first) AS '".$obj_name.".value', 0 AS use_frequency FROM care_personell AS pn, care_person AS p,care_personell_assignment as cpa 
					WHERE (pn.pid = p.pid) AND (pn.nr=cpa.personell_nr) AND cpa.role_nr=17 AND cpa.status NOT IN('deleted') ";
			if ($mode == 'search') {
				$sql .= " AND ((pn.nr $sql_LIKE '%$searchkey%') OR (CONCAT(p.name_last, ' ',p.name_first) $sql_LIKE '$searchkey%') OR (CONCAT(p.name_last, ' ', p.name_first) $sql_LIKE '%$searchkey%'))";
				$sql .= " ORDER BY p.name_first DESC";
...
if ($linecount) {
	...
	for($i = 0; $i < sizeof ( $sql_value ); $i ++) {
		if (($mode != 'search') && ($limitselect && ($count == $quicklistmaxnr)))
			break;
		else
			$count ++;
