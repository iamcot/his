...
if($search&&isset($keyword)&&($keyword)){
	...
	if($editmode){
		...
		$encounter=&$lab_obj->searchLimitEncounterLabDhuyetResults($suchword,$pagen->MaxCount(),$pgx,$oitem,$odir);
	...
	if($linecount=$lab_obj->LastRecordCount()){
		...
		if(isset($totalcount) && $totalcount){
			...
			if($editmode){
				...
				@$lab_obj->searchEncounterLabDhuyetResults($suchword);
			}
...
*/
	...
	while($zeile=$encounter->FetchRow()){
			...
			if($editmode){ 
				...
				echo'
					<a href="labor_result_duonghuyet1.php'.URL_APPEND.'&pn='.$zeile['encounter_nr'].'&batch_nr='.$zeile['batch_nr'].'&noexpand=1&nostat=1&user_origin=lab"  title="'.$LDClk2See.'">
					<button onClick="javascript:window.location.href=\'labor_result_duonghuyet1.php'.URL_REDIRECT_APPEND.'&pn='.$zeile['encounter_nr'].'&batch_nr='.$zeile['batch_nr'].'&noexpand=1&nostat=1&user_origin=lab\'"><img '.createComIcon($root_path,'update2.gif','0','absmiddle',FALSE).' alt="'.$LDClk2See.'"><font size=1> '.$LDLabReport;
			}
