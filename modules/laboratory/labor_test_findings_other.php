							      ...
							      if($ergebnis=$enc_obj->Transact($sql))
									...
									//echo $sql;
									  ...
									  signalNewDiagnosticsReportEvent($findings_date);
										...
										/*$sql4="select other_type from care_test_request_other where batch_nr='".$batch_nr."'";
										$test=$db->execute($sql4);
										if($test->recordcount()){
										$stored_request=$test->fetchrow();
									}
										$other_types=explode("_",$stored_request['other_type']);
										
										foreach($other_types as $other_type){
										$sql1="select item_code from care_test_param_other where id='".$other_type."'";
		
										$temp=$db->execute($sql1);
										if($temp->recordcount()){
										$result=$temp->fetchrow();
										$sql2="select item_unit_cost from care_billing_item where item_code='".$result['item_code']."'";
										$buf=$db->execute($sql2);
										$buf->recordcount();
										$bill=$buf->fetchrow();
										$eComBill->createBillItem($pn, $result['item_code'],$bill['item_unit_cost'], 1, $bill['item_unit_cost'],date("Y-m-d G:i:s") );	
										}
										}*/
										//$eComBill->createBillItem($pn, '0003','12000', 1, '12000',date("Y-m-d G:i:s") );										
									   // exit;
								       }
								       else
								       {
