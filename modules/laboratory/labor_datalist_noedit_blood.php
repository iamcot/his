...
if($user_origin=='lab'||$user_origin=='lab_mgmt'){
  	...
  	if(isset($from) && $from=='input') $breakfile=$root_path.'modules/laboratory/labor_datainput_blood.php'.URL_APPEND.'&encounter_nr='.$encounter_nr.'&job_id='.$job_id.'&parameterselect='.$parameterselect.'&allow_update='.$allow_update.'&user_origin='.$user_origin;
		else $breakfile=$root_path.'modules/laboratory/labor_search_blood.php'.URL_APPEND;
}else{
...
if(!$encounter_nr) header("location:".$root_path."modules/laboratory/labor_search_blood.php?sid=$sid&lang=$lang");
...
if($encounter=&$enc_obj->getBasic4Data($encounter_nr)) {
	...
	if ($rows=$lab_obj->LastRecordCount()){
		...
		$lab_obj->getDBCache('blood_result_'.$encounter_nr.'_'.$modtime,$cache);
	...
	while (list($paramId,$encounterNr)=each($paramEnc)) {
		...
		for($i=0;$i<count($jIDArray);$i++) {	 
			if(array_key_exists($jIDArray[$i],$encounterNr)) {	
				...
				$cache .= "<td align=\"right\" class=\"" . $class ."\">eqweqwe</td>";
				$ptrack++;
				$columns++;
			}
		}
...
$lab_obj->deleteDBCache('blood_result_'.$encounter_nr.'_%');
# Save new cache data
$lab_obj->saveDBCache('blood_result_'.$encounter_nr.'_'.$modtime,$cache);
}
