...
function addelem(elem,hid,last,first,b,nr,nr_t,nr_d,nr_s,nr_s_t,nr_s_d,nr_o,nr_s_o,mode)
{
 
 
    if((nr!=nr_s) && (nr!=nr_d) && (nr!=nr_s_t) && (nr!=nr_s_d) && (nr!=nr_o) && (nr!=nr_s_o) ){  
    window.opener.document.forms[0].elements[elem].value+=' '+last+' '+first+'\n';
	window.opener.document.forms[0].elements[hid].value+=nr;
    }else{
        alert("<?php echo $LDAlert;?>");
        return false;
    }	
}
    ...
    while($row=$nurses->FetchRow()){
	      echo $row['personell_nr'];
             if ($mode=="a"){
                ...
                echo '
                <a href="#" onClick="addelem(\''.$mode.$elemid.'\',\'h'.$mode.$elemid.'\',\''.ucfirst($row['name_last']).'\',\''.ucfirst($row['name_first']).'\',\''.$row['date_birth'].'\',\''.$row['personell_nr'].'\',\''.$mode.'\')">
                <img ';
                echo createComIcon($root_path,'mans-gr.gif','0').'/>';
				
            }elseif($mode=="r"){
			
                echo '<a href="#" onClick="addelem(\''.$mode.$elemid.'\',\'h'.$mode.$elemid.'\',\''.ucfirst($row['name_last']).'\',\''.ucfirst($row['name_first']).'\',\''.$row['date_birth'].'\',\''.$row['personell_nr'].'\',\''.$mode.'\')">';
