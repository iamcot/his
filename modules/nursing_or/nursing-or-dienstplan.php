...
switch($retpath)
{
	...
	case "qview": $rettarget='nursing-or-shift-fastview.php'.URL_APPEND.'&hilitedept='.$dept_nr; break;
...
$aj_pnr=unserialize($a_pnr['ha'.$n]);	
	...
	if(sizeof($aj_pnr)>1){
		...
		echo '&nbsp;<a href="javascript:popinfo(\''.$aj_pnr['ha'.$n.'_'.$j].'\')">'.$ajelems['a'.$n.'_'.$j].'</a><br/>';
		
		}
	}else{
	echo '&nbsp;<a href="javascript:popinfo(\''.$aj_pnr['ha'.$n.'_0'].'\')">'.$ajelems['a'.$n.'_0'].'</a>';
	}
	echo'</td>
	<td height=5 '.$backcolor.'>';
	if(sizeof($aoj_pnr)>1){
		for($j=0;$j<sizeof($aoj_pnr);$j++){
		echo '&nbsp;<a href="javascript:popinfo(\''.$aoj_pnr['hao'.$n.'_'.$j].'\')">'.$aojelems['ao'.$n.'_'.$j].'</a><br/>';
		
		}
	}else{
	echo '&nbsp;<a href="javascript:popinfo(\''.$aoj_pnr['hao'.$n.'_0'].'\')">'.$aojelems['ao'.$n.'_0'].'</a>';
	}
	echo'</td>
	<td height=5 '.$backcolor.'>';
	if(sizeof($rj_pnr)>1){
		for($j=0;$j<sizeof($rj_pnr);$j++){
		echo '&nbsp;<a href="javascript:popinfo(\''.$rj_pnr['hr'.$n.'_'.$j].'\')">'.$rjelems['r'.$n.'_'.$j].'</a><br/>';
		
		}
	}else{
	echo '&nbsp;<a href="javascript:popinfo(\''.$rj_pnr['ra'.$n.'_0'].'\')">'.$rjelems['r'.$n.'_0'].'</a>';
	}
	echo'</td>
	</tr>';
	if ($wd==6)  $wd=-1;
