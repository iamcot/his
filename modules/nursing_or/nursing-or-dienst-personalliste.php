   ...
   str=document.getElementById("chucvu").value;
	...
	xmlhttp.onreadystatechange=function()
	  ...
	  if (xmlhttp.readyState==4 && xmlhttp.status==200)
		{
		document.getElementById("nurses").innerHTML=xmlhttp.responseText;
		}
	  }
	xmlhttp.open("GET","getnurses.php?chucvu_nr="+str+"&nr="+nr+"&dept_nr="+<?php echo $dept_nr; ?>,true);
	xmlhttp.send();
...
<table border=0  bgcolor="#6f6f6f" cellspacing=0 cellpadding=0>
  <tr>
	...
	<?php if ($row['chucvu_nr']==0){$chucvu='Không';}
		elseif($row['chucvu_nr']==7){$chucvu=$LDDieuDuongTruong;}
...
<?php
					...
					while($row=$search_result->FetchRow())
					{
                        ...
                        echo "</td>";	
						echo"<td >";
						echo'<select name="chucvu" id="chucvu" onclick="javascript:addnurses(\''.$row['nr'].'\',\''.$dept_nr.'\')">';
	echo'<option value="0" >Không</option>';
	
	$job_type=$pers_obj->getTypeChucVu(2);
	...
	echo'</select>
		...
		echo '
						<td id="nurses">&nbsp;
							<a href="nursing-or-list-add.php'.URL_APPEND.'&nr='.$row['nr'].'&dept_nr='.$dept_nr.'&mode=save&retpath='.$retpath.'&ipath='.$ipath.'&chucvu_nr=0" title="'.$LDAddDoctorToList.'">
