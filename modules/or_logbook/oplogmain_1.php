...
echo '
		...
		<a href="oplogmain_1.php?sid='.$sid.'&lang='.$lang.'&internok='.$internok.'&thisday='.$yesday.'&dept_nr='.$dept_nr.'&saal='.$saal.'" title="'.formatDate2Local($yesday,$date_format).'">
		&lt;&lt; '.$LDPrevDay.'</a></td>
...
?>
		...
		<nobr>
			...
			<form action="oplogmain_1.php" method="post" name="chgdept" onSubmit="return pruf(this)">
			<tr>
			<td>
			...
			<td>
				<!-- <select name="saal" size=1 onChange="syncSaal(this,document.chgdept.dept)"> -->
                            <input type="hidden" name="dept_nr"value="<?php echo '39'; ?>">
			<?php echo $LDOr.": " ?>
...
<?php
	echo '
        ...
        if($thisday!=$today){
                echo '
                        <a href="oplogmain_1.php?sid='.$sid.'&lang='.$lang.'&internok='.$internok.'&thisday='.$tomorow.'&dept_nr='.$dept_nr.'&saal='.$saal.'" title="'.formatDate2Local($tomorow,$date_format).'"><nobr>'.$LDNextDay.' &gt;&gt;</a>';
        }
