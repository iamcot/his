<?php
    ...
    $pday=date(d);
    if($keyword==1){
        header("Location:op-pflege-log-getpharma.php?sid=$sid&lang=$lang&winid=$winid&keyword=$inputdata&batch_nr=$batch_nr&dept_nr=$dept_nr&ward_nr=$ward_nr");
        exit();
    }
    switch($winid)
    {
        case 'pharma':
            $element='material_codedlist';
            //$maxelement=10;
            $quickid='pharma';
            //Lấy thuốc trong khoa (mã thuốc)
            $pharma_info=$pharma->getAllPharmaInDept($dept_nr,$ward_nr,ASC)." LIMIT 5";
            $quicklist=$db->Execute($pharma_info);
            break;
    ...
    if($mode=='save')
    {
        if($fileexist)
        {
            $prescription_id=$pharma->getInfoPrescription($batch_nr,$dept_nr,$ward_nr);
            ...
            if($use=='&' || $use==''){
                $pharma_chosed=$pharma->getPharmaInfo($ln,$dept_nr,$ward_nr);
                echo '<p align="center"><font size="3" color="red"><b>'.$LDWarningNumberPharma.' <i>'.$pharma_chosed['product_name'].'</i> '.$LDActive;
                echo '<br>'.$LDDbNoSave.'</b></font></p>';
            }else{
                if($delitem!="")
                {
                    ...
                    if((sizeof($location)-1)<=1){
                        ...
                        $edit_number=$pharma->UneditNumberPharmaInDept($ln,$use,$prescription_id['prescription_id'],$prescription_id['encounter_nr'],$dept_nr,$ward_nr,$number_use[0],$number_use[1],$price);
                        //insert log
                        $logs->writeline_his($_SESSION['sess_login_userid'], $thisfile, $pharma->getLastQuery(), date('Y-m-d H:i:s'));  
                        $sql="UPDATE care_pharma_prescription_info SET total_cost='".($prescription_id[total_cost]-($use*$price))."' WHERE dept_nr='".$dept_nr."' AND ward_nr='".$ward_nr."' AND encounter_nr=".$prescription_id['encounter_nr']." AND date_time_create='".$temp_1[date_request]."'";
                        if(!$ergebnis=$db->Execute($sql)){
                    ...
                    $edit_number=$pharma->editNumberPharmaInDept($ln,$use,$prescription_id['prescription_id'],$prescription_id['encounter_nr'],$dept_nr,$ward_nr,$price);
                    //insert log
                    $logs->writeline_his($_SESSION['sess_login_userid'], $thisfile, $pharma->getLastQuery(), date('Y-m-d H:i:s')); 
                    $t=0;
                    while($t<sizeof($edit_number)){
                        ...
                        $sql="UPDATE care_pharma_prescription_info SET total_cost='".($prescription_id[total_cost]+$cost[1])."' WHERE dept_nr='".$dept_nr."' AND ward_nr='".$ward_nr."' AND encounter_nr=".$prescription_id['encounter_nr']." AND date_time_create='".$temp_1[date_request]."'";
                        if(!$ergebnis=$db->Execute($sql)){
                            echo $sql."$LDDbNoSave<br>";
                        }
                        //insert log
                ...
                $sql="UPDATE $dbtable SET $element='".$result[$element]."',encoding='$result[encoding]', modify_id='$user'
                                WHERE batch_nr='$batch_nr'";
                if($ergebnis=$db->Execute($sql))
                {
                    $saveok=1;
...
<!--<div align="right">
    ...
    ?>
        <a href="<?php echo $root_path.'modules/or_logbook/op-pflege-log-getinfo_pharma.php'.URL_REDIRECT_APPEND.'&batch_nr='.$batch_nr.'&dept_nr='.$dept_nr.'&ward_nr='.$ward_nr.'&saal='.$room_now.'&winid=pharma&flag=close'?>">        
            <img <?php echo createLDImgSrc($root_path,'cancel.gif','0') ?>/>
        </a>
    <?php
        }
        ...
        <tr>
            ...
            <td>
                <table border=0 width=100% cellspacing=1>
                    <tr>
                        <td bgcolor="#cfcfcf" class="v13_n" colspan=6>&nbsp;
                            <font color="#ff0000">
                            <b>
                    ...
                    <?php
                        ...
                        {                            
                            $issue_paper=$pharma->getPharmaInfo($qlist['product_encoder'],$dept_nr,$ward_nr);
                            $detail=$pharma->getPharmaInfoDetail($qlist['product_encoder'],$dept_nr,$ward_nr);
                            echo '
                                <tr bgcolor="#ffffff">
                                <td class="v13" width=15%>'.$issue_paper['product_name'].'
                                </td>';
                            echo "                                
                            ...
                            $unit=$pharma->getunitPharma($qlist['product_encoder']);
                            $numbernow=$pharma->getnumberPharma($qlist['product_encoder'],$dept_nr,$ward_nr);
...
<form name="infoform" action="<?php echo 'op-pflege-log-getpharma.php?sid='.$sid.'&lang='.$lang.'&winid='.$winid.'&inputdata='.$inputdata.'&dept_nr='.$dept_nr.'$ward_nr'.$ward_nr;?>" method="post" onSubmit="return pruf(this)">
    <table border=0 width=100% bgcolor="#6f6f6f" cellspacing=0 cellpadding=0 >
        <tr>
            <td>
                    ...
                    <?php
                        ...
                        {    
                            ...
                            for($i=0;$i<$entrycount;$i++)
                            {
                                ...
                                $use=explode("+",$nbuf[$i]);
//                                $check_used=$pharma->getnumberPharma($elems[n],$dept_nr,$ward_nr);
                                ...
                                <tr bgcolor="#ffffff">
                                <td   class="v13" >
                                &nbsp;<a href="javascript:delete_item(\''.$i.'\',\''.$use[1].'\',\''.$elems[n].'\')"><img '.createComIcon($root_path,'delete2.gif','0').' alt="'.$LDDeleteEntry.'"></a>
                                </td>
                                <td   class="v13" >
        ...
        <tr>
                    ...
                    <tr>
                        <td  class="v12"  bgcolor="#ffffff" colspan=4 align=center>
                            <div>
                                <font size=3>
                                    <b><?php echo str_replace("~tagword~",$title,$LDSearchNewPerson.":</b></font><br/>(".$LDNoteSearch.")") ?>
                                <br/>
