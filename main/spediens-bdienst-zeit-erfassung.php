...
$dept=39;
$thisfile="spediens-bdienst-zeit-erfassung.php".URL_REDIRECT_APPEND."&retpath=op&encoder=";
...
for($i=0;$i<$maxelement;$i++)
			...
			echo $tg;
						...
						$dt=$_POST['date'.$i];
						$an=$_POST['standby_name'.$i];
						$av=$_POST['standby_start'.$i];
						$ab=$_POST['standby_end'.$i];
						$rn=$_POST['oncall_name'.$i];
						$rv=$_POST['oncall_start'.$i];
						$rb=$_POST['oncall_end'.$i];
						$op=$_POST['op_room'.$i];
						$dg=$_POST['diagnosis'.$i];
						$td=$_POST['report_nr'.$i];
						$en=$_POST['encoding'.$i];
						$hist=$_POST['history'.$i];
			...
			header("location:$thisfile&saved=1&dept=$dept&pmonth=$pmonth&pyear=$pyear&pday=$pday");
		 }// end of if(mode==save)
		 else
		 
		 {	$pday1=$pday+4;
			$sql="SELECT * FROM $dbtable WHERE  date>='$pyear-$pmonth-$pday' and date<='$pyear-$pmonth-$pday1'";//echo $sql;
...
function isgdatum(val,idx)
{
    ...
    //n?u th?m $sid s? b? l?i URI qu� d�i
    urlholder="getall.php<?php echo URL_APPEND ?>&elemid="+elem+"&mode="+mode+"";
...
$entries=sizeof($content)+1; $toggle=0;
...
<td valign=top>
	<input type=text  id="a'.$i.'" name="standby_name'.$i.'" value="'.$content[$i]['standby_name'].'" size=20 width=20 onKeyUp=newdata(\''.$i.'\') ><a href="javascript:popselect(\''.$i.'\',\'a\')"><img src="'.$root_path.'gui/img/common/default/l_arrowgrnsm.gif" ></a>
</td>
<td valign=top>
...
<td rowspan=2 valign=top>';
if($content[$i]['op_room']){
	echo '<input type=text name="op_room'.$i.'" size=3 value="'.$content[$i]['op_room'].'" onKeyUp=newdata(\''.$i.'\')>

</td>';
}else{
echo '<input type=text name="op_room'.$i.'" size=3 value="" onKeyUp=newdata(\''.$i.'\')>
...
<td valign=top>
	<input type=text id="r'.$i.'"name="oncall_name'.$i.'" value="'.$content[$i]['oncall_name'].'" size=20 width=20 onKeyUp=newdata(\''.$i.'\')><a href="javascript:popselect(\''.$i.'\',\'r\')"><img src="'.$root_path.'gui/img/common/default/l_arrowgrnsm.gif" ></a>
</td>
<td valign=top>
