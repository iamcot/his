...
$mark_v_offset=1; /* Vertical offset of the marking blocks */
	...
	ImageFilledRectangle($im,$left_border,1,($w-($hdiv*2)),($top_border-4),$pink);
	 ...
	 /* Write the info part */
	if($ttf_ok){
		ImageTTFText($im,12,0,$hdiv,15,$dark_pink,$arial,($LDHospitalName.': '.$LDCentralLab));
	}
	else{
   ...
   //echo $day_ones." ".$day_tens;
	...
	if($ttf_ok){
		ImageTTFText($im,6,0,($left_border+($hdiv*10)),($sy+5),$white,$arial,$LDBatchNumber);
	}else{
	...
	/* Write some text */
	if($ttf_ok){
		ImageTTFText($im,11,0,($left_border),($sy+10),$dark_pink,$arial,$LDMaterial.': ');
		$sx=$left_border+($hdiv*5);
		ImageTTFText($im,10,0,($sx),($sy+10),$purple,$arial,strtr($stored_request['material_note'],$translation_table));
	}else{
	...
	$sy+=$vdiv;
	if($ttf_ok){
		ImageTTFText($im,11,0,($left_border),($sy+10),$dark_pink,$arial,$LDDiagnosis.':');
		$sx=$left_border+($hdiv*5);
		ImageTTFText($im,10,0,($sx),($sy+10),$purple,$arial,strtr($stored_request['diagnosis_note'],$translation_table));
	}else{
	...
	$sy+=($vdiv*2);
	if($ttf_ok){
		ImageTTFText($im,8,0,($left_border),($sy+5),$dark_pink,$arial,$LDImmuneSupp);
	}else{
	...
	if($ttf_ok)
		ImageTTFText($im,8,0,($sx+12),($sy+7),$dark_pink,$arial,$LDYes);
	else
		doWrite($LDYes);
	...
	if($ttf_ok)
		ImageTTFText($im,8,0,($sx+12),($sy+8),$dark_pink,$arial,$LDNo);
	else
		doWrite($LDNo);
