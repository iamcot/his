...
$yunit_temp=260/6.5; // Unit of y coord per temperature value in pixels  nhiet do 260/6.5;
$yunit_bp=260/130; // Unit of y coord per blood pressure value in pixels  huyet ap 260/130;
/**
* Validate date value
*/
...
if($dblink_ok){	
	...
	$end=date('Y-m-d',mktime(0,0,0,$mo,$dy+6,$yr));  //$end=$dateshifter->shift_dates($start,-6,'d');

	$ok=false;
	$sql="SELECT msr_date,msr_time,value FROM $dbtable
						WHERE encounter_nr=$pn AND msr_type_nr=2 AND msr_date BETWEEN '$start' AND '$end'
...
$tabhi=260; # Height of graph chart in pixels
$tablen=700; # Total width of graph chart in pixels
$tabcols=$tablen/28; # Total number of vertical lines
$tabrows=$tabhi/26; # Total number of horizontal lines


header ('Content-type:image/PNG');

$im=@ImageCreateFromPNG($root_path.'main/imgcreator/datacurve2.png'); // Loads the ready made image (makes this routine faster)
...
for($n=0,$xof=0;$n<7;$n++,$xof+=$xoffs)
{
	...
	$date=date('Y-m-d',mktime(0,0,0,$mo,$dy+($n),$yr));	
#**************** begin of curve tracing  Blood Pressure***************
	...
	if($bprows){
    	for($i=0;$i<$bprows;$i++)
    	{
	    	...
	    	$oy2=(($bp['value'])-40)*$yunit_bp;$oy2=258-$oy2;
        	ImageArc($im,$ox2,$oy2,6,6,0,360,$text_red);
...
#**************** begin of curve tracing  Temperature***************
	...
	if($trows){
		for($i=0;$i<$trows;$i++){
		    ...
		    $ty2=(($bc['value'])-35)*$yunit_temp;$ty2=258-$ty2;
