...
//------
	...
	ImageCopy($im,$label,($lab_x1+4),8,0,0,$label_w,$label_h);
	 ...
	 /* Write the info part */	
	if($ttf_ok){
	ImageTTFText($im,16,0,$left_border,30,$dark_pink,$arial,$LDHospitalName);
	ImageTTFText($im,14,0,$left_border,55,$dark_pink,$arialb,$LDCentralLab);
	}
	else{
	...
	if($ttf_ok){
	ImageTTFText($im,9,0,19,95,$purple,$arial,$LDRoomNr);
	ImageTTFText($im,10,0,22,110,$dark_pink,$arial,$stored_request['room_nr']);
	}
	else{
	...
	if($ttf_ok){
	ImageTTFText($im,8,0,($sx),($sy),$purple,$arial,$LDSamplingTime);
	$sx+=($hdiv*5); /* advance to right */
	ImageTTFText($im,8,0,($sx),($sy),$purple,$arial,$LDDay);
	$sx+=($hdiv*2); /* advance to right */
	ImageTTFText($im,8,0,($sx),($sy),$purple,$arial,$LDMinutes);
	}
	else{
	...
	for($i=1;$i<8;$i++)
	{
	   if($day_names==$i)
	  {
		...
		else
	   {
		...
		if($ttf_ok){
			ImageTTFText($im,7,0,($sx+1),($sy),$dark_pink,$arial,$LDShortDay[$i]);
		}else{
   ...
   else
   {
    $hour_ones=$hour;
	   ...
	   if($hour_tens==20)
	  {
	   ...
	   $sx+=($hdiv*5); /* advance to right */
		if($ttf_ok){
			ImageTTFText($im,8,0,($sx),($sy+3),$purple,$arial,$LDHours);
		}else{
   ...
   $sx=$left_border;
	if($ttf_ok){
		ImageTTFText($im,8,0,($sx),($sy+8),$purple,$arial,$LDUrgent);
	}else{
 ...
 // $tdcount=0;
  ...
  for($i=0;$i<=$max_row;$i++) {
	...
	if($ttf_ok){
		ImageTTFText($im,10,0,($sx+$left_border),($sy+25),$dark_pink,$arial,strtr($stored_request['doctor_sign'],$translation_table));
		ImageTTFText($im,10,0,($sx+($hdiv*17)+4),($sy+25),$dark_pink,$arial,strtr($stored_request['notes'],$translation_table));
	}else{
	...
	if($ttf_ok){
		ImageTTFText($im,8,0,($sx+$left_border),($sy+10),$purple,$arial,$LDEmergencyProgram);
	}else{
	...
	$sy=$top_border+($vdiv*68)+8;
	if($ttf_ok){
		ImageTTFText($im,8,0,($sx+10),($sy+10),$purple,$arial,$LDPhoneOrder);
	}else{	
	...
	if($ttf_ok){
		ImageTTFText($im,7,90,($sx-8),($sy+($vdiv*5)-3),$dark_pink,$arial,$LDHematology);
	}else{	
	...
	if($ttf_ok){
		ImageTTFText($im,7,90,($sx-8),($sy+($vdiv*5)-3),$dark_pink,$arial,$LDCoagulation);
	}else{
	...
	if($ttf_ok){
		ImageTTFText($im,7,90,($sx-8),($sy+($vdiv*5)-3),$dark_pink,$arial,$LDUrine);
	}else{
	...
	if($ttf_ok){
		ImageTTFText($im,7,90,($sx-8),($sy+($vdiv*5)-3),$dark_pink,$arial,$LDSerum);
	}else{
	...
	for($i=23,$n=0;$i<42;$i+=6,$n++)
	{
	   ...
	   if($n==3)
	   {
		 ...
		 if($n>3){
			if($ttf_ok){
				ImageTTFText($im,7,90,($sx-15),($sy+($vdiv*5)-3),$dark_pink,$arial,$LDSerum);
			}else{	
